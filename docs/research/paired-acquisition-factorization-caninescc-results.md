...
- [Standalone repository](https://github.com/matthewvaishnav/paired-acquisition-factorization-caninescc)

Do not link to a study-package GitHub Pages PDF until that page is actually published. The repository link is the stable public entry point for the study package.
...
- Seeds: 901--905.
- Variants: `paired_reference` and `pathoalign_dep20` where `pathoalign_dep20` is the historical code identifier for the paired-acquisition neural factorization run.
...
Interpretation: Paired-Acquisition Neural Factorization reduced scanner identifiability by roughly 0.38 absolute while preserving same-region retrieval and improving cross-scanner cosine.
...
Factorization audit means for Paired-Acquisition Neural Factorization:
