# Canine PathoAlign crossfold identity-audit seed sweep

## Purpose

This benchmark audit tests whether the PathoAlign representation-separation pattern holds across folds 0--4 and seeds 911--915 on the external multi-scanner canine SCC paired-acquisition dataset.
...
| PathoAlign biological features | `results/external_multiscanner_caninescc/pathoalign_dinov2_crossfold/fold_{0..4}/runs/pathoalign_dep20_seed_{911..915}/projected_features.npz`, key `features` |
| PathoAlign acquisition features | `results/external_multiscanner_caninescc/pathoalign_dinov2_crossfold/fold_{0..4}/runs/pathoalign_dep20_seed_{911..915}/projected_features.npz`, key `acquisition_features` |
...
| PathoAlign biological features | 25 | 0.299955 | 0.014763 | 0.203190 | 0.959463 | 0.003424 | 0.998966 | 0.000391 | 0.835016 | 0.003196 | 180.615 | 1.185 | 0.000000 |
| PathoAlign acquisition features | 25 | 0.968407 | 0.005381 | 0.224040 | 0.031155 | 0.003593 | 0.420661 | 0.007702 | 0.308065 | 0.017155 | 47.355 | 0.513 | 0.000000 |

## Interpretation

Across 25 PathoAlign biological-branch runs, scanner-probe accuracy is 0.299955, far below raw DINOv2 at 0.878261 and far below the paired-reference projection at 0.745660. At the same time, same-region retrieval improves from 0.872547 in raw DINOv2 to 0.959463, and same-sample retrieval remains essentially saturated at 0.998966.

Across 25 PathoAlign acquisition-branch runs, scanner-probe accuracy is 0.968407 while region-level retrieval falls to 0.031155. This is the expected opposite pattern if PathoAlign separates scanner/acquisition information into the acquisition branch rather than merely deleting it.
...
The paired-reference projection improves biological retrieval relative to raw DINOv2, but it remains highly scanner-decodable. PathoAlign biological features retain or improve the biological retrieval gains while sharply reducing scanner decodability.

## Clean result statement

Across five folds and five seeds on an external paired-scanner canine SCC benchmark, PathoAlign suppresses scanner decodability in the biological branch while preserving biological retrieval, and retains scanner decodability in the acquisition branch.

Numerically, scanner-probe accuracy falls from 0.878261 in raw DINOv2 to 0.299955 in PathoAlign biological features, while region retrieval rises from 0.872547 to 0.959463. Conversely, PathoAlign acquisition features retain scanner-probe accuracy at 0.968407 while region retrieval falls to 0.031155.
