...
REQUIRED_COLUMNS = (
    ...
    "access_status",
    "license_status",
    "biological_unit_id_available",
    "block_id_available",
    "section_id_available",
    "preparation_condition_available",
    "preparation_batch_available",
    "scanner_device_id_available",
    "scanner_model_available",
    "scan_batch_available",
    "site_available",
    "post_preparation_workflow_available",
    "acquisition_order_available",
    "section_order_available",
    "section_distance_available",
    "paired_same_section_scans_available",
    "matched_serial_sections_available",
    "raw_wsi_available",
    "patch_coordinates_available",
    "immutable_source_id_available",
    "metadata_source",
    "evidence_status",
    "notes",
)
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if row["dataset_id"] in ids:
            raise RegistryError(f"duplicate_dataset_id:{row['dataset_id']}")
...
def missing_for(row: dict[str, str], requirements: dict[str, str]) -> list[str]:
    return sorted({reason for field, reason in requirements.items() if row[field] != "yes"})


def contrast_result(row: dict[str, str], requirements: dict[str, str], kind: str) -> dict[str, Any]:
    blockers = missing_for(row, requirements)
    if kind == "workflow" and row["site_available"] == "yes" and row["post_preparation_workflow_available"] != "yes":
        blockers.append("site_not_equivalent_to_workflow")
    if "factor_nesting_unresolved" in row["notes"]:
        blockers.append("factor_nesting_unresolved")
    if row["evidence_status"] in {"inferred", "unknown"}:
        blockers.append("metadata_inferred_not_verified")
    blockers = sorted(set(blockers))
    verified = row["evidence_status"] in VERIFIED_EVIDENCE
    status = "confirmatory_design_candidate" if not blockers and verified else (
        "candidate_discovery" if any(row[field] in {"yes", "partial"} for field in requirements) else "descriptive_inventory"
    )
    return {"status": status, "blocking_reasons": blockers}
...
def analyze_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    if row["access_status"] != "yes" or row["license_status"] != "yes":
        global_blockers.append("access_or_license_unresolved")
    if row["evidence_status"] in {"inferred", "unknown"}:
        global_blockers.append("metadata_inferred_not_verified")
    if "factor_nesting_unresolved" in row["notes"]:
        global_blockers.append("factor_nesting_unresolved")

    for result in (preparation, scanner, workflow):
        if global_blockers:
            result["blocking_reasons"] = sorted(set(result["blocking_reasons"] + global_blockers))
            if result["status"] == "confirmatory_design_candidate":
    ...
    all_blockers = sorted(set(global_blockers + preparation["blocking_reasons"] + scanner["blocking_reasons"] + workflow["blocking_reasons"]))
...
def analyze_registry(rows: list[dict[str, str]], input_sha256: str, source: str) -> dict[str, Any]:
    ...
    fingerprint_basis = stable_json(payload).encode("utf-8")
    payload["audit_fingerprint"] = sha256_bytes(fingerprint_basis)
    return payload
...
def render_report(result: dict[str, Any]) -> str:
    ...
    for candidate in result["candidates"]:
        lines.extend([
            ...
            "- Blocking reasons: " + (", ".join(f"`{x}`" for x in candidate["blocking_reasons"]) or "none"),
...
def base_test_row() -> dict[str, str]:
    ...
    row.update({
        ...
        "notes": "prospective_crossing_verified",
    })
    return row
...
def run_self_tests() -> dict[str, Any]:
    ...
    row = base_test_row(); row["block_id_available"] = "unknown"; result = analyze_row(row)
    assert result["preparation_contrast"]["status"] != "confirmatory_design_candidate"; passed.append("confirmatory_with_unknown_required_field_fails")

    row = base_test_row(); row["evidence_status"] = "inferred"; result = analyze_row(row)
    assert result["descriptive_readiness_tier"] != "confirmatory_design_candidate"; passed.append("inferred_metadata_cannot_support_confirmatory")

    row = base_test_row(); row["post_preparation_workflow_available"] = "no"; row["site_available"] = "yes"; result = analyze_row(row)
    assert "site_not_equivalent_to_workflow" in result["workflow_contrast"]["blocking_reasons"]; passed.append("site_only_does_not_satisfy_workflow")

    row = base_test_row(); row["scanner_device_id_available"] = "no"; row["scanner_model_available"] = "yes"; result = analyze_row(row)
    assert "missing_scanner_identity" in result["scanner_contrast"]["blocking_reasons"]; passed.append("scanner_model_without_device_pairing_is_not_same_section_support")

    row = base_test_row(); row["immutable_source_id_available"] = "no"; result = analyze_row(row)
    assert "missing_immutable_source_provenance" in result["scanner_contrast"]["blocking_reasons"]; passed.append("missing_immutable_source_id_blocks_scanner_confirmatory")

    row = base_test_row(); row["block_id_available"] = "no"; row["matched_serial_sections_available"] = "no"; result = analyze_row(row)
    assert "missing_block_identity" in result["preparation_contrast"]["blocking_reasons"]; passed.append("serial_sections_without_block_identity_block_preparation")

    row = base_test_row(); row["notes"] = "factor_nesting_unresolved"; result = analyze_row(row)
    assert "factor_nesting_unresolved" in result["blocking_reasons"]; passed.append("preparation_nested_in_scanner_is_not_confirmatory")

    row = base_test_row(); row["license_status"] = "unknown"; result = analyze_row(row)
    assert "access_or_license_unresolved" in result["blocking_reasons"]; passed.append("access_or_license_unknown_blocks_confirmatory")

    row = base_test_row(); row["preparation_batch_available"] = "partial"; result = analyze_row(row)
    assert result["descriptive_readiness_tier"] == "candidate_discovery"; passed.append("candidate_discovery_allows_partial_nonconfirmatory_metadata")

    row = base_test_row(); rows = [row]; digest = sha256_bytes(b"fixture"); first = stable_json(analyze_registry(rows, digest, "fixture")); second = stable_json(analyze_registry(rows, digest, "fixture"))
    assert first == second; passed.append("deterministic_output")
    ...
    with tempfile.TemporaryDirectory(prefix="metadata-readiness-custom-") as directory:
        path = Path(directory) / "fixture.csv"; write_fixture(path, [base_test_row()]); loaded, digest = load_registry(path); analyze_registry(loaded, digest, str(path))
    after = DEFAULT_REPORT.read_bytes() if DEFAULT_REPORT.exists() else b""
    assert original == after; passed.append("custom_input_does_not_modify_checked_report")
...
def main() -> int:
    ...
    result = analyze_registry(rows, digest, str(args.input))
    if args.format == "json":
