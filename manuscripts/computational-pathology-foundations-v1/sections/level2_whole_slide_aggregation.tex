...
\item \textbf{Canonical TransnnMIL} (\texttt{transnnmil.py}): a dual-branch
  ...
  multiple-instance learning \cite{ilse2018attention}), alongside
  cluster/attention baselines such as CLAM \cite{lu2021clam}. The repaired model fuses the
...
\item \textbf{Hierarchical spatial pooling} (\texttt{hierarchical\_pooling.py}):
  ...
  transformers (hierarchical MIL pooling has prior art, e.g.,
  H$^2$-MIL \cite{hou2022h2mil}). Implemented and tested; a synthetic-MIL
  ablation exists, and a
...
(\texttt{\seqsplit{tests/models/test\_transnnmil\_canonical\_fusion.py}}). The
incremental
...
\item \textbf{Pending:} repaired matched controlled reruns (the ablation plan
  in \texttt{\seqsplit{docs/results/panda-transnnmil-ablation-plan.md}}) are
  required
