...
The external paired-scanner study uses 44 canine cutaneous squamous-cell-carcinoma biological samples digitized on five scanners. After geometry qualification, 805 complete five-view regions are available. The evaluation uses five fixed descriptive tissue categories---Dermis, Epidermis, Inflammation/Necrosis, SCC, and Subcutis---with biological-sample-blocked folds, fit-only standardization, and same-region/same-sample exclusions in nearest-neighbour candidate pools.

\begin{table*}[t]
\centering
\caption{Five-fold canine SCC representation comparison. Category balanced accuracy is descriptive; lower scanner balanced accuracy and higher retrieval are favorable.}
\label{tab:canine}
\small
\begin{tabular}{lrrrr}
\toprule
