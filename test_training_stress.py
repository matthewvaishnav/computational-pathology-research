    ...
    for i, test_case in enumerate(test_cases):
        try:
            ...
            if expected == "should_fail" and not valid:
                ...
                results["failed"] += 1
                results["details"].append(f"❌ Test {i+1} - Unexpected result: valid={valid}, expected={expected}")
            ...
            try:
                ...
                if estimated_usage > available:
                    ...
                    if expected == "should_fail":
                        results["passed"] += 1
                        results["details"].append(f"✅ Memory scenario {available}MB/{batch_size}bs - Failed as expected")
                    else:
                        results["failed"] += 1
                        results["details"].append(f"❌ Memory scenario {available}MB/{batch_size}bs - Unexpected failure")
                else:
                    # Would succeed
                    if expected == "should_work":
                        results["passed"] += 1
                        results["details"].append(f"✅ Memory scenario {available}MB/{batch_size}bs - Worked as expected")
                    else:
                        results["failed"] += 1
                        results["details"].append(f"❌ Memory scenario {available}MB/{batch_size}bs - Unexpected success")
