...
class PathologyFederatedAggregator:
    ...
    def _validate_updates(self, client_updates: Dict[str, Dict[str, torch.Tensor]]) -> None:
        ...
        expected_shapes = {param_name: tensor.shape for param_name, tensor in first_params.items()}
    ...
    def calculate_expertise_weight(
        self, metadata: HospitalMetadata, cancer_type: CancerType
    ) -> float:
    ...
    def _cancer_specific_weights(
        ...
        for client_id, metadata in hospital_metadata.items():
            ...
            if (
                ...
                and metadata.hospital_type == HospitalType.TEACHING_HOSPITAL
            ):
                base_weight *= 1.2
    ...
    def attention_weighted_aggregation(
        self,
        client_updates: Dict[str, Dict[str, torch.Tensor]],
    ...
    def pathology_type_specific_aggregation(
        self,
        client_updates: Dict[str, Dict[str, torch.Tensor]],
        cancer_type: CancerType,
        hospital_metadata: Dict[str, HospitalMetadata],
    ...
    def _hormone_receptor_weighted_agg(
        self,
        client_updates: Dict[str, Dict[str, torch.Tensor]],
        hospital_metadata: Dict[str, HospitalMetadata],
    ) -> Dict[str, torch.Tensor]:
    ...
    def _weighted_average(
        self, client_updates: Dict[str, Dict[str, torch.Tensor]], weights: Dict[str, float]
    ) -> Dict[str, torch.Tensor]:
    ...
    def aggregate_updates(
        ...
        hospital_metadata: Dict[str, HospitalMetadata],
        slide_quality: Dict[str, SlideQuality],
        cancer_type: CancerType = CancerType.GENERAL,
    ) -> Dict[str, torch.Tensor]:
        """
