...
class NoiseCalibrator:
    ...
    def calibrate(
        ...
        """
        logger.info(f"Starting noise calibration: ε={config.target_epsilon}, δ={config.target_delta}")
        ...
        logger.info(f"Baseline accuracy: {baseline_accuracy:.4f}, target: {min_target_accuracy:.4f}")
        ...
        def objective(params):
            ...
            return self._evaluate_parameter_combination(
                noise_multiplier, clip_norm, train_data, validation_data, 
                config, min_target_accuracy
            )
        ...
        final_accuracy = self._evaluate_with_noise(
            optimal_noise_multiplier, optimal_clip_norm, 
            train_data, validation_data, config.calibration_rounds
        )
    ...
    def _evaluate_parameter_combination(
        ...
        evaluation = {
            ...
            "meets_privacy": epsilon_cost <= config.target_epsilon and delta_cost <= config.target_delta,
...
class AdaptiveNoiseCalibrator:
    ...
    def _adapt_parameters(
        ...
        elif accuracy_trend > 0.01 and epsilon_remaining > 0.5:  # Good performance, budget available
...
if __name__ == "__main__":
    ...
    for i, entry in enumerate(result.calibration_history[-5:]):  # Last 5 entries
        print(f"  {i+1}: σ={entry['noise_multiplier']:.3f}, "
    ...
    for round_num in range(1, 21):
        ...
        if round_num % 5 == 0:
            print(f"Round {round_num}: σ={noise_mult:.4f}, acc={accuracy:.4f}, "
    ...
    print(f"  Noise range: {summary['noise_multiplier_range'][0]:.4f} - {summary['noise_multiplier_range'][1]:.4f}")
