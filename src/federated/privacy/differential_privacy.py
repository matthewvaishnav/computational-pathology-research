...
class PrivacyBudget:
    ...
    def is_exhausted(self) -> bool:
        ...
        return (
            self.consumed_epsilon >= self.total_epsilon or self.consumed_delta >= self.total_delta
        )
    ...
    def consume(self, epsilon: float, delta: float) -> bool:
        ...
        if (
            ...
            or self.consumed_delta + delta > self.total_delta
        ):
            return False
...
class GradientNoiseGenerator:
    ...
    def __init__(
        self, privacy_params: PrivacyParameters, mechanism: str = "gaussian", device: str = "cpu"
    ):
        """
    ...
    def add_noise_to_gradients(
        self, gradients: Dict[str, torch.Tensor], clip_norm: Optional[float] = None
    ...
    def _add_gaussian_noise(self, gradient: torch.Tensor) -> torch.Tensor:
        ...
        noise = torch.normal(mean=0.0, std=noise_scale, size=gradient.shape, device=self.device)
...
class FederatedPrivacyManager:
    ...
    def __init__(
        ...
        self.privacy_budget = PrivacyBudget(total_epsilon=total_epsilon, total_delta=total_delta)
    ...
    def prepare_round(
        self, round_number: int, num_clients: int, target_epsilon_per_round: Optional[float] = None
        ...
        privacy_params = PrivacyParameters(
            epsilon=target_epsilon_per_round, delta=round_delta, sensitivity=self.sensitivity
        )

        # Create noise generator
        noise_generator = GradientNoiseGenerator(
            privacy_params=privacy_params, mechanism=self.mechanism
        )
    ...
    def privatize_gradients(
        self, round_number: int, client_gradients: Dict[str, Dict[str, torch.Tensor]]
        ...
        for client_id, gradients in client_gradients.items():
            ...
            noisy_gradients = noise_generator.add_noise_to_gradients(
                gradients, clip_norm=self.gradient_clip_norm
            )
            privatized_gradients[client_id] = noisy_gradients

        logger.debug(
            f"Privatized gradients for {len(client_gradients)} clients in round {round_number}"
        )
    ...
    def finalize_round(self, round_number: int) -> bool:
        ...
        # Consume privacy budget
        success = self.privacy_budget.consume(privacy_params.epsilon, privacy_params.delta)
...
def compute_privacy_amplification(
    epsilon: float, delta: float, sampling_rate: float, mechanism: str = "gaussian"
) -> Tuple[float, float]:
    """
