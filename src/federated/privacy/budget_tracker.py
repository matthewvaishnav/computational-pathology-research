...
class PrivacyBudgetTracker:
    ...
    def _check_budget_alerts(self) -> None:
        ...
        if alert_type:
            ...
            recent_alerts = [
                ...
                if a.alert_type == alert_type and time.time() - a.timestamp < 300  # 5 minutes
            ]
    ...
    def _generate_alert_message(self, alert_type: str, usage_rate: float, threshold: float) -> str:
        ...
        if alert_type == "exhausted":
            ...
            return (
                f"Warning: High privacy budget usage: {usage_rate:.1%} (threshold: {threshold:.1%})"
            )
        else:
    ...
    def get_usage_forecast(self, target_rounds: int) -> Dict:
        ...
        max_rounds_epsilon = (
            int(self.remaining_epsilon / avg_epsilon_per_round)
            if avg_epsilon_per_round > 0
            else float("inf")
        )
        max_rounds_delta = (
            int(self.remaining_delta / avg_delta_per_round)
            if avg_delta_per_round > 0
            else float("inf")
        )
        ...
        return {
            ...
            "remaining_epsilon_after": (
                self.remaining_epsilon - forecast_epsilon if feasible else None
            ),
    ...
    def analyze_composition(self) -> Dict:
        ...
        composition_efficiency = (
            self.consumed_epsilon / (total_rounds * self.total_epsilon / 100)
            if total_rounds > 0
            else 0
        )
        ...
        return {
            ...
            "average_epsilon_per_round": (
                self.consumed_epsilon / total_rounds if total_rounds > 0 else 0
            ),
            "average_delta_per_round": (
                self.consumed_delta / total_rounds if total_rounds > 0 else 0
            ),
        }
    ...
    def plot_budget_usage(self, save_path: Optional[str] = None) -> None:
        ...
        ax1.axhline(
            y=self.total_epsilon * (1 - self.warning_threshold),
            color="orange",
            linestyle="--",
            alpha=0.7,
        ...
        ax1.axhline(
            y=self.total_epsilon * (1 - self.critical_threshold),
            color="red",
            linestyle="--",
            alpha=0.7,
        ...
        ax2.axhline(
            y=self.total_delta * (1 - self.warning_threshold),
            color="orange",
            linestyle="--",
            alpha=0.7,
            label="Warning threshold",
        ...
        ax2.axhline(
            y=self.total_delta * (1 - self.critical_threshold),
            color="red",
            linestyle="--",
            alpha=0.7,
            label="Critical threshold",
...
if __name__ == "__main__":
    ...
    tracker = PrivacyBudgetTracker(
        total_epsilon=1.0, total_delta=1e-5, warning_threshold=0.7, critical_threshold=0.9
    )
    ...
    for mechanism, stats in composition.get("mechanism_breakdown", {}).items():
        print(
            f"  {mechanism}: {stats['rounds']} rounds, "
