...
class PathologyFLCoordinator:
    ...
    def _log_aggregation_results(
        self, hospital_metadata: Dict[str, HospitalMetadata], slide_quality: Dict[str, SlideQuality]
    ):
        ...
        for client_id, metadata in hospital_metadata.items():
            ...
            quality_weight = self.aggregator.calculate_quality_weight(slide_quality[client_id])
...
async def demo_pathology_fl():
    ...
    # Create config
    config = {"expertise_weight": 0.5, "quality_weight": 0.3, "num_rounds": 5}
    ...
    updated_model = await coordinator.federated_round(client_updates, slide_qualities, "breast")
