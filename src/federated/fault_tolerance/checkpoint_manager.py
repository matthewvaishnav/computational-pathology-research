...
class CheckpointManager:
    ...
    **Validates: Requirements 9.2, 9.4**
    ...
    def load_checkpoint(
        ...
        **Validates: Requirements 9.4**
        ...
        if round_id is not None:
            ...
            checkpoint_meta = next((c for c in reversed(self.checkpoints) if c.is_complete), None)
    ...
    def get_latest_checkpoint_metadata(self) -> Optional[CheckpointMetadata]:
        ...
        return next((c for c in reversed(self.checkpoints) if c.is_complete), None)
    ...
    def load_checkpoint_index(self) -> None:
        ...
        try:
            ...
            self.checkpoints = [CheckpointMetadata(**c) for c in index_data.get("checkpoints", [])]
    ...
    def _calculate_disk_usage(self) -> float:
        ...
        for checkpoint_meta in self.checkpoints:
            try:
                ...
                logger.debug(
                    f"Failed to get checkpoint size for {checkpoint_meta.checkpoint_path}: {e}"
                )
