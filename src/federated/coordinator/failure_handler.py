...
class ClientFailureHandler:
    ...
    def detect_client_failure(
        ...
        logger.warning(
            f"Client failure detected: {client_id} - {failure_type.value}: {error_message}"
        )
    ...
    def get_round_progress(self) -> Dict[str, any]:
        ...
        return {
            ...
            "completion_rate": (
                len(self.received_updates) / len(self.expected_clients)
    ...
    def get_failure_statistics(self) -> Dict[str, any]:
        ...
        avg_success_rate = (
            sum(metrics.success_rate for metrics in self.client_metrics.values()) / total_clients
        )
