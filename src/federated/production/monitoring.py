...
class FederatedLearningMonitor:
    ...
    def _setup_alerters(self) -> Dict[AlertChannel, Any]:
        ...
        if 'email' in alerting_config:
            ...
            if all(k in email_config for k in ['smtp_server', 'username', 'password', 'from_email', 'to_emails']):
    ...
    def _check_system_thresholds(self, metrics: SystemMetrics):
        ...
        if metrics.gpu_utilization and metrics.gpu_utilization > thresholds.get('gpu_utilization', 95):
            self.send_alert(Alert(
                severity=AlertSeverity.WARNING,
    ...
    def _cleanup_old_metrics(self):
        ...
        # Clean FL metrics
        self.fl_metrics_history = [
            m for m in self.fl_metrics_history 
            if m.timestamp > cutoff_time
        ]
    ...
    def get_recent_metrics(self, hours: int = 1) -> Dict[str, List[Dict[str, Any]]]:
        ...
        recent_fl = [
            m.to_dict() for m in self.fl_metrics_history 
            if m.timestamp > cutoff_time
        ]
        
        return {
            'system_metrics': recent_system,
            'fl_metrics': recent_fl
        }
