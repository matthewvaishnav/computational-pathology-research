...
class SecureAggregator(BaseAggregator):
    ...
    def handle_dropout(self, failed_clients: List[str]):
        ...
        remaining_clients = [c for c in self.expected_clients if c not in failed_clients]
        ...
        self.min_clients_required = max(2, int(len(remaining_clients) * self.dropout_threshold))
