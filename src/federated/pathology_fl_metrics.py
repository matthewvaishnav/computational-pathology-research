...
class PathologyFLMetrics:
    ...
    def record_round(self, round_number: int, hospitals: Dict[str, float], 
                    convergence: float, comm_time: float, agg_time: float):
    ...
    def get_efficiency_report(self) -> Dict:
        ...
        avg_comm_time = sum(m.communication_time for m in self.round_metrics) / len(self.round_metrics)
        avg_agg_time = sum(m.aggregation_time for m in self.round_metrics) / len(self.round_metrics)
        avg_hospitals = sum(m.participating_hospitals for m in self.round_metrics) / len(self.round_metrics)
