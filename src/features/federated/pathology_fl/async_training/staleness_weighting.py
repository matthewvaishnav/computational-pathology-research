    ...
    **Validates: Requirements 7.4, 7.7**
    ...
    def calculate_staleness(
        ...
        **Validates: Requirement 7.4**
        ...
        if staleness < 0:
            logger.warning(f"Update version {update_version} > current version {current_version}")
            return 0
    ...
    def calculate_weights(
        ...
        **Validates: Requirements 7.4, 7.7**
        ...
        if use_dataset_size:
            ...
            base_weights = {u.client_id: u.dataset_size / total_samples for u in updates}
        else:
            # Uniform weights
        ...
        if total_weight > 0:
            normalized_weights = {
                client_id: weight / total_weight for client_id, weight in staleness_weights.items()
            }
        else:
        ...
        logger.info(
            f"Calculated weights for {len(updates)} updates " f"(current_version={current_version})"
        )
    ...
    def get_statistics(self, updates: List[UpdateMetadata], current_version: int) -> Dict:
        ...
        staleness_values = [
            self.calculate_staleness(u.model_version, current_version) for u in updates
        ]
