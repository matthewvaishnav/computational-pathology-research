...
class ClientFailureHandler:
    ...
    def register_client(self, client_id: str) -> None:
        ...
        if client_id not in self.client_status:
            self.client_status[client_id] = ClientStatus(client_id=client_id, last_seen=time.time())
    ...
    def _handle_client_failure(self, client_id: str, error_msg: str) -> None:
        ...
        if status.consecutive_failures >= self.max_retries or not failure_event.recoverable:
    ...
    def _mark_client_failed(self, client_id: str) -> None:
        ...
        logger.error(
            f"Client {client_id} marked as failed after {status.consecutive_failures} attempts"
        )
    ...
    def check_client_timeouts(self) -> List[str]:
        ...
        for client_id in list(self.active_clients):
            ...
            if (
                ...
                and current_time - status.last_seen > self.timeout_seconds
            ):

                timed_out_clients.append(client_id)
                self._handle_client_failure(
                    client_id, f"Client timeout after {self.timeout_seconds}s"
                )
    ...
    def get_client_health_summary(self) -> Dict[str, Dict]:
        ...
        for client_id, status in self.client_status.items():
            summary[client_id] = {
                ...
                "last_failure_type": (
                    status.last_failure.failure_type.value if status.last_failure else None
                ),
            }
    ...
    def attempt_client_recovery(self, client_id: str) -> bool:
        ...
        if (
            status.last_failure
            and time.time() - status.last_failure.timestamp < self.recovery_cooldown
        ):
...
class FederatedRoundManager:
    ...
    async def execute_round(
        ...
        timeout_task = asyncio.create_task(self._monitor_timeouts(client_tasks))
            ...
            for client_id, task in client_tasks.items():
                try:
                    result = await asyncio.wait_for(
                        task, timeout=self.failure_handler.timeout_seconds
                    )
                    ...
                    self.failure_handler.report_client_response(client_id, False, "Task timeout")

                except Exception as e:
                    failed_clients.append(client_id)
                    self.failure_handler.report_client_response(client_id, False, str(e))
        ...
        can_continue = (
            self.failure_handler.can_continue_training() and success_rate >= min_success_rate
        )
...
if __name__ == "__main__":
    ...
    handler = ClientFailureHandler(timeout_seconds=30, max_retries=2, min_clients_threshold=2)
    ...
    for client_id, stats in health.items():
        print(
            f"  {client_id}: success_rate={stats['success_rate']:.2%}, "
