...
from src.features.federated.pathology_fl.aggregator.base import BaseAggregator
from src.features.federated.pathology_fl.aggregator.byzantine_robust import ByzantineDetector
...
class TrainingOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"TrainingOrchestrator initialized with {self.aggregator.algorithm_name} aggregation"
        )
    ...
    def unregister_client(self, client_id: str):
        ...
        logger.info(
            f"Client {client_id} unregistered. Total clients: {len(self.available_clients)}"
        )
    ...
    def select_clients(self, available_clients: Optional[List[str]] = None) -> List[str]:
        ...
        num_to_select = max(
            self.min_clients_per_round, int(len(available_clients) * self.client_selection_fraction)
        )
        ...
        logger.info(
            f"Selected {len(selected)}/{len(available_clients)} clients for round {self.current_round + 1}"
        )
    ...
    def collect_updates(
        self, client_updates: List[ClientUpdate], validate: bool = True
    ) -> List[ClientUpdate]:
        """
    ...
    def aggregate_updates(
        self, client_updates: List[ClientUpdate], apply_byzantine_detection: bool = True
        ...
        if apply_byzantine_detection and self.enable_byzantine_detection:
            ...
            byzantine_indices, clean_indices = self.byzantine_detector.detect_byzantine_clients(
                client_updates
            )
        ...
        logger.info(
            f"Aggregating {len(client_updates)} updates using {self.aggregator.algorithm_name}"
        )
    ...
    def update_global_model(
        self, aggregated_update: Dict[str, torch.Tensor], increment_version: bool = True
    ):
        """
    ...
    def complete_round(
        self, convergence_metrics: Optional[Dict[str, float]] = None, save_checkpoint: bool = True
    ):
        """
    ...
    def save_checkpoint(
        self, metrics: Optional[Dict[str, float]] = None, include_optimizer: bool = True
    ):
        """
    ...
    def _log_audit_event(
        self, event_type: str, client_id: Optional[str], round_id: int, details: Dict[str, any]
    ):
        """
