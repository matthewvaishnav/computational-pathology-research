...
class PrometheusMetricsExporter:
    ...
    def __init__(self, metrics_file: str = "./fl_metrics/prometheus.txt"):
        ...
        self.metric_metadata: Dict[str, Dict[str, str]] = {
            ...
            "fl_model_accuracy": {"type": "gauge", "help": "Global model accuracy"},
            "fl_model_loss": {"type": "gauge", "help": "Global model loss"},
            ...
            "fl_privacy_epsilon": {"type": "gauge", "help": "Current privacy budget (epsilon)"},
            "fl_gradient_norm": {"type": "gauge", "help": "L2 norm of aggregated gradients"},
            "fl_current_round": {"type": "gauge", "help": "Current training round number"},
            "fl_model_version": {"type": "gauge", "help": "Current global model version"},
        }
...
class ConvergenceDetector:
    ...
    def update(self, metrics: Dict[str, float], round_id: int) -> bool:
        ...
        if improved:
            self.best_value = current_value
            self.rounds_without_improvement = 0
            logger.info(f"Round {round_id}: {self.metric_name} improved to {current_value:.4f}")
        else:
...
class AlertGenerator:
    ...
    def __init__(
        self, alert_file: str = "./fl_alerts/alerts.jsonl", enable_console_alerts: bool = True
    ):
        """
    ...
    def check_byzantine_attack(
        self, num_detected: int, total_clients: int, round_id: int, threshold: float = 0.2
    ):
        """
    ...
    def check_privacy_budget(
        self, current_epsilon: float, budget_limit: float, client_id: str, round_id: int
    ):
        """
    ...
    def check_client_dropout(
        self, expected_clients: List[str], received_clients: List[str], round_id: int
    ):
        """
    ...
    def end_round(
        ...
        if num_byzantine > 0:
            ...
            self.alerts.check_byzantine_attack(
                num_detected=num_byzantine, total_clients=total_clients, round_id=round_id
            )
    ...
    def log_privacy_budget(self, round_id: int, epsilon: float, client_id: Optional[str] = None):
        ...
        if client_id:
            self.prometheus.record_metric(
                "fl_privacy_epsilon", epsilon, labels={"client_id": client_id}
            )
        else:
