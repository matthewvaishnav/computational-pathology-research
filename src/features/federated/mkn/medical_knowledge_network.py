...
class MedicalKnowledgeNetwork:
    ...
    def query_diagnostic_knowledge(self, query: Dict) -> Dict:
        ...
        for pattern_data in patterns:
            ...
            if pattern_symptoms:
                match_score = len(set(symptoms) & set(pattern_symptoms)) / len(
                    set(symptoms) | set(pattern_symptoms)
                )
            else:
                match_score = 0.0
        ...
        for disease_type, patterns in self.diagnostic_patterns.items():
            for pattern_data in patterns:
                if pattern_data.get("pattern_id") == pattern_id:
                    ...
                    new_accuracy = (
                        old_accuracy * (pattern_data["validation_count"] - 1) + accuracy
                    ) / pattern_data["validation_count"]
        ...
        for key in set(predicted.keys()) | set(actual.keys()):
            ...
            if key in predicted and key in actual:
                ...
                if isinstance(pred_val, (int, float)) and isinstance(actual_val, (int, float)):
                    ...
                    if (
                        abs(pred_val - actual_val) / max(abs(actual_val), 1.0) < 0.1
                    ):  # 10% tolerance
                        matches += 1
...
def test_mkn_knowledge_query():
    ...
    mkn.register_clinical_expert(
        "expert_1", {"clinical_experience_years": 15, "peer_review_rating": 4.2}
    )

    mkn.register_clinical_expert(
        "expert_2", {"clinical_experience_years": 25, "peer_review_rating": 4.7}
    )
    ...
    # Query knowledge
    query = {"disease_type": "lung_cancer", "symptoms": ["persistent_cough", "chest_pain"]}
...
def test_mkn_consensus_diagnosis():
    ...
    for expert_id in experts:
        mkn.register_clinical_expert(
            expert_id, {"clinical_experience_years": 12, "peer_review_rating": 4.0}
        )
    ...
    for i, expert_id in enumerate(experts):
        pattern = {"disease_type": "prostate_cancer", "predicted_outcome": patterns[i]}
