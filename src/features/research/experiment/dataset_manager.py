...
class DatasetOrganizer:
    ...
    def add_samples(self, dataset_id: str, sample_paths: List[Path]) -> int:
        ...
        self.update_dataset(dataset_id, num_samples=metadata.num_samples, size_gb=metadata.size_gb)
    ...
    def _calculate_detailed_stats(self, samples: List[Dict], dataset_id: str) -> DatasetStats:
        ...
        size_stats = {
            "small_percent": (
                size_distribution["small"] / total_samples * 100 if total_samples > 0 else 0
            ),
            "medium_percent": (
                size_distribution["medium"] / total_samples * 100 if total_samples > 0 else 0
            ),
            "large_percent": (
                size_distribution["large"] / total_samples * 100 if total_samples > 0 else 0
            ),
        ...
        quality_distribution = {
            "excellent": (
                len([q for q in quality_scores if q >= 0.9]) / total_samples * 100
                if total_samples > 0
                else 0
            ),
            "good": (
                len([q for q in quality_scores if 0.7 <= q < 0.9]) / total_samples * 100
                if total_samples > 0
                else 0
            ),
            "fair": (
                len([q for q in quality_scores if 0.5 <= q < 0.7]) / total_samples * 100
                if total_samples > 0
                else 0
            ),
            "poor": (
                len([q for q in quality_scores if q < 0.5]) / total_samples * 100
                if total_samples > 0
                else 0
            ),
        }
...
def organize_dataset(
    organizer: DatasetOrganizer, name: str, sample_paths: List[Path], **kwargs
) -> str:
