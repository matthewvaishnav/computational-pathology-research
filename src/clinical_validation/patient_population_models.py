...
class PatientPopulationGenerator:
    ...
    def __init__(self, random_seed: int = 42):
        ...
        self.risk_multipliers = {
            "smoking": {"lung_cancer": 15.0, "bladder_cancer": 3.0, "colon_cancer": 1.5},
            "family_history": {"breast_cancer": 2.0, "prostate_cancer": 2.5, "colon_cancer": 2.0},
            "age_elderly": {
                "breast_cancer": 3.0,
                "lung_cancer": 4.0,
                "prostate_cancer": 8.0,
            ...
            "obesity": {"breast_cancer": 1.3, "colon_cancer": 1.5, "endometrial_cancer": 2.0},
        }
    ...
    def generate_population_characteristics(
        self, site_id: str, population_type: str, total_patients: int
    ...
    def generate_patient_cohort(
        ...
        for i in range(cohort_size):
            ...
            income = self._sample_from_distribution(
                {k.value: v for k, v in characteristics.income_distribution.items()}
            )
            ...
            risk_scores = self._calculate_risk_scores(
                age, gender, ethnicity, smoking, family_history, diabetes, hypertension, obesity
            )
            ...
            if disease_focus:
                ...
                has_disease = np.random.random() < (
                    characteristics.disease_prevalence[disease_focus] / 100000
                )
                if has_disease:
                    symptom_duration = max(1, int(np.random.exponential(30)))  # Days
                    presentation_stage = self._sample_presentation_stage(
                        age, income, characteristics.screening_participation_rate
                    )
                    diagnostic_delay = max(
                        0, int(np.random.exponential(characteristics.diagnostic_delay_mean_days))
                    )
...
if __name__ == "__main__":
    ...
    for pop_type, pop_name in population_types:
        ...
        characteristics = generator.generate_population_characteristics(
            site_id=f"site_{pop_type}", population_type=pop_type, total_patients=10000
        )
        ...
        patients = generator.generate_patient_cohort(
            characteristics=characteristics, cohort_size=500, disease_focus="lung_cancer"
        )
        ...
        print(
            f"Age: {analysis['demographics']['age_stats']['mean']:.1f} ± {analysis['demographics']['age_stats']['std']:.1f}"
        )
        ...
        print(
            f"Smoking rate: {analysis['health_behaviors']['smoking_distribution'].get('current', 0):.1%}"
        )
