...
class ClinicalSubgroupAnalyzer:
    ...
    def analyze_clinical_subgroups(
        ...
        for clinical_col in clinical_columns:
            ...
            if (
                data[clinical_col].dtype in ["float64", "int64"]
                and len(data[clinical_col].unique()) > 10
            ):
        ...
        for r in range(2, min(len(intersect_columns) + 1, max_combinations + 1)):
            for combo in combinations(intersect_columns, r):
                ...
                data["_temp_combo"] = data[list(combo)].apply(
                    lambda x: " & ".join([f"{col}={val}" for col, val in zip(combo, x)]), axis=1
                )
    ...
    def _calculate_subgroup_metrics(
        self, predictions: np.ndarray, true_labels: np.ndarray, subgroup_name: str
    ) -> SubgroupMetrics:
    ...
    def _calculate_confidence_intervals(
        ...
        def proportion_ci(p: float, n: int) -> Tuple[float, float]:
            ...
            margin = (
                z_score * np.sqrt(p * (1 - p) / n + z_score**2 / (4 * n**2)) / (1 + z_score**2 / n)
            )
    ...
    def compare_subgroups(
        self, subgroup_metrics: List[SubgroupMetrics], metric_name: str = "accuracy"
    ...
    def detect_bias(
        ...
        bias_report = {"overall_bias_detected": False, "biased_variables": [], "bias_details": {}}
        ...
        for variable, metrics_list in subgroup_results.items():
            ...
            # Get metric values
            metric_values = [
                getattr(m, reference_metric)
                for m in metrics_list
                if getattr(m, reference_metric) is not None
            ]
            subgroup_names = [
                f"{m.subgroup_name}={m.subgroup_value}"
                for m in metrics_list
                if getattr(m, reference_metric) is not None
            ]
