...
class DataQualitySimulator:
    ...
    def simulate_image_quality_degradation(
        ...
        if np.random.random() < profile.compression_artifacts:
            degraded_image = self._add_compression_artifacts(degraded_image, profile.compression_artifacts)
    ...
    def simulate_annotation_quality(
        self, 
        annotations: Dict, 
        profile: QualityProfile
    ) -> Dict:
        ...
        if np.random.random() < profile.missing_data_rate:
            ...
            if 'labels' in degraded_annotations:
                keep_mask = np.random.random(len(degraded_annotations['labels'])) > profile.missing_data_rate
        ...
        for key, tensor in batch_data.items():
            if key == 'images' and tensor.dim() == 4:  # BCHW format
                ...
                for i in range(batch_size):
                    ...
                    degraded_img = self.simulate_image_quality_degradation(
                        tensor[i], profile
                    )
