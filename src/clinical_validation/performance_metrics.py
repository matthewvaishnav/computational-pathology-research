...
class ClinicalPerformanceAnalyzer:
    ...
    def calculate_metrics(
        ...
        if predictions_proba is not None:
            ...
            if predictions_proba.ndim > 1:
                proba = predictions_proba[:, 1] if predictions_proba.shape[1] == 2 else predictions_proba
            else:
            ...
            if len(np.unique(true_labels)) > 1:
                ...
                auc_roc_ci = self._bootstrap_ci(true_labels, proba, roc_auc_score, n_iterations=1000)
    ...
    def _wilson_ci(self, p: float, n: int) -> Tuple[float, float]:
        ...
        margin = self.z_score * np.sqrt(p*(1-p)/n + self.z_score**2/(4*n**2)) / (1 + self.z_score**2/n)
    ...
    def calculate_inter_rater_agreement(
        ...
        if rater_type == "binary":
            # Cohen's kappa
            agreement_metrics['cohens_kappa'] = cohens_kappa(
                np.column_stack([rater1, rater2])
            )[0]
            
            # Sensitivity agreement (both positive)
            both_positive = np.sum((rater1 == 1) & (rater2 == 1))
            total_positive = np.sum((rater1 == 1) | (rater2 == 1))
            agreement_metrics['sensitivity_agreement'] = both_positive / total_positive if total_positive > 0 else 0
            
            # Specificity agreement (both negative)
            both_negative = np.sum((rater1 == 0) & (rater2 == 0))
            total_negative = np.sum((rater1 == 0) | (rater2 == 0))
            agreement_metrics['specificity_agreement'] = both_negative / total_negative if total_negative > 0 else 0
    ...
    def generate_precision_recall_curve(
        ...
        ax.plot(recall, precision, color='darkorange', lw=2, label=f'PR curve (AP = {ap_score:.3f})')
    ...
    def generate_calibration_plot(
        ...
        ax2 = ax.twinx()
        ax2.hist(predictions_proba, bins=bins, alpha=0.3, color='gray', label='Prediction distribution')
    ...
    def generate_metrics_report(self, metrics: PerformanceMetrics) -> Dict[str, Any]:
        ...
        report = {
            ...
            'advanced_metrics': {
                ...
                'calibration_error': f"{metrics.calibration_error:.4f}" if metrics.calibration_error else "N/A",
                'brier_score': f"{metrics.brier_score:.4f}" if metrics.brier_score else "N/A"
            },
            'confidence_intervals': {
                'sensitivity': f"({metrics.sensitivity_ci[0]:.4f}, {metrics.sensitivity_ci[1]:.4f})" if metrics.sensitivity_ci else "N/A",
                'specificity': f"({metrics.specificity_ci[0]:.4f}, {metrics.specificity_ci[1]:.4f})" if metrics.specificity_ci else "N/A",
                'precision': f"({metrics.precision_ci[0]:.4f}, {metrics.precision_ci[1]:.4f})" if metrics.precision_ci else "N/A",
                'auc_roc': f"({metrics.auc_roc_ci[0]:.4f}, {metrics.auc_roc_ci[1]:.4f})" if metrics.auc_roc_ci else "N/A"
            },
            'sample_info': {
                'total_samples': metrics.n_samples,
                'positive_samples': metrics.n_positive,
                'negative_samples': metrics.n_negative,
                'prevalence': f"{metrics.n_positive / metrics.n_samples:.4f}" if metrics.n_samples > 0 else 0
            },
            'optimal_threshold': {
                'threshold': f"{metrics.optimal_threshold:.4f}" if metrics.optimal_threshold else "N/A",
