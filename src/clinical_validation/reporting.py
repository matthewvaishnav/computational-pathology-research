...
class PublicationReporter:
    ...
    def create_demographics_table(
        self, data: pd.DataFrame, demographic_columns: List[str], stratify_by: Optional[str] = None
    ) -> pd.DataFrame:
    ...
    def create_performance_table(
        self, metrics_dict: Dict[str, Dict[str, float]], include_ci: bool = True
    ) -> pd.DataFrame:
    ...
    def create_subgroup_table(
        self, subgroup_results: Dict[str, List[Dict[str, Any]]]
    ) -> pd.DataFrame:
    ...
    def create_bias_table(self, bias_results: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    def export_table_to_latex(self, df: pd.DataFrame, caption: str = "", label: str = "") -> str:
...
\\end{{table}}"""
    ...
    def export_table_to_html(self, df: pd.DataFrame, title: str = "") -> str:
    ...
    def create_summary_report(self, study_data: Dict[str, Any], output_format: str = "text") -> str:
    ...
    def generate_statistical_summary(self, test_results: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for result in test_results:
            row = {
                ...
                "Effect Size": (
                    f"{result.get('effect_size', 0):.4f}" if result.get("effect_size") else "N/A"
                ),
    ...
    def create_validation_summary(self, validation_results: Dict[str, Any]) -> str:
...
if __name__ == "__main__":
    ...
    # Create sample data
    sample_metrics = {
        "Model A": {"Accuracy": 0.92, "Sensitivity": 0.88, "Specificity": 0.94, "AUC-ROC": 0.95},
        "Model B": {"Accuracy": 0.89, "Sensitivity": 0.85, "Specificity": 0.91, "AUC-ROC": 0.92},
    }
    ...
    latex_table = reporter.export_table_to_latex(
        perf_table, caption="Model Performance Comparison", label="tab:performance"
    )
    print("LaTeX Export:")
    print(latex_table)
    print()
    ...
    study_data = {
        "overview": {"Total Patients": 1000, "Study Duration": "12 months", "Sites": 5},
        "demographics": {"Mean Age": "55 ± 12 years", "Female": "45%", "Disease Prevalence": "30%"},
        "primary_results": {"Sensitivity": 0.92, "Specificity": 0.94, "AUC-ROC": 0.95},
        "conclusions": [
