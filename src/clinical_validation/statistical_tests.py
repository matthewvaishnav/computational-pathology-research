...
from scipy.stats import (
    ttest_ind, ttest_rel, mannwhitneyu, wilcoxon, chi2_contingency,
    fisher_exact, mcnemar, kruskal, friedmanchisquare, shapiro,
    levene, bartlett, kstest, anderson
)
from statsmodels.stats.power import (
    ttest_power, zt_ind_solve_power, chisquare_power
)
...
class ClinicalStatisticalTests:
    ...
    def proportion_test(
        ...
        return TestResult(
            ...
            interpretation=self._interpret_effect_size(effect_size, "cohen_h") if effect_size else None
        )
    ...
    def equivalence_test(
        ...
        return TestResult(
            ...
            interpretation=f"Equivalent within ±{equivalence_margin}" if equivalent else "Not equivalent"
        )
    ...
    def non_inferiority_test(
        ...
        pooled_se = np.sqrt((np.var(treatment, ddof=1) / len(treatment)) + 
        ...
        return TestResult(
            ...
            interpretation=f"Non-inferior (margin: {non_inferiority_margin})" if non_inferior else "Inferiority not ruled out"
        )
    ...
    def power_analysis_t_test(
        ...
        if power is None and sample_size is not None:
            ...
            sample_size = int(np.ceil(zt_ind_solve_power(
...
if __name__ == "__main__":
    ...
    # Power analysis
    power_result = stats_tests.power_analysis_t_test(
