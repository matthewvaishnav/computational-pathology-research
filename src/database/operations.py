...
class UserOperations(BaseOperations):
    ...
    def create_user(
        self, username: str, email: str, password_hash: str, role: str = "pathologist"
    ) -> User:
        """Create a new user."""
...
class CaseOperations(BaseOperations):
    ...
    def get_case_statistics(self) -> Dict[str, Any]:
        ...
        status_counts = (
            self.session.query(Case.status, func.count(Case.id)).group_by(Case.status).all()
        )

        priority_counts = (
            self.session.query(Case.priority, func.count(Case.id)).group_by(Case.priority).all()
        )
