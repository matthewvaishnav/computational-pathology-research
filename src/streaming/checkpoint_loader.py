        ...
        try:
            ...
            if any('resnet' in k.lower() or 'layer' in k.lower() for k in state_dict.keys()):
                ...
                if config:
                    ...
                    pretrained = fe_config.get('pretrained', False)  # Don't use pretrained when loading from checkpoint
                else:
            ...
            # Check if it's a foundation model
            elif any('foundation' in k.lower() or 'encoder' in k.lower() for k in state_dict.keys()):
                ...
                logger.warning("Unknown feature extractor architecture, attempting to reconstruct from state dict")
                from src.models.feature_extractors import ResNetFeatureExtractor
