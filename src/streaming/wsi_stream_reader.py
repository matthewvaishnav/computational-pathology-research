...
from .format_handlers import get_wsi_handler, get_supported_wsi_formats, validate_wsi_format
...
class WSIStreamReader:
    ...
    def initialize_streaming(self) -> StreamingMetadata:
        ...
        try:
            ...
            logger.info(f"Streaming initialized: {dimensions} pixels, {estimated_patches} patches, format: {self.wsi_path.suffix}")
            ...
            for y in range(tiles_y):
                for x in range(tiles_x):
                    ...
                    try:
                        ...
                        # Convert to tensor
                        tile_array = np.array(tile_pil.resize((self.current_tile_size, self.current_tile_size)))
    ...
    def _create_tile_batch(self, tiles: List[torch.Tensor], coords: List[List[int]], 
                          batch_id: int, total_batches: int) -> TileBatch:
    ...
    def _adapt_to_memory_pressure(self):
        ...
        if self.memory_pressure:
            ...
            logger.info(f"Adapted to memory pressure: tile_size={self.current_tile_size}, buffer_size={self.buffer_size}")
