...
def create_mock_attention_model(
    feature_dim: int = 512, hidden_dim: int = 256, num_classes: int = 2, device: str = "cpu"
) -> nn.Module:
    """
    ...
    model = MockAttentionMIL(
        feature_dim=feature_dim, hidden_dim=hidden_dim, num_classes=num_classes
    )
    model = model.to(device)
    model.eval()
...
def create_mock_models(feature_dim: int = 512, device: str = "cpu") -> Tuple[nn.Module, nn.Module]:
    """
