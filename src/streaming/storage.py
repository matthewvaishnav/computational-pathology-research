...
class LocalStorage:
    ...
    def _write_hdf5(
        ...
        with h5py.File(filepath, "w") as f:
            ...
            f.create_dataset(
                "features",
                data=features,
                compression=self.config.feature_compression if self.config.enable_compression else None,
                compression_opts=self.config.compression_level if self.config.enable_compression else None,
            )
    ...
    def _read_pytorch(self, filepath: str) -> Dict[str, Any]:
        ...
        data = {
            "features": loaded["features"].numpy()
        }
...
class CloudStorage:
    ...
    def _initialize_azure(self) -> None:
        ...
        try:
            ...
            logger.info("Azure Blob Storage client initialized: container=%s", self.config.cloud_bucket)
        except ImportError:
