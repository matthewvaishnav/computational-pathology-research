...
class PACSWSIStreamingClient:
    ...
    def __init__(
        ...
        # Init PACS adapter
        self.pacs_adapter = PACSAdapter(config_profile=pacs_config_profile, ae_title=ae_title)
        ...
        logger.info(
            f"Init PACSWSIStreamingClient: profile={pacs_config_profile}, "
            f"max_retries={max_retries}"
        )

    @retry_with_backoff(max_retries=3, base_delay=1.0, max_delay=60.0)
    def query_wsi_studies(
        self, patient_id: Optional[str] = None, max_results: int = 100
    ) -> list[StudyInfo]:
        ...
        studies, result = self.pacs_adapter.query_studies(
            patient_id=patient_id, modality="SM", max_results=max_results  # Slide Microscopy
        )
    ...
    def retrieve_and_stream_wsi(
        self, study_uid: str, tile_size: int = 1024, batch_size: int = 32, resume: bool = True
    ) -> tuple[Optional[WSIStreamReader], Optional[PACSWSIMetadata]]:
        ...
        while retries <= self.max_retries:
            ...
            try:
                result = self.pacs_adapter.retrieve_study(
                    study_instance_uid=study_uid, destination_path=dest_path
                )
                ...
                if (
                    "network" not in result.message.lower()
                    and "timeout" not in result.message.lower()
                    and "connection" not in result.message.lower()
                ):
                    return result
    ...
    def _create_stream_reader(
        self, wsi_path: Path, study_uid: str, retrieval_time: float
    ) -> tuple[WSIStreamReader, PACSWSIMetadata]:
        """Create WSI stream reader from file."""
        try:
            # Create streaming reader
            reader = WSIStreamReader(wsi_path=str(wsi_path), tile_size=1024, batch_size=32)
    ...
    def query_series_for_study(self, study_uid: str) -> list[SeriesInfo]:
        ...
        try:
            series, result = self.pacs_adapter.query_series(study_instance_uid=study_uid)
    ...
    ) -> tuple[Optional[Path], Optional[PACSWSIMetadata]]:
    ...
    def add_to_worklist(
        self, study: StudyInfo, priority: str = "ROUTINE", assigned_to: Optional[str] = None
    ) -> WorklistEntry:
    ...
    def update_worklist_status(self, study_uid: str, status: str):
    ...
    def deliver_result_to_pacs(self, result: AnalysisResult) -> bool:
        ...
        logger.info(
            f"Deliver result to PACS: study={result.study_uid}, "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "worklist_by_status": {
                ...
                "IN_PROGRESS": len(
                    [e for e in self.worklist.values() if e.status == "IN_PROGRESS"]
                ),
