...
async def simulate_wsi_processing(
    ...
    try:
        ...
        logger.info(
            f"Processing complete! Final confidence: {dashboard_state.current_confidence:.3f}"
        )
...
async def run_error_demo():
    ...
    try:
        ...
        for i in range(5):
            await update_dashboard_status(
                patches_processed=i * 100, total_patches=1000, confidence=0.5 + i * 0.05
            )
...
async def run_parameter_update_demo():
    ...
    for i in range(10):
        await update_dashboard_status(
            patches_processed=i * 100, total_patches=1000, confidence=0.5 + i * 0.04
        )
...
def main():
    ...
    parser.add_argument(
        "--patches", type=int, default=1000, help="Total patches to process (single mode)"
    )
    parser.add_argument("--batch-size", type=int, default=64, help="Batch size (single mode)")
    parser.add_argument(
        "--time", type=float, default=30.0, help="Processing time in seconds (single mode)"
    )
    ...
    if args.mode == "single":
        asyncio.run(
            simulate_wsi_processing(
                total_patches=args.patches, batch_size=args.batch_size, processing_time=args.time
            )
        )
