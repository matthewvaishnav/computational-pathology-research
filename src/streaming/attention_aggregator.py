...
class StreamingAttentionAggregator:
    ...
    def __init__(self, 
        ...
        logger.info(f"StreamingAttentionAggregator initialized with threshold={confidence_threshold}")
    
    def update_features(self, 
                       new_features: torch.Tensor, 
                       coordinates: np.ndarray) -> ConfidenceUpdate:
        ...
        if self.accumulated_features is None:
            ...
            self.accumulated_features = torch.cat([self.accumulated_features, new_features.cpu()], dim=0)
    ...
    def is_confident_enough(self) -> bool:
        ...
        return (current_confidence >= self.confidence_threshold and 
                self.num_patches >= self.min_patches_for_confidence)
