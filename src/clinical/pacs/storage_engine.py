...
from .data_models import (
    PACSEndpoint, AnalysisResults, OperationResult, ValidationResult,
    DetectedRegion, DiagnosticRecommendation
)
...
class StructuredReportBuilder:
    ...
    def build_measurement_report(
        ...
        sr_dataset = self._create_base_sr_dataset(
            analysis_results, original_study_uid
        )
    ...
    def add_ai_algorithm_identification(
        ...
        algorithm_seq.AlgorithmParameters = f"Confidence threshold: {analysis_results.confidence_score}"
    ...
    def _add_image_references(
        ...
        ref_image = Dataset()
        ref_image.ReferencedSOPClassUID = "1.2.840.10008.5.1.4.1.1.77.1.6"  # VL Whole Slide Microscopy
    ...
    def _build_content_sequence(
        ...
        algo_item.TextValue = f"{analysis_results.algorithm_name} v{analysis_results.algorithm_version}"
    ...
    def _execute_c_store(self, endpoint: PACSEndpoint, sr_dataset: Dataset) -> OperationResult:
        ...
        try:
            ...
            if status.Status == 0x0000:  # Success
                logger.info(f"C-STORE completed successfully for SOP UID: {sr_dataset.SOPInstanceUID}")
            ...
            return OperationResult.error_result(
                operation_id=operation_id,
                message=f"C-STORE failed: {str(e)}",
                errors=[str(e)]
            )
