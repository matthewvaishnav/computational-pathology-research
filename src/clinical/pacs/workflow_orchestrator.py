...
class WorkflowOrchestrator:
    ...
    def process_new_studies(
        ...
        def submit_next_study() -> None:
            ...
            future = self._processing_executor.submit(
                self._process_single_study, next_study
            )
    ...
    def handle_processing_failure(
        ...
        self._failed_studies[study_uid] = {
            "error": str(error),
            "timestamp": datetime.now(),
            "recovery_attempts": self._failed_studies.get(study_uid, {}).get("recovery_attempts", 0) + 1
        }
    ...
    def _process_single_study(self, study: StudyInfo) -> OperationResult:
        ...
        try:
            ...
            retrieval_result = self.pacs_adapter.retrieve_study(
                ...
                destination_path=f"./data/retrieved/{study_uid}"
            )
