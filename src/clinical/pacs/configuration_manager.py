...
from .data_models import (
    PACSConfiguration, PACSEndpoint, SecurityConfig, PerformanceConfig,
    PACSVendor, ValidationResult, OperationResult
)
        ...
        if not config_file.exists():
            ...
            if encrypted:
                ...
                if fallback_file.exists():
                    ...
                    encrypted = False
                    logger.warning(f"Encrypted config not found, using unencrypted: {fallback_file}")
                else:
        ...
        try:
            ...
            for endpoint_id, endpoint_data in endpoints_data.items():
                ...
                security_config = SecurityConfig(
                    ...
                    certificate_path=Path(sec_data["certificate_path"]) if sec_data.get("certificate_path") else None,
                    client_cert_path=Path(sec_data["client_cert_path"]) if sec_data.get("client_cert_path") else None,
                    client_key_path=Path(sec_data["client_key_path"]) if sec_data.get("client_key_path") else None,
                    ca_bundle_path=Path(sec_data["ca_bundle_path"]) if sec_data.get("ca_bundle_path") else None,
        ...
        for endpoint_id, endpoint in config.pacs_endpoints.items():
            endpoint_dict = {
                ...
                "security_config": {
                    ...
                    "certificate_path": str(endpoint.security_config.certificate_path) if endpoint.security_config.certificate_path else None,
                    "client_cert_path": str(endpoint.security_config.client_cert_path) if endpoint.security_config.client_cert_path else None,
                    "client_key_path": str(endpoint.security_config.client_key_path) if endpoint.security_config.client_key_path else None,
                    "ca_bundle_path": str(endpoint.security_config.ca_bundle_path) if endpoint.security_config.ca_bundle_path else None,
