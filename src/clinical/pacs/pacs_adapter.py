...
class PACSAdapter:
    ...
    def query_studies(
        ...
        try:
            ...
            error_result = OperationResult.error_result(
                operation_id=operation_id, message=f"Query failed: {str(e)}", errors=[str(e)]
            )
            return [], error_result
    ...
    def reload_configuration(self, new_profile: Optional[str] = None) -> OperationResult:
        ...
        try:
            ...
            return OperationResult.error_result(
                operation_id=operation_id, message=f"Reload failed: {str(e)}", errors=[str(e)]
            )
    ...
    def get_adapter_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_endpoint": (
                self.configuration.get_primary_endpoint().endpoint_id
                if self.configuration.get_primary_endpoint()
                else None
            ),
