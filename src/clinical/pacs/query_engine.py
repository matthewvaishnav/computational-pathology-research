...
from .data_models import (
    StudyInfo, SeriesInfo, PACSEndpoint, ValidationResult, 
    OperationResult, DicomPriority, QueryResult
)
...
class QueryEngine:
    ...
    def query_studies(
        ...
        try:
            ...
            if not assoc.is_established:
                raise ConnectionError(f"Failed to establish association with {endpoint.host}:{endpoint.port}")
    ...
    def _parse_study_response(self, response_ds: Dataset, endpoint: PACSEndpoint) -> Optional[StudyInfo]:
        ...
        try:
            ...
            try:
                study_date = datetime.strptime(study_date_str, "%Y%m%d") if study_date_str else datetime.now()
            except ValueError:
    ...
    def _apply_post_query_filters(
        ...
        if date_range:
            start_date, end_date = date_range
            filtered = [
                study for study in filtered
                if start_date <= study.study_date <= end_date
            ]
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_associations": len(self.ae.active_associations) if hasattr(self.ae, 'active_associations') else 0,
        }