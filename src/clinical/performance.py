...
class GPUAccelerator:
    ...
    def move_to_device(self, data: Union[torch.Tensor, Dict, List]) -> Union[torch.Tensor, Dict, List]:
...
class BatchProcessor:
    ...
    def mark_processed(self, request_ids: List[str], results: List[Dict]):
        ...
        processing_time = time.time() - min(
            req['timestamp'] for req in self.request_queue 
...
class OptimizedInferencePipeline:
    ...
    def inference_single(
        ...
        with self.profiler.profile_stage("wsi_encoding"):
            with torch.no_grad():
                with self.gpu_accelerator.autocast_context():
                    # Add batch dimension
                    patch_features_batch = patch_features.unsqueeze(0)  # [1, num_patches, feature_dim]
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        gpu_stats = self.resource_monitor.get_gpu_usage() if self.gpu_accelerator.device.type == 'cuda' else {}
        ...
        return {
            ...
            'metrics': {
                ...
                'slow_inference_count': self.metrics_tracker.get_latest('slow_inference_count') or 0,
            },
            'gpu_stats': gpu_stats,
