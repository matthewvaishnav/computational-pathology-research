        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            events = [
                AuditEvent(
                    ...
                    event_type=AuditEventType.PREDICTION_OPERATION if i % 2 == 0 else AuditEventType.USER_ACCESS,
    ...
    def test_get_audit_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.log_prediction_operation(
                user_id="user1", session_token=None, input_data={}, output_data={},
                model_version="v1.0", processing_time_ms=1000
            )
            logger.log_user_access(
                event_type="login", user_id="user2", session_token=None,
                resource="system", action="authenticate", success=True
            )
...
class TestComplianceAuditLogger:
    ...
    def test_compliance_logger_with_backup(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Log an event
            event_id = logger.log_prediction_operation(
                user_id="user123", session_token=None, input_data={}, output_data={},
                model_version="v1.0", processing_time_ms=1500
            )
    ...
    def test_generate_compliance_report(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.log_prediction_operation(
                user_id="user123", session_token=None, input_data={}, output_data={},
                model_version="v1.0", processing_time_ms=1200
            )
            logger.log_user_access(
                event_type="login", user_id="user123", session_token=None,
                resource="system", action="authenticate", success=True
            )
    ...
    def test_validate_audit_chain(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.log_user_access(
                event_type="login", user_id="user456", session_token=None,
                resource="system", action="authenticate", success=True
            )
...
class TestAuditLogAnalyzer:
    ...
    def test_detect_anomalous_patterns(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(10):
                logger.log_user_access(
                    event_type="data_access", user_id="normal_user", session_token=None,
                    resource="patient_data", action="read", success=True
                )
            
            # Create suspicious activity (high failure rate)
            for i in range(15):
                logger.log_user_access(
                    event_type="login", user_id="suspicious_user", session_token=None,
                    resource="system", action="authenticate", 
            ...
            suspicious_user_anomaly = next(
                (a for a in anomalies['failed_operations_spike'] if a['user_id'] == 'suspicious_user'),
                None
            )
