...
class ModelValidator:
    ...
    def __init__(
        ...
        logger.info(f"ModelValidator initialized with accuracy_threshold={accuracy_threshold}, "
    ...
    def validate_model(
        ...
        if num_classes == 2:
            ...
            try:
                ...
                auc_score = 0.0
                logger.warning("Could not calculate AUC - some classes may be missing from validation set")
        ...
        auc_passed = auc_score >= self.auc_threshold if num_classes == 2 else True  # Only check AUC for binary
    ...
    def detect_concept_drift(self) -> Dict[str, Any]:
        ...
        if drift_detected:
            ...
            if len(accuracy_trend) > 1:
                ...
                declining_count = sum(1 for i in range(1, len(accuracy_trend)) 
    ...
    def detect_performance_degradation(self) -> bool:
        ...
        auc_degraded = (latest_performance['auc'] < self.auc_threshold 
                       if latest_performance['auc'] > 0 else False)
        
        # Also check for significant drop from historical average
        if len(self.performance_history) >= 5:
            historical_accuracy = np.mean([m['accuracy'] for m in list(self.performance_history)[:-1]])
    ...
    def recommend_retraining(self) -> Dict[str, Any]:
        ...
        if not latest_performance.get('validation_passed', True):
            should_retrain = True
            urgency = 'critical'
            reasons.append(f"Model failed validation thresholds (accuracy: {latest_performance.get('accuracy', 0):.3f}, "
        ...
        if drift_results['drift_detected']:
            ...
            reasons.append(f"Concept drift detected ({drift_results['drift_type']}) with "
        ...
        if 'subpopulation_results' in latest_performance:
            ...
            if poor_subpop_performance:
                should_retrain = True
                urgency = 'medium' if urgency not in ['critical', 'high'] else urgency
                reasons.append(f"Poor performance in subpopulations: {', '.join(poor_subpop_performance)}")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if len(self.performance_history) >= 5:
            ...
            auc_trend = 'improving' if recent_aucs and recent_aucs[-1] > recent_aucs[0] else 'declining'
        else:
    ...
    def export_performance_history(self, filepath: str):
        ...
        history_data = {
            ...
            'subpopulation_history': {
                k: list(v) for k, v in self.subpopulation_history.items()
            },
            'taxonomy_history': {
                k: list(v) for k, v in self.taxonomy_history.items()
            },
            'configuration': {
...
class PerformanceMonitor:
    ...
    def _format_alert_message(self, alert_type: str, data: Dict[str, Any]) -> str:
        ...
        if alert_type == 'validation_failed':
            return (f"Model validation failed - Accuracy: {data['accuracy']:.3f}, "
                   f"AUC: {data['auc']:.3f}")
            ...
            return (f"Concept drift detected - Type: {drift_info.get('drift_type', 'unknown')}, "
