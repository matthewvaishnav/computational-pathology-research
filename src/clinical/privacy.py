...
class PatientIdentifierAnonymizer:
    ...
    def anonymize_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        identifier_fields = [
            'patient_id', 'mrn', 'medical_record_number', 'ssn', 
            'social_security_number', 'name', 'first_name', 'last_name',
...
class DataExportMonitor:
    ...
    def detect_unauthorized_export(self, session_token: str, data_access_pattern: Dict[str, Any]) -> bool:
...
class PrivacyManager:
    ...
    def request_data_export(
        ...
        if not self.check_access_permission(session_token, Permission.EXPORT_DATA):
            self.logger.warning(f"Export denied: insufficient permissions for session {session_token}")
            return False
    ...
    def delete_patient_data(
        ...
        if not self.check_access_permission(session_token, Permission.DELETE_PATIENT_DATA):
            self.logger.warning(f"Deletion denied: insufficient permissions for session {session_token}")
            return False
        ...
        if preserve_audit:
            ...
            self.logger.info(f"Patient data marked for deletion with audit preservation: {anonymized_id}")
        else:
    ...
    def get_privacy_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'export_attempts_today': len([
                attempt for attempt in self.export_monitor.export_attempts
...
class SessionTimeoutManager:
    ...
    def set_custom_timeout(self, session_token: str, timeout_minutes: int) -> None:
        ...
        self.logger.info(f"Set custom timeout {timeout_minutes}min for session {session_token[:8]}...")
...
class UnauthorizedAccessDetector:
    ...
    def record_failed_attempt(self, user_id: str, ip_address: Optional[str] = None) -> None:
        ...
        if len(self.failed_attempts[user_id]) >= self.max_failed_attempts:
            ...
            self.logger.warning(f"User {user_id} locked out due to {self.max_failed_attempts} failed attempts")
...
class EnhancedPrivacyManager(PrivacyManager):
    ...
    def authenticate_user(
        ...
        if self.access_detector.is_user_locked(user_id):
            self.access_logger.log_access(
                "authentication", user_id, "", "user_session", "login", 
                False, ip_address=ip_address
            )
        ...
        self.access_logger.log_access(
            "authentication", user_id, session_token, "user_session", "login", 
            True, ip_address=ip_address
        )
    ...
    def access_patient_data(
        ...
        if not session:
            self.access_logger.log_access(
                "data_access", "unknown", session_token, f"patient:{patient_id}", 
                action, False, ip_address=ip_address
            )
            return False
        ...
        if self.timeout_manager.is_session_expired(session):
            ...
            self.access_logger.log_access(
                "data_access", session.user_id, session_token, f"patient:{patient_id}", 
                action, False, ip_address=ip_address
            )
            return False
        ...
        self.access_logger.log_access(
            "data_access", session.user_id, session_token, f"patient:{patient_id}", 
            action, True, patient_data, ip_address
        )
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        access_events_today = [
            event for event in self.access_logger.access_log
            if event.timestamp >= today_start
        ]
        
        failed_access_today = [
            event for event in access_events_today
    ...
    def generate_security_report(self) -> Dict[str, Any]:
        ...
        recent_events = [
            event.to_dict() for event in self.access_logger.access_log[-50:]
        ]
