...
class TreatmentResponseMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_scan_date": self.baseline_scan_date.isoformat() if self.baseline_scan_date else None,
            "response_scan_date": self.response_scan_date.isoformat() if self.response_scan_date else None,
...
class TreatmentResponseAnalyzer:
    ...
    def _predict_response_durability(
        ...
        response_prob = response_scan.disease_probabilities.get(
            response_scan.disease_state, 0.0
        )
    ...
    def _analyze_treatment_history_durability(
        ...
        prior_treatments = [
            ...
            if t.treatment_date < current_treatment.treatment_date
        ]
    ...
    def _calculate_kinetics_confidence(
        self, days_to_response: int, treatment_type: str, 
        baseline_scan: ScanRecord, response_scan: ScanRecord
    ) -> float:
        """Calculate confidence in kinetics classification."""
        confidence_factors = []
    ...
    def _detect_unexpected_response(
        self, timeline: PatientTimeline, treatment: TreatmentEvent,
        baseline_scan: ScanRecord, response_scan: ScanRecord,
        metrics: TreatmentResponseMetrics
    ) -> Dict[str, Any]:
        """
        ...
        if (metrics.response_category == TreatmentResponseCategory.PROGRESSIVE_DISEASE and
            metrics.days_to_response and metrics.days_to_response < 14):
        ...
        if (metrics.response_category == TreatmentResponseCategory.STABLE_DISEASE and
            treatment.treatment_type in ["surgery", "radiation"] and
            metrics.days_to_response and metrics.days_to_response > 30):
        ...
        if (metrics.response_time_deviation and metrics.response_time_deviation > 2.0 and
            metrics.response_category in [TreatmentResponseCategory.COMPLETE_RESPONSE,
        ...
        recent_treatments = [
            t for t in timeline.get_treatments()
            if (baseline_scan.scan_date - t.treatment_date).days <= 90
        ]
        if (len(recent_treatments) == 0 and
            metrics.response_category in [TreatmentResponseCategory.COMPLETE_RESPONSE,
                                         TreatmentResponseCategory.PARTIAL_RESPONSE]):
            unexpected_indicators.append(0.8)
        ...
        for factor_name, factor_values in factor_data.items():
            if len(set(factor_values)) > 1:  # Only analyze factors with variation
                try:
                    ...
                    if factor_name == "age" and all(isinstance(v, (int, float)) for v in factor_values if v is not None):
                        # Numerical correlation
                        valid_pairs = [(r, f) for r, f in zip(response_data, factor_values) if f is not None]
    ...
    def compare_therapeutic_regimens(
        ...
        for regimen, metrics_list in regimen_groups.items():
            ...
            response_counts = {
                "complete": sum(1 for m in metrics_list if m.response_category == TreatmentResponseCategory.COMPLETE_RESPONSE),
                "partial": sum(1 for m in metrics_list if m.response_category == TreatmentResponseCategory.PARTIAL_RESPONSE),
                "stable": sum(1 for m in metrics_list if m.response_category == TreatmentResponseCategory.STABLE_DISEASE),
                "progressive": sum(1 for m in metrics_list if m.response_category == TreatmentResponseCategory.PROGRESSIVE_DISEASE),
            }
            ...
            overall_response_rate = (response_counts["complete"] + response_counts["partial"]) / total_cases
            
            # Time to response statistics
            response_times = [m.days_to_response for m in metrics_list if m.days_to_response is not None]
            time_stats = {}
            if response_times:
                time_stats = {
            ...
            magnitudes = [m.response_magnitude for m in metrics_list if m.response_magnitude is not None]
            magnitude_stats = {}
            if magnitudes:
                magnitude_stats = {
    ...
    def visualize_treatment_response_trajectory(
        ...
        if not scans:
            # Handle empty data
            for ax in axes.flat:
                ax.text(0.5, 0.5, "No scan data available", ha="center", va="center",
        ...
        if phases:
            ...
            bars = axes[1, 0].bar(phase_names, phase_probs, 
                                 color=phase_colors[:len(phase_names)], alpha=0.7)
            ...
            for bar, phase in zip(bars, phases):
                ...
                axes[1, 0].text(bar.get_x() + bar.get_width()/2., height + 0.01,
                               f'n={phase["num_scans"]}', ha='center', va='bottom')
        else:
            axes[1, 0].text(0.5, 0.5, "No phase data available", ha="center", va="center",
        ...
        if prob_trajectories:
            ...
            for (state, trajectory), color in zip(prob_trajectories.items(), colors):
                ...
                axes[1, 1].plot(traj_days, traj_probs, 'o-', label=state, 
            ...
            axes[1, 1].text(0.5, 0.5, "No trajectory data available", ha="center", va="center",
    ...
    def visualize_unexpected_responses(
        ...
        if not unexpected_cases:
            ...
            ax.text(0.5, 0.5, "No unexpected responses found", ha="center", va="center",
        ...
        if type_counts:
            ...
            axes[0, 0].pie(counts, labels=[t.replace('_', ' ').title() for t in types], 
        ...
        if category_counts:
            ...
            axes[1, 1].bar([c.replace('_', ' ').title() for c in categories], ccounts, 
    ...
    def visualize_regimen_comparison(
        ...
        if not regimen_stats:
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.text(0.5, 0.5, "No regimen data available", ha="center", va="center",
                   transform=ax.transAxes, fontsize=14)
        ...
        for i, category in enumerate(response_categories):
            ...
            axes[0, 1].bar(regimens, rates, bottom=bottom, label=category.title(),
        ...
        if time_means:
            axes[1, 0].bar(valid_regimens, time_means, yerr=time_stds, 
                          alpha=0.7, color='#9b59b6', capsize=5)
    ...
    def _interpret_response_category(self, metrics: TreatmentResponseMetrics) -> str:
        ...
        interpretations = {
            TreatmentResponseCategory.COMPLETE_RESPONSE: 
                "Complete elimination or reduction of disease to undetectable levels. "
                "Excellent treatment outcome with high likelihood of durable benefit.",
            
            TreatmentResponseCategory.PARTIAL_RESPONSE:
                "Significant reduction in disease burden. Positive treatment response "
                "indicating therapeutic benefit, though residual disease remains.",
            
            TreatmentResponseCategory.STABLE_DISEASE:
                "Disease progression has been halted without significant reduction. "
                "Treatment may be providing disease control benefit.",
            
            TreatmentResponseCategory.PROGRESSIVE_DISEASE:
                "Disease has continued to advance despite treatment. Consider "
    ...
    def _interpret_response_kinetics(self, metrics: TreatmentResponseMetrics) -> str:
        ...
        interpretations = {
            ResponseKinetics.RAPID: 
                "Rapid response within 2 weeks. May indicate high treatment sensitivity "
                "or aggressive disease biology requiring close monitoring.",
            
            ResponseKinetics.STANDARD:
                "Standard response timing consistent with expected treatment kinetics. "
                "Typical biological response pattern for this treatment type.",
            
            ResponseKinetics.DELAYED:
                "Delayed response beyond typical timeframe. May indicate slower "
                "biological response or need for extended treatment duration.",
            
            ResponseKinetics.BIPHASIC:
                "Biphasic response pattern with initial improvement followed by plateau. "
                "Consider treatment modification or combination approaches.",
            
            ResponseKinetics.PROGRESSIVE:
                "Continuous progression despite treatment. Immediate reassessment "
    ...
    def _generate_clinical_recommendations(self, metrics: TreatmentResponseMetrics) -> List[str]:
        ...
        if metrics.response_category == TreatmentResponseCategory.COMPLETE_RESPONSE:
            ...
            if metrics.response_durability_score and metrics.response_durability_score < 0.6:
                recommendations.append("Consider extended monitoring due to lower durability prediction")
            ...
            if metrics.is_unexpected:
                recommendations.append("Multidisciplinary team review recommended for unexpected progression")
    ...
    def _assess_patient_risk_factors(self, metrics: TreatmentResponseMetrics) -> Dict[str, str]:
        ...
        if "smoking_status" in factors and factors["smoking_status"]:
            if factors["smoking_status"] in ["current", "former"]:
                risk_assessment["smoking"] = "Smoking history may impact treatment response and healing"
        
        if "num_prior_treatments" in factors and factors["num_prior_treatments"]:
            num_prior = factors["num_prior_treatments"]
            if num_prior > 2:
                risk_assessment["treatment_history"] = "Multiple prior treatments may indicate treatment resistance"
