        ...
        try:
            ...
            if format.lower() == "json":
                ...
                with open(output_path, 'w', newline='', encoding='utf-8') as f:
                    if records:
                        fieldnames = [
                            'event_id', 'event_type', 'timestamp', 'user_id', 'severity',
                            'description', 'model_version', 'ip_address', 'signature'
                        ]
        ...
        for log_file in self.storage_directory.glob("audit_*.jsonl"):
            try:
                ...
                if file_date < cutoff_date:
                    ...
                    self.logger.info(f"Deleted old audit log {log_file} with {file_record_count} records")
...
class ComplianceAuditLogger(AuditLogger):
    ...
    def generate_compliance_report(
        ...
        compliance_report = {
            'report_metadata': {
                ...
                'integrity_verification_passed': all(r['integrity_valid'] for r in integrity_results),
                'public_key_fingerprint': self.signer.public_key_fingerprint,
    ...
    def archive_old_records(
        ...
        self.compliance_logger.info(
            f"Archived {len(records_to_archive)} records to {archive_path}"
        )
...
class AuditLogAnalyzer:
    ...
    def detect_anomalous_patterns(
        ...
        for user_id, activity in user_activity.items():
            ...
            off_hours_activity = sum(activity['hourly_distribution'][22:24]) + \
                               sum(activity['hourly_distribution'][0:6])
    ...
    def generate_usage_report(
        ...
        for record in records:
            ...
            if (record.event.event_type == AuditEventType.PREDICTION_OPERATION and
                'processing_time_ms' in record.event.details):
                processing_times.append(record.event.details['processing_time_ms'])
        
        # Calculate derived metrics
        error_count = (report['summary_statistics']['severity_distribution'].get('error', 0) +
