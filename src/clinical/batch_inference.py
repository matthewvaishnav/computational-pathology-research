...
class ConcurrentInferenceManager:
    ...
    async def get_result_async(self, request_id: str, timeout: Optional[float] = None) -> Optional[InferenceResult]:
    ...
    def _get_request_batch(self) -> List[InferenceRequest]:
        ...
        while (
            len(batch) < self.inference_pipeline.batch_processor.max_batch_size and
            (time.time() - batch_start_time) < self.batch_timeout_seconds
        ):
            try:
    ...
    def _process_request_batch(self, batch_requests: List[InferenceRequest]):
        ...
        try:
            ...
            for request in batch_requests:
                batch_data.append({
                    'wsi_patches': request.wsi_patches,
    ...
    def get_statistics(self) -> Dict:
        ...
        if stats['completed_requests'] > 0:
            stats['slow_request_rate'] = (stats['slow_requests'] / stats['completed_requests']) * 100
        else:
...
class PerformanceMonitor:
        ...
        while self.monitoring_active:
            try:
                ...
                snapshot = {
                    ...
                    'patches_per_second': pipeline_metrics['metrics'].get('avg_patches_per_second', 0)
                }
    ...
    def _check_performance_alerts(self, snapshot: Dict):
        ...
        if snapshot['queue_size'] > 50:  # Queue getting large
            logger.warning(
                f"PERFORMANCE ALERT: Request queue size is {snapshot['queue_size']}"
            )
    ...
    def get_performance_report(self) -> Dict:
        ...
        avg_processing_time = sum(s['avg_processing_time'] for s in recent_snapshots) / len(recent_snapshots)
