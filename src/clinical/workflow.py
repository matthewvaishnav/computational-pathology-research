...
class ClinicalWorkflowSystem:
    ...
    def __init__(self, config: Union[ClinicalWorkflowConfig, Dict[str, Any]]):
        ...
        self.uncertainty_quantifier = UncertaintyQuantifier(
            num_classes=self.taxonomy.get_num_classes(),
            calibration_method="temperature"
        )
        
        self.ood_detector = OODDetector(
            feature_dim=self.embedding_dim,
            detection_methods=["mahalanobis", "reconstruction"]
        ).to(self.device)
        ...
        self.patient_context_integrator = PatientContextIntegrator(
            wsi_feature_dim=self.embedding_dim,
            clinical_metadata_dim=128,
            fusion_dim=512
        ).to(self.device)
        
        # 4. Initialize longitudinal tracking
        self.longitudinal_tracker = LongitudinalTracker(
            storage_path=config.patient_timeline_path
        )
        ...
        self.report_generator = ClinicalReportGenerator(
            template_dir=config.report_template_dir
        )
        ...
        try:
            ...
            with torch.no_grad():
                ...
                uncertainty_result = self.uncertainty_quantifier(
                    logits,
                    ood_scores=ood_result['ood_score']
                )
