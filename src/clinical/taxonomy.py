...
class DiseaseTaxonomy:
    ...
    def __init__(
        ...
        logger.info(
            f"Loaded disease taxonomy '{self.name}' with {len(self.diseases)} disease states"
        )
    ...
    def _validate_taxonomy(self) -> None:
        ...
        for disease_id, disease in self.diseases.items():
            ...
            if parent_id is not None and parent_id not in self.diseases:
                errors.append(
                    f"Disease '{disease_id}' references non-existent parent '{parent_id}'"
                )
            ...
            for child_id in children_ids:
                if child_id not in self.diseases:
                    errors.append(
                        f"Disease '{disease_id}' references non-existent child '{child_id}'"
                    )
                else:
                    ...
                    if child_parent != disease_id:
                        errors.append(
                            f"Inconsistent parent-child relationship: '{disease_id}' lists "
                            f"'{child_id}' as child, but '{child_id}' has parent '{child_parent}'"
                        )
    ...
    def get_diseases_at_level(self, level: int) -> List[str]:
        ...
        """
        return [
            disease_id for disease_id in self.disease_ids if self.get_level(disease_id) == level
        ]
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"DiseaseTaxonomy(name='{self.name}', version='{self.version}', "
