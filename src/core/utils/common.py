            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(
                            "Function %s failed after %d retries: %s", func.__name__, max_retries, e
                        )
                        raise
...
class ProgressTracker:
    ...
    def _log_progress(self):
        ...
        logger.info(
            f"{self.description}: {self.current}/{self.total} ({percentage:.1f}%) - ETA: {eta_str}"
        )
...
@contextmanager
def error_context(operation: str, cleanup_func: Optional[Callable] = None, reraise: bool = True):
    """
