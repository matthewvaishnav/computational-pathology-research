...
Example:
    >>> import torch
    >>> from src.core.utils.validation import validate_tensor_shape, validate_wsi_features
    >>>
...
def validate_tensor_shape(
    tensor: torch.Tensor, expected_shape: Tuple[Optional[int], ...], name: str = "tensor"
) -> None:
    """
...
def validate_batch_size(
    tensor: torch.Tensor, expected_batch_size: int, name: str = "tensor"
) -> None:
    """
...
def validate_wsi_features(
    features: torch.Tensor, expected_feature_dim: int = 1024, name: str = "wsi_features"
) -> None:
    """
...
def validate_genomic_features(
    features: torch.Tensor, expected_feature_dim: int = 2000, name: str = "genomic_features"
) -> None:
    """
...
def validate_clinical_text(
    tokens: torch.Tensor, max_seq_length: Optional[int] = None, name: str = "clinical_text"
) -> None:
    """
    ...
    def wrapper(*args, **kwargs):
        ...
        if batch is not None and isinstance(batch, dict):
            try:
                ...
                raise ValidationError(f"Validation failed in {func.__name__}:\n{str(e)}") from e
