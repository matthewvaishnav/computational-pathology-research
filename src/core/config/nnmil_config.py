...
class nnMILConfig:
    ...
    @staticmethod
    def _extract_dataset_fingerprint(dataset_path: Path, task_type: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if dataset_path.exists():
                fingerprint.update(nnMILConfig._analyze_dataset_structure(dataset_path, task_type))
            else:
                logging.warning(
                    f"Dataset path {dataset_path} does not exist. " f"Using default fingerprint."
                )

        except Exception as e:
            logging.warning(
                f"Failed to analyze dataset {dataset_path}: {e}. " f"Using default fingerprint."
            )

        return fingerprint

    @staticmethod
    def _analyze_dataset_structure(dataset_path: Path, task_type: str) -> Dict[str, Any]:
        """
