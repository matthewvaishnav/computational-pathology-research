...
class TimeoutLock:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if hold_time > 5.0:  # Warn if lock held > 5 seconds
            logger.warning(f"Lock '{self.name}' held for {hold_time:.2f}s by {self._owner}")
...
class BoundedQueue(Generic[T]):
    ...
    def __init__(self, maxsize: int = 1000, drop_policy: str = "block", name: str = "unnamed"):
        """
    ...
    def put(self, item: T, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            if self.drop_policy == "block":
                ...
                if self._dropped_count % 100 == 0:
                    logger.warning(f"Queue '{self.name}' dropped {self._dropped_count} items")
...
def create_bounded_queue(
    maxsize: int = 1000, drop_policy: str = "oldest", name: str = "unnamed"
) -> BoundedQueue:
    """
...
def create_graceful_thread(
    target: Callable, name: str = "unnamed", cleanup_callback: Optional[Callable] = None
) -> GracefulThread:
    """
