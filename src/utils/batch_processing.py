...
class AdaptiveBatcher:
            ...
            for batch_size, time_taken, memory_used in recent_history:
                if time_taken > 0:
                    ...
                    if (
                        ...
                        and memory_gb < self.config.memory_limit_gb * 0.8
                    ):
    ...
    def record_batch_performance(
        self, batch_size: int, processing_time: float, memory_peak_mb: float, success: bool
    ):
...
class OptimizedBatchProcessor:
    ...
    def process_batch_sync(
        self, batch: List[T], processor_func: Callable[[List[T]], List[R]], batch_id: int = 0
    ) -> BatchResult:
        """
        ...
        try:
            ...
            self.batcher.record_batch_performance(len(batch), processing_time, peak_memory, True)
            ...
            self.batcher.record_batch_performance(len(batch), processing_time, peak_memory, False)
    ...
    async def process_batch_async(
        ...
        result = await loop.run_in_executor(
            self.executor, self.process_batch_sync, batch, processor_func, batch_id
        )
    ...
    def process_items_parallel(
        ...
        logger.info(
            f"Completed processing {len(items)} items in {total_time:.2f}s "
    ...
    async def process_items_async(
        ...
        tasks = [
            self.process_batch_async(batch, processor_func, i) for i, batch in enumerate(batches)
        ]
...
def batch_process(
    batch_size: int = DEFAULT_BATCH_SIZE, adaptive: bool = True, memory_limit_gb: float = 4.0
):
    """
    ...
    def decorator(func: Callable[[List[T]], List[R]]) -> Callable[[List[T]], List[R]]:
        config = BatchConfig(
            batch_size=batch_size, adaptive_batching=adaptive, memory_limit_gb=memory_limit_gb
        )
...
def create_batch_processor(
    batch_size: int = DEFAULT_BATCH_SIZE, max_workers: int = 4, adaptive: bool = True
    ...
    """
    config = BatchConfig(batch_size=batch_size, max_workers=max_workers, adaptive_batching=adaptive)
