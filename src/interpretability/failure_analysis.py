...
class FailureAnalyzer:
    ...
    def identify_failures(
        ...
        if not (len(ground_truth) == len(confidences) == len(embeddings) == len(slide_ids) == n_samples):
        ...
        logger.info(f"Identified {n_failures}/{n_samples} failures ({100*n_failures/n_samples:.1f}%)")
    ...
    def analyze_cluster_characteristics(
        self,
        clusters: Dict[int, List[FailureCase]]
    ) -> pd.DataFrame:
    ...
    def export_failure_report(
        ...
        if not failures:
            ...
            df = pd.DataFrame(columns=[
                'slide_id', 'prediction', 'ground_truth', 'confidence', 'cluster_id'
            ])
