...
class PACSIntegrationService:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "workflow": {"polling_interval": 60, "max_concurrent_tasks": 10, "auto_start": True},
            "pacs_connections": {},
            "notifications": {"email_enabled": False, "sms_enabled": False, "hl7_enabled": True},
        }
    ...
    async def _start_dicom_server(self):
        ...
        self.dicom_server = create_medical_ai_dicom_server(
            port=config["port"], storage_dir=config["storage_dir"]
        )
    ...
    def _start_hl7_server(self):
        ...
        self.hl7_server = setup_hl7_integration(
            worklist_manager=self.worklist_manager, host=hl7_config["host"], port=hl7_config["port"]
        )
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "components": {
                ...
                "worklist_manager": (
                    self.worklist_manager.get_worklist_statistics()
                    if self.worklist_manager
                    else None
                ),
                "workflow_orchestrator": (
                    self.workflow_orchestrator.get_workflow_status()
                    if self.workflow_orchestrator
                    else None
                ),
                "hl7_server": {
    ...
    async def process_emergency_study(self, accession_number: str, pacs_name: str) -> bool:
        ...
        try:
            ...
            studies = self.pacs_client.find_studies(
                pacs_name=pacs_name, accession_number=accession_number
            )
