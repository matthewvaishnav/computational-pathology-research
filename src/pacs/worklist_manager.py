...
class WorklistEntry:
    ...
    def from_dicom_dataset(cls, ds: Dataset) -> 'WorklistEntry':
        ...
        sps = ds.ScheduledProcedureStepSequence[0] if ds.get('ScheduledProcedureStepSequence') else Dataset()
...
class WorklistManager:
    ...
    def query_worklist(self, 
                      ...
                      status: Optional[WorklistStatus] = None) -> List[WorklistEntry]:
    ...
    def create_pathology_worklist_entry(self,
                                      patient_id: str,
                                      patient_name: str,
                                      accession_number: str,
                                      study_description: str = "Digital Pathology Analysis",
                                      priority: str = "ROUTINE") -> WorklistEntry:
    ...
    def get_worklist_statistics(self) -> Dict[str, Any]:
        ...
        for status in WorklistStatus:
            status_counts[status.value] = sum(
                1 for entry in self.worklist_entries.values() 
        ...
        return {
            ...
            'scheduled_for_today': len([
                entry for entry in self.worklist_entries.values()
                if entry.scheduled_procedure_step_start_date == datetime.now().strftime("%Y%m%d")
            ])
        }
    ...
    def cleanup_old_entries(self, days_old: int = 30) -> int:
        ...
        for accession_number, entry in self.worklist_entries.items():
            if (entry.status in [WorklistStatus.COMPLETED, WorklistStatus.CANCELLED] and
                entry.scheduled_procedure_step_start_date < cutoff_date):
