    ...
    def test_connection(self, pacs_name: str) -> bool:
        ...
        try:
            ...
            assoc = self.ae.associate(
                connection.host, connection.port, ae_title=connection.ae_title
            )
                    ...
                    if status and status.Status == 0xFF00:  # Pending
                        if identifier and count < max_results:
                            study = StudyInfo(
                                ...
                                series_count=int(
                                    getattr(identifier, "NumberOfStudyRelatedSeries", 0) or 0
                                ),
                                instance_count=int(
                                    getattr(identifier, "NumberOfStudyRelatedInstances", 0) or 0
                                ),
                            )
        ...
        try:
            ...
            if assoc.is_established:
                ...
                responses = assoc.send_c_move(
                    move_ds, destination_ae, StudyRootQueryRetrieveInformationModelMove
                )
