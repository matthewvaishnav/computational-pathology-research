...
class SingleModalityModel(nn.Module):
    ...
    def __init__(
        ...
        if modality not in self.SUPPORTED_MODALITIES:
            raise ValueError(
                f"Modality must be one of {self.SUPPORTED_MODALITIES}, got {modality}"
            )
    ...
    def forward(
        ...
        if self.modality == 'wsi':
            ...
            if wsi_features is None:
                raise ValueError("WSI features required for SingleModalityModel with modality='wsi'")
            embedding = self.encoder(wsi_features, mask=batch.get('wsi_mask'))

        elif self.modality == 'genomic':
            genomic = batch.get('genomic')
            if genomic is None:
                raise ValueError("Genomic data required for SingleModalityModel with modality='genomic'")
            embedding = self.encoder(genomic)

        elif self.modality == 'clinical':
            clinical_text = batch.get('clinical_text')
            if clinical_text is None:
                raise ValueError("Clinical text required for SingleModalityModel with modality='clinical'")
...
def get_baseline_model(
    ...
    **kwargs
    ...
    if baseline_type == 'single_modality':
        ...
        return SingleModalityModel(modality=modality, config=config, embed_dim=kwargs.get('embed_dim', 256))
    elif baseline_type == 'late_fusion':
        return LateFusionModel(config=config, embed_dim=kwargs.get('embed_dim', 256), dropout=kwargs.get('dropout', 0.1))
    elif baseline_type == 'attention':
        return AttentionBaseline(config=config, embed_dim=kwargs.get('embed_dim', 256), dropout=kwargs.get('dropout', 0.1))
    else:
