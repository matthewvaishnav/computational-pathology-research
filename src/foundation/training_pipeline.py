...
from multi_disease_model import MultiDiseaseFoundationModel, ModelConfig
from self_supervised_pretrainer import SelfSupervisedPreTrainer, PreTrainingConfig
from zero_shot_detection import ZeroShotDetector, DiseaseKnowledgeBase, VisionLanguageEncoder
from data_collection import WSIDataCollector, UnlabeledWSIDataset
...
class FoundationModelTrainer:
        ...
        for epoch in range(self.config.finetune_epochs):
            ...
            if self.config.use_wandb:
                wandb.log({
                    ...
                    **{f"finetune/accuracy_{disease}": acc for disease, acc in disease_accuracies.items()}
                })
    ...
    def final_evaluation(self) -> Dict[str, Any]:
        ...
        if self.config.use_wandb:
            wandb.log({
                ...
                **{f"final/accuracy_{disease}": acc for disease, acc in results["disease_specific_accuracies"].items()}
            })
    ...
    def save_final_model(self):
        ...
        torch.save({
            ...
            "metrics_history": [asdict(m) for m in self.metrics_history],
            "knowledge_base": self.knowledge_base.diseases if self.config.enable_zero_shot else None
        }, final_model_path)
