...
class DiseaseKnowledgeBase:
    ...
    def get_disease_descriptions(self) -> List[str]:
        ...
        for disease in self.diseases.values():
            ...
            full_description = f"{disease.description}. Key features: {', '.join(disease.pathological_features)}"
...
class ZeroShotDetector:
    ...
    def _build_similarity_index(self):
        ...
        if self.text_embeddings is not None:
            ...
            self.similarity_index = faiss.IndexFlatIP(dimension)  # Inner product (cosine similarity)
    ...
    def predict(
        ...
        requires_expert_review = (
            best_confidence < self.similarity_threshold or
            uncertainty_score > self.uncertainty_threshold
        )
    ...
    def batch_predict(
        self,
        image_features_batch: torch.Tensor,
        top_k: int = 5
    ...
    def get_similar_diseases(
        ...
        top_k_values, top_k_indices = torch.topk(similarities, min(top_k + 1, len(self.disease_names)))
