...
class MultiDiseaseFoundationModel(nn.Module):
    ...
    def _build_encoder(self) -> nn.Module:
        ...
        if self.config.encoder_type.startswith("resnet"):
            ...
            encoder = timm.create_model(
                self.config.encoder_type, pretrained=True, num_classes=0, global_pool=""
            )
        else:
        ...
        for disease in self.config.supported_diseases:
            if disease in disease_classes:
                self.disease_heads[disease] = DiseaseSpecificHead(
                    self.config.feature_dim, disease_classes[disease], disease
                )
    ...
    def zero_shot_predict(
        self, patches: torch.Tensor, text_embeddings: torch.Tensor
    ...
    def add_disease_head(self, disease: str, num_classes: int, freeze_encoder: bool = True) -> None:
        ...
        self.disease_heads[disease] = DiseaseSpecificHead(
            self.config.feature_dim, num_classes, disease
        )
