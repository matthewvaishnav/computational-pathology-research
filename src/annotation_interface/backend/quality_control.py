    ...
    def validate(self, annotation: Annotation) -> ValidationResult:
        ...
        if geometry.type == AnnotationType.POLYGON:
            ...
            if geometry.center is None or geometry.radius is None:
                result.add_issue(
                    ValidationIssue.INCOMPLETE_GEOMETRY,
                    "Circle geometry missing center or radius"
                )
...
class InterRaterAgreement:
    ...
    def calculate_cohens_kappa(
        annotations_rater1: List[Annotation],
        annotations_rater2: List[Annotation],
        slide_id: str
    ) -> Dict:
        """
    ...
    def _find_matching_annotations(
        annots1: List[Annotation],
        annots2: List[Annotation]
    ) -> List[Dict]:
...
class QualityMetricsTracker:
    ...
    def _compute_expert_metrics(
        ...
        expert_data = defaultdict(lambda: {
            "count": 0,
            "avg_confidence": [],
            "avg_quality_score": [],
            "annotation_times": []
        ...
        for expert_id, data in expert_data.items():
            expert_metrics[expert_id] = {
                ...
                "avg_quality_score": round(
                    np.mean(data["avg_quality_score"]), 3
                ) if data["avg_quality_score"] else None,
                "avg_annotation_time": round(
                    np.mean(data["annotation_times"]), 2
                ) if data["annotation_times"] else None,
                "median_annotation_time": round(
                    np.median(data["annotation_times"]), 2
                ) if data["annotation_times"] else None
            }
