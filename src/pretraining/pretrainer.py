...
class SelfSupervisedPretrainer:
    ...
    def __init__(
        ...
        self.contrastive_loss = PatchContrastiveLoss(
            temperature=temperature, use_hard_negatives=use_hard_negatives
        )
    ...
    def compute_reconstruction_loss(
        self, patches: torch.Tensor, decoder: Optional[nn.Module] = None
    ...
    def train_step(
        ...
        if use_contrastive and "patches_i" in batch and "patches_j" in batch:
            contrastive_loss = self.compute_contrastive_loss(
                batch["patches_i"], batch["patches_j"], batch.get("labels", None)
            )
        ...
        if use_reconstruction and "patches" in batch:
            reconstruction_loss, mask = self.compute_reconstruction_loss(batch["patches"], decoder)
    ...
    def pretrain(
        ...
        # Training history
        history = {"total_loss": [], "contrastive_loss": [], "reconstruction_loss": []}

        logger.info(f"Starting pretraining for {num_epochs} epochs")
        logger.info(
            f"Contrastive weight: {self.contrastive_weight}, "
            f"Reconstruction weight: {self.reconstruction_weight}"
        )

        for epoch in range(num_epochs):
            epoch_losses = {"total_loss": [], "contrastive_loss": [], "reconstruction_loss": []}
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                loss_dict = self.train_step(
                    batch, optimizer, decoder, use_contrastive, use_reconstruction
                )
            ...
            if checkpoint_dir is not None and (epoch + 1) % checkpoint_interval == 0:
                self.save_checkpoint(checkpoint_dir, epoch, optimizer, decoder)
