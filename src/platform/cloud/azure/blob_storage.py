...
class AzureBlobStorageConnector:
                ...
                "rules": [
                    {
                        ...
                        "type": "Lifecycle",
                        "definition": {
                            "filters": {"blobTypes": ["blockBlob"], "prefixMatch": ["histocore/"]},
                            "actions": {
                                "baseBlob": {
    ...
    def upload_file(
        ...
        try:
            ...
            return UploadResult(blob_name=blob_name, success=False, error_message=error_msg)
    ...
    def upload_files_batch(
        ...
        for local_path, blob_name in file_mappings:
            future = self.executor.submit(
                self.upload_file, local_path, blob_name, access_tier, metadata, tags
            )
        ...
        for future in futures:
            try:
                ...
                results.append(
                    UploadResult(blob_name="unknown", success=False, error_message=str(e))
                )

        successful_uploads = sum(1 for r in results if r.success)
        logger.info(
            "Batch upload completed: %d/%d successful", successful_uploads, len(file_mappings)
        )
    ...
    def list_blobs(
        ...
        try:
            ...
            blob_list = self.container_client.list_blobs(
                name_starts_with=prefix, include=include_options
            )
    ...
    def copy_blob(
        ...
        try:
            ...
            if copy_status == "pending":
                ...
                logger.info(
                    "Copy operation started for %s -> %s", source_blob_name, destination_blob_name
                )
            else:
            ...
            logger.error(
                "Failed to copy blob %s to %s: %s", source_blob_name, destination_blob_name, e
            )
            return False
            ...
            for blob in self.container_client.list_blobs():
                if blob.last_modified < cutoff_date:
                    if not dry_run:
                        ...
                        deleted_blobs.append(blob.name)
                        logger.info(
                            "Would delete old blob: %s (last modified: %s)",
                            blob.name,
                            blob.last_modified,
                        )
...
def create_blob_storage_connector(
    account_name: str, container_name: str, **kwargs
) -> AzureBlobStorageConnector:
    """Create Azure Blob Storage connector with configuration."""
    config = BlobStorageConfig(account_name=account_name, container_name=container_name, **kwargs)
