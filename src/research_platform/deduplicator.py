...
class Deduplicator:
    ...
    def _deduplicate_exact(self, image_paths: List[Path]) -> Tuple[List[Path], List[DuplicateGroup]]:
    ...
    def _deduplicate_perceptual(self, image_paths: List[Path]) -> Tuple[List[Path], List[DuplicateGroup]]:
    ...
    def _deduplicate_feature(self, image_paths: List[Path]) -> Tuple[List[Path], List[DuplicateGroup]]:
...
class DatasetDeduplicator:
    ...
    def deduplicate_dataset(self, image_paths: List[Path],
        ...
        self.logger.info(f"Dedup complete: {len(unique)} unique, {total_duplicates} duplicates removed")
...
def deduplicate_images(image_paths: List[Path],
                      method: str = 'perceptual',
                      threshold: float = 0.95) -> Tuple[List[Path], List[DuplicateGroup]]:
    """Deduplicate images"""
...
def deduplicate_dataset(image_paths: List[Path],
                       output_dir: Path = None,
                       method: str = 'perceptual',
