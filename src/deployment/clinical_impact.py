...
class TurnaroundTimeMetric:
    ...
    def pathologist_review_time(self) -> float:
        ...
        return (self.pathologist_review_complete - self.pathologist_review_start).total_seconds() / 60
...
class ClinicalImpactTracker:
    ...
    def calculate_diagnostic_accuracy_metrics(self, 
        ...
        high_conf_accuracy = (high_confidence_cases['ai_prediction'] == 
                             high_confidence_cases['ground_truth']).sum() / len(high_confidence_cases) if len(high_confidence_cases) > 0 else 0
        ...
        metrics = {
            ...
            "cases_by_confidence": {
                ...
                "medium (0.6-0.8)": len(df[(df['confidence_score'] >= 0.6) & (df['confidence_score'] < 0.8)]),
    ...
    def calculate_turnaround_time_metrics(self,
                                        site_id: Optional[str] = None,
                                        days_back: int = 30) -> Dict[str, Any]:
        ...
        df['ai_processing_time'] = (df['ai_processing_complete'] - df['ai_processing_start']).dt.total_seconds() / 60
        df['pathologist_review_time'] = (df['pathologist_review_complete'] - df['pathologist_review_start']).dt.total_seconds() / 60
        df['total_turnaround_time'] = (df['report_finalized'] - df['slide_received']).dt.total_seconds() / 3600
    ...
    def calculate_user_satisfaction_metrics(self,
                                          site_id: Optional[str] = None,
                                          days_back: int = 30) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "overall_metrics": {
                "ease_of_use": {
                    "mean": df['ease_of_use'].mean(),
                    "std": df['ease_of_use'].std()
                },
                "accuracy_perception": {
    ...
    def perform_statistical_significance_testing(self,
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            baseline_df = pd.read_sql_query(baseline_query, conn, 
                                          params=[baseline_start.isoformat(), baseline_end.isoformat()])
            ...
            comparison_df = pd.read_sql_query(comparison_query, conn,
                                            params=[comparison_start.isoformat()])
        ...
        comparison_accuracy = (comparison_df['ai_prediction'] == comparison_df['ground_truth']).mean()
        ...
        results = {
            ...
            "statistical_tests": {
                "accuracy_improvement": {
                    ...
                    "improvement_percent": ((comparison_accuracy - baseline_accuracy) / baseline_accuracy) * 100
                },
    ...
    def calculate_cost_benefit_analysis(self, site_id: str, days_back: int = 90) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "projections": {
                ...
                "annual_net_savings": (labor_cost_savings * (365 / days_back)) - (ai_system_monthly_cost * 12),
                "payback_period_months": monthly_ai_cost / (labor_cost_savings * (30 / days_back)) if labor_cost_savings > 0 else float('inf')
            }
        }
        
        return analysis
        
    def generate_impact_report(self, site_id: Optional[str] = None, days_back: int = 30) -> Dict[str, Any]:
        ...
        if site_id:
            report["cost_benefit_analysis"] = self.calculate_cost_benefit_analysis(site_id, days_back)
...
def generate_demo_data(tracker: ClinicalImpactTracker, days_back: int = 30):
    ...
    users = [
        ("PATH001", "pathologist"), ("PATH002", "pathologist"),
        ("TECH001", "technician"), ("TECH002", "technician"),
        ("ADMIN001", "admin")
    ]
...
if __name__ == "__main__":
    ...
    print(f"Average processing time: {report['diagnostic_accuracy']['average_processing_time']:.1f}s")
    print(f"User satisfaction: {report['user_satisfaction']['overall_metrics']['overall_satisfaction']['mean']:.2f}/5")