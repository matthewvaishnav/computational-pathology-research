...
class EpicPatient:
    ...
    def to_fhir_patient(self) -> Patient:
        ...
        patient.identifier = [{"use": "usual", "system": "http://epic.com/mrn", "value": self.mrn}]
        patient.active = self.active
        patient.name = [{"use": "official", "family": self.last_name, "given": [self.first_name]}]
        patient.gender = self.gender.lower()
        patient.birthDate = self.date_of_birth.date()

        if self.phone:
            patient.telecom = [{"system": "phone", "value": self.phone, "use": "home"}]

        if self.email:
            if not patient.telecom:
                patient.telecom = []
            patient.telecom.append({"system": "email", "value": self.email})
...
class EpicDiagnosticReport:
    ...
    def to_fhir_diagnostic_report(self) -> DiagnosticReport:
        ...
        report.code = {"coding": [{"system": "http://loinc.org", "code": self.code}]}
...
class EpicFHIRPlugin(EMRPlugin):
    ...
    def metadata(self) -> PluginMetadata:
        ...
        return PluginMetadata(
            ...
            configuration_schema={
                ...
                "auth_method": {
                    "type": "string",
                    "enum": ["client_credentials", "jwt_bearer"],
    ...
    async def _jwt_bearer_auth(self) -> bool:
        ...
        try:
            ...
            assertion = jwt.encode(
                payload, private_key, algorithm="RS256", headers={"kid": self.key_id}
            )
    ...
    async def update_diagnostic_report(self, report_id: str, report: EpicDiagnosticReport) -> bool:
    ...
    def _parse_fhir_diagnostic_report(
        self, report_data: Dict[str, Any]
    ) -> Optional[EpicDiagnosticReport]:
        ...
        try:
            ...
            return EpicDiagnosticReport(
                ...
                patient_id=report_data.get("subject", {})
                .get("reference", "")
                .replace("Patient/", ""),
                ...
                performer=(
                    report_data.get("performer", [{}])[0].get("reference")
