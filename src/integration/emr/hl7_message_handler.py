...
from hl7apy.exceptions import InvalidName, ChildNotFound
...
class HL7MessageHandler:
    ...
    async def process_message(self, hl7_message: str, source: str = "unknown") -> Dict[str, Any]:
        ...
        try:
            ...
            processed_data = await self._route_and_process_message(parsed_message, message_info, transformed_data)
            ...
            if self.auto_ack and 'message_id' in locals():
                try:
                    ack_message = self._generate_ack_message(
                        message_info, HL7AckCode.AE, str(e)
                    )
                    processing_result['ack_message'] = ack_message
                except:
                    pass
    ...
    async def _validate_message(self, parsed_message: Message, message_info: HL7MessageInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_info.message_type in self.validation_rules:
                ...
                validation_result['valid'] = validation_result['valid'] and rule_result.get('valid', True)
    ...
    async def _transform_message(self, parsed_message: Message, message_info: HL7MessageInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_info.message_type in [HL7MessageType.ADT_A01, HL7MessageType.ADT_A04, HL7MessageType.ADT_A08]:
    ...
    def _transform_adt_message(self, parsed_message: Message) -> Dict[str, Any]:
        ...
        try:
            ...
            patient_data = HL7Patient(
                ...
                date_of_birth=self._parse_hl7_datetime(pid.pid_7.value) if hasattr(pid, 'pid_7') and pid.pid_7.value else None,
    ...
    def _transform_orm_message(self, parsed_message: Message) -> Dict[str, Any]:
        ...
        try:
            ...
            order_data = HL7Order(
                ...
                requested_datetime=self._parse_hl7_datetime(obr.obr_6.value) if hasattr(obr, 'obr_6') and obr.obr_6.value else None,
            ...
            if hasattr(parsed_message, 'obx'):
                ...
                for obx in obx_segments:
                    observation = HL7Observation(
                        ...
                        observation_result_status=obx.obx_11.value if hasattr(obx, 'obx_11') else '',
                        observation_datetime=self._parse_hl7_datetime(obx.obx_14.value) if hasattr(obx, 'obx_14') and obx.obx_14.value else None
                    )
    ...
    async def _route_and_process_message(self, parsed_message: Message, message_info: HL7MessageInfo, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_info.message_type in self.message_handlers:
                return await self.message_handlers[message_info.message_type](parsed_message, transformed_data)
    ...
    def _generate_ack_message(self, message_info: HL7MessageInfo, ack_code: HL7AckCode, error_message: Optional[str] = None) -> str:
...
async def example_adt_handler(parsed_message: Message, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
    """Example ADT message handler"""
    print(f"Processing ADT message for patient: {transformed_data.get('patient', {}).get('patient_name')}")
    return {'processed': True, 'action': 'patient_updated'}


async def example_orm_handler(parsed_message: Message, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
    """Example ORM message handler"""
    print(f"Processing order for patient: {transformed_data.get('patient', {}).get('patient_name')}")
    return {'processed': True, 'action': 'order_created'}


async def example_oru_handler(parsed_message: Message, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
    """Example ORU message handler"""
    print(f"Processing results for patient: {transformed_data.get('patient', {}).get('patient_name')}")
