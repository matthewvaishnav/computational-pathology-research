...
class AWSHealthLakePlugin(CloudPlugin):
    ...
    async def _create_signed_headers(
        self, method: str, url: str, credentials: Dict[str, str]
    ) -> Dict[str, str]:
    ...
    async def create_fhir_resource(
        ...
        try:
            ...
            async with self.session.post(
                create_url, json=resource_data, headers=headers
            ) as response:
    ...
    async def start_import_job(
        self, input_s3_uri: str, job_name: Optional[str] = None
    ) -> Optional[str]:
    ...
    async def start_export_job(
        self, output_s3_uri: str, job_name: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            response = self.healthlake_client.start_fhir_export_job(
                OutputDataConfig={
                    "S3Configuration": {"S3Uri": output_s3_uri, "KmsKeyId": "alias/aws/s3"}
                },
                DatastoreId=self.datastore_id,
                DataAccessRoleArn=self._get_data_access_role_arn(),
                JobName=job_name,
