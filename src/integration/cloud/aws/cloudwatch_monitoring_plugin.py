...
class AWSCloudWatchMonitoringPlugin(MonitoringPlugin):
    ...
    async def put_metric(self, metric_name: str, value: float, unit: MetricUnit,
                        dimensions: Optional[Dict[str, str]] = None,
    ...
    async def _metric_flush_task(self):
        ...
        try:
            while True:
                ...
                if (datetime.now() - self.last_flush_time).total_seconds() >= self.metric_flush_interval:
                    await self._flush_metrics()
    ...
    async def put_log_event(self, log_group: str, log_stream: str, message: str,
                           timestamp: Optional[datetime] = None):
    ...
    async def get_metric_statistics(self, metric_name: str, start_time: datetime,
    ...
    async def query_logs(self, log_group: str, query: str, start_time: datetime,
                        end_time: datetime, limit: int = 100) -> List[LogEvent]:
        ...
        try:
            async with self.session.client('logs') as logs:
                ...
                for result_row in result['results']:
                    ...
                    log_event = LogEvent(
                        timestamp=datetime.fromtimestamp(int(event_data.get('@timestamp', 0)) / 1000),
