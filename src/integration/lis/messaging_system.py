...
class FHIRMessageBuilder:
    ...
    def create_service_request(order_data: Dict[str, Any]) -> ServiceRequest:
        ...
        service_request.identifier = [
            {"system": "http://hospital.com/orders", "value": order_data["accession_number"]}
        ]
        ...
        service_request.subject = {"reference": f"Patient/{order_data['patient_id']}"}
    ...
    def create_diagnostic_report(result_data: Dict[str, Any]) -> DiagnosticReport:
        ...
        report.identifier = [
            {"system": "http://hospital.com/results", "value": result_data["accession_number"]}
        ]
        report.status = "final"
        report.code = {
            "coding": [
        ...
        report.subject = {"reference": f"Patient/{result_data['patient_id']}"}
    ...
    def create_observation(result_data: Dict[str, Any]) -> Observation:
        ...
        observation.subject = {"reference": f"Patient/{result_data['patient_id']}"}
        ...
        if result_data.get("reference_range"):
            observation.referenceRange = [{"text": result_data["reference_range"]}]
...
class MessagingSystem:
    ...
    async def send_order_message(self, order_data: Dict[str, Any], destination: str) -> str:
    ...
    async def send_result_message(self, result_data: Dict[str, Any], destination: str) -> str:
    ...
    async def send_fhir_message(
        self, resource_data: Dict[str, Any], resource_type: str, destination: str
    ) -> str:
        """Send FHIR message"""
    ...
    async def _send_http_message(self, message: MessageQueue) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                headers = {"Content-Type": "application/hl7-v2", "X-Message-ID": message.message_id}

                async with session.post(
                    message.destination, data=message.content, headers=headers
                ) as response:
    ...
    def _generate_ack_message(
        self, original_control_id: str, ack_code: str, error_message: Optional[str] = None
    ) -> str:
