...
class CernerPathNetPlugin(LISPlugin):
        ...
        try:
            ...
            async with self.session.get(orders_url, params=params) as response:
                if response.status == 200:
                    ...
                    return [
                        self._parse_pathnet_order(order) for order in orders_data.get("orders", [])
                    ]
                else:
    ...
    async def update_order_status(
        self, accession_number: str, status: PathNetOrderStatus, comment: Optional[str] = None
    ) -> bool:
    ...
    async def get_worklist(
        ...
        try:
            ...
            params = {"department": self.department_code}
            ...
            async with self.session.get(worklist_url, params=params) as response:
                if response.status == 200:
                    ...
                    return [
                        self._parse_pathnet_order(order)
                        for order in worklist_data.get("orders", [])
                    ]
                else:
    ...
    def _parse_pathnet_order(self, order_data: Dict[str, Any]) -> PathNetOrder:
        ...
        return PathNetOrder(
            ...
            collected_datetime=(
                datetime.fromisoformat(order_data["collected_datetime"])
                if order_data.get("collected_datetime")
                else None
            ),
            received_datetime=(
                datetime.fromisoformat(order_data["received_datetime"])
                if order_data.get("received_datetime")
                else None
            ),
            resulted_datetime=(
                datetime.fromisoformat(order_data["resulted_datetime"])
                if order_data.get("resulted_datetime")
                else None
            ),
