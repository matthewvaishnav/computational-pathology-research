        ...
        def wrapper(*args, **kwargs):
            ...
            if self.policy.audit_all_operations:
                self._audit_operation(
                    operation_name, {"args": str(args)[:100], "kwargs": str(kwargs)[:100]}
                )
...
if __name__ == "__main__":
    ...
    policy = SecurityPolicy(
        ...
        resource_limits=ResourceLimits(
            max_memory_mb=256, max_cpu_percent=25, max_execution_time_sec=60
        ),
    ...
    # Test validation
    print(
        f"Can read /data/public/file.txt: {sandbox.validate_file_access('/data/public/file.txt')}"
    )
    print(
        f"Can write /data/public/file.txt: {sandbox.validate_file_access('/data/public/file.txt', write=True)}"
    )
