    ...
    def _compute_uncertainty_metrics(
        ...
        if disease_type:
            ...
            for disease in predictions[0].keys():
                ...
                disease_total_entropy = -(mean_pred * torch.log(mean_pred + 1e-8)).sum(dim=1).mean().item()
...
class EnsembleUncertainty:
    ...
    def _compute_ensemble_uncertainty(
        ...
        if disease_type:
            ...
            total_uncertainty = -(ensemble_pred * torch.log(ensemble_pred + 1e-8)).sum(dim=1).mean().item()
            ...
            for disease in predictions[0].keys():
                ...
                disease_total = -(ensemble_pred * torch.log(ensemble_pred + 1e-8)).sum(dim=1).mean().item()
...
class UncertaintyQuantificationSystem:
    ...
    def estimate_uncertainty(
        ...
        if method == "mc_dropout" or (method == "ensemble" and self.ensemble is None):
            uncertainty_metrics = self.mc_dropout.estimate_uncertainty(
                patches, disease_type
            )
        elif method == "ensemble":
            uncertainty_metrics = self.ensemble.estimate_uncertainty(
                patches, disease_type
            )
        elif method == "both":
            ...
            uncertainty_metrics = UncertaintyMetrics(
                epistemic_uncertainty=(mc_metrics.epistemic_uncertainty + ensemble_metrics.epistemic_uncertainty) / 2,
                aleatoric_uncertainty=(mc_metrics.aleatoric_uncertainty + ensemble_metrics.aleatoric_uncertainty) / 2,
                total_uncertainty=(mc_metrics.total_uncertainty + ensemble_metrics.total_uncertainty) / 2,
                confidence_interval=(
                ...
                mutual_information=(mc_metrics.mutual_information + ensemble_metrics.mutual_information) / 2,
                expected_calibration_error=0.0,
                reliability_score=(mc_metrics.reliability_score + ensemble_metrics.reliability_score) / 2,
                prediction_variance=(mc_metrics.prediction_variance + ensemble_metrics.prediction_variance) / 2,
    ...
    def fit_calibrator(
        ...
        self.logger.info(f"Calibrator fitted. ECE: {calibration_metrics.expected_calibration_error:.4f}")
    ...
    def should_request_second_opinion(
        ...
        if uncertainty_metrics.epistemic_uncertainty > thresholds['epistemic_uncertainty']:
            reasons.append(f"High epistemic uncertainty ({uncertainty_metrics.epistemic_uncertainty:.3f})")
        
        if uncertainty_metrics.ensemble_disagreement > thresholds['ensemble_disagreement']:
            reasons.append(f"High ensemble disagreement ({uncertainty_metrics.ensemble_disagreement:.3f})")
        
        ci_width = uncertainty_metrics.confidence_interval[1] - uncertainty_metrics.confidence_interval[0]
