...
class IntelligentMedicalReferee:
    ...
    def arbitrate_predictions(self, fl_result: Dict, dmi_result: Dict, case_data: Dict) -> Dict:
        ...
        explanation = self._generate_explanation(
            fl_prob, dmi_prob, final_prob, decision_type, 
            fl_reasoning, dmi_reasoning, evidence_alignment
        )
    ...
    def _cross_validate_evidence(self, fl_reasoning: Dict, dmi_reasoning: Dict, case_data: Dict) -> Dict:
        ...
        if common_indicators:
            ...
            for indicator in common_indicators:
                ...
                if (fl_weight > 0.5 and clinical_weight > 0.5) or (fl_weight < 0.5 and clinical_weight < 0.5):
        ...
        data_quality = min(fl_reasoning.get("training_data_size", 0) / 100000, 1.0)  # Normalize to 100k samples
        expertise_quality = min(dmi_reasoning.get("expertise_weight", 0) / 10.0, 1.0)  # Normalize to weight 10
    ...
    def _deep_analysis_arbitration(self, fl_result: Dict, dmi_result: Dict, 
                                 fl_reasoning: Dict, dmi_reasoning: Dict, 
                                 evidence_alignment: Dict) -> Dict:
    ...
    def _generate_explanation(self, fl_prob: float, dmi_prob: float, final_prob: float,
                            decision_type: str, fl_reasoning: Dict, dmi_reasoning: Dict,
                            evidence_alignment: Dict) -> str:
        ...
        if decision_type == "consensus":
            ...
            explanation.append(f"  • Strong data foundation ({fl_reasoning.get('training_data_size', 0):,} samples)")
            explanation.append(f"  • High model confidence ({fl_reasoning.get('model_confidence', 0):.1%})")
            
        elif decision_type == "dmi_favored":
            explanation.append("⚖️ Favoring medical experts because:")
            explanation.append(f"  • High expertise weight ({dmi_reasoning.get('expertise_weight', 0):.1f})")
            explanation.append(f"  • Strong consensus ({dmi_reasoning.get('consensus_strength', 0):.1%})")
