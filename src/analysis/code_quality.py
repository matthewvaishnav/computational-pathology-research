...
class CodeQualityScanner:
    ...
    def _measure_documentation_coverage(self) -> float:
        ...
        try:
            ...
            return (
                round((documented_functions / total_functions) * 100, 2)
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.FunctionDef):
                            ...
                            if not re.match(
                                ...
                                suggested = (
                                    re.sub(r"([A-Z])", r"_\1", node.name).lower().lstrip("_")
                                )
                                violations.append(
                                    {
                                        "file": str(py_file),
        ...
        try:
            ...
            for py_file in python_files:
                try:
                    ...
                    overall_score = complexity_score * 0.4 + doc_score * 0.3 + pylint_score * 0.3
