...
4. Document optimization in code comments"""
        ...
        return guides.get(issue.category, f"""1. Review issue in {issue.file_path}
2. {issue.recommendation}
    ...
    def _resolve_dependencies(self, tasks: List[Task]) -> Dict[str, List[str]]:
        ...
        for i, task in enumerate(tasks):
            for j, other_task in enumerate(tasks):
                ...
                if (task.role == Role.SECURITY and 
                    other_task.role != Role.SECURITY and
                    task.priority.value < other_task.priority.value):
                    dependencies[other_task.id].append(task.id)
                
                # Architecture before performance
                if ('architecture' in task.title.lower() and 
                    'performance' in other_task.title.lower()):
                    dependencies[other_task.id].append(task.id)
                
                # Coverage before quality
                if ('coverage' in task.title.lower() and 
                    'quality' in other_task.title.lower()):
                    dependencies[other_task.id].append(task.id)
