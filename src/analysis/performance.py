...
class PerformanceProfiler:
    ...
    def _measure_memory_usage(self) -> tuple[float, float]:
        ...
        try:
            ...
            top_objects = sorted(
                [
                    (obj_type, size, count)
                    for obj_type, (size, count) in zip(
                        object_sizes.keys(), zip(object_sizes.values(), object_counts.values())
                    )
                ],
                key=lambda x: x[1],
                reverse=True,
            ...
            if size > 1e8:  # > 100MB
                if obj_type in ["list", "dict", "tuple"]:
                    recommendations.append(
                        f"Large {obj_type} objects detected - consider using generators or chunking"
                    )
                elif obj_type == "Tensor":
                    recommendations.append(
                        "Large tensors in memory - consider gradient checkpointing"
                    )
    ...
    def _generate_flame_graph(self) -> str:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["py-spy", "--version"], capture_output=True, timeout=5, check=False
                )
    ...
    def _calculate_performance_score(
        self, gpu_util: float, bottlenecks: List[Dict[str, Any]], memory_peak: float
    ) -> float:
        """
