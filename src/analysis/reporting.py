...
|-----------|-------|------------|"""
    ...
    def _generate_critical_issues(self, result: AnalysisResult) -> str:
        ...
        for i, issue in enumerate(result.critical_issues, 1):
            ...
            issue_lines.append(
                f"""### {i}. {issue.title} {priority_emoji} {severity_emoji}
...
"""
        ...
        if p0_tasks:
            ...
            for i, task in enumerate(p0_tasks, 1):
                task_lines.append(
                    f"{i}. **{task.title}** ({task.effort_hours:.1f}h, {task.role.value})\n"
                )
            ...
            with tempfile.NamedTemporaryFile(
                ...
                if output_path:
                    ...
                    result_proc = subprocess.run(
                        ["pandoc", md_file.name, "-t", "html", "--standalone"],
                        capture_output=True,
                        text=True,
                        check=True,
                    )
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                ...
                try:
                    ...
                    # Fallback to pandoc
                    subprocess.run(
                        ["pandoc", html_file.name, "-o", output_path, "--pdf-engine=wkhtmltopdf"],
                        check=True,
                    )
