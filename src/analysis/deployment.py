...
class DeploymentValidator:
    ...
    def _validate_dockerfile(self) -> float:
        ...
        try:
            ...
            run_indices = [
                i for i, line in enumerate(lines) if "RUN" in line and "pip install" in line.lower()
            ]

            if copy_indices and run_indices:
                # Good: COPY requirements.txt before RUN pip install
                req_copy = any(
                    "requirements" in lines[i].lower() or "pyproject" in lines[i].lower()
        ...
        try:
            ...
            for py_file in self.project_path.rglob("*.py"):
                try:
                    ...
                    if any(
                        endpoint in content
                        for endpoint in ["/health", "/healthz", "/ready", "/liveness"]
                    ):
    ...
    def _calculate_deployment_score(
        self, dockerfile_score: float, k8s_score: float, ci_cd_score: float, monitoring_score: float
    ) -> float:
        """
