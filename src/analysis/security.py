...
class SecurityScanner:
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    if critical_issues:
                        logger.warning(
                            f"CRITICAL: {len(critical_issues)} high-severity security issues found!"
                        )
                        for issue in critical_issues[:3]:  # Log first 3 critical issues
                            logger.warning(
                                f"  - {issue['type']} in {Path(issue['file']).name}:{issue['line']}"
                            )
            ...
            if result.returncode != 0:
                logger.warning(
                    f"Bandit scan completed with warnings (exit code: {result.returncode})"
                )
                if result.stderr:
            ...
            logger.warning(
                "Bandit not installed - install with 'pip install bandit' for security scanning"
            )
            return []
    ...
    def _assess_hipaa_compliance(self) -> float:
        ...
        if encryption:
            ...
            logger.warning(
                "✗ Missing encryption - HIPAA requires data encryption at rest and in transit"
            )
        ...
        if missing_controls:
            ...
            logger.info(
                "Refer to HIPAA Security Rule 45 CFR §164.308-318 for implementation guidance"
            )
