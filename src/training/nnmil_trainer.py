...
class nnMILTrainer:
    ...
    def __init__(
        ...
        self.scaler = (
            torch.cuda.amp.GradScaler() if self.use_amp and self.device == "cuda" else None
        )
    ...
    def _log_batch_metrics(self, epoch: int, batch_idx: int, loss: float):
        ...
        self.writer.add_scalar(
            "Train/LearningRate", self.optimizer.param_groups[0]["lr"], global_step
        )
    ...
    def get_training_stats(self) -> Dict[str, Any]:
        ...
        final_val_metric = (
            self.training_history["val_metric"][-1] if self.training_history["val_metric"] else None
        )
        ...
        return {
            ...
            "avg_epoch_time": sum(self.training_history["epoch_time"])
            / len(self.training_history["epoch_time"]),
        }
