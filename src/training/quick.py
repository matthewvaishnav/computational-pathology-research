...
class QuickTrainer:
    ...
    def __init__(self, dataset="pcam", model="nnmil", epochs=10, batch_size=32, 
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.dataset == "pcam":
            ...
            train_dataset = PCamDataset(
                root="data/pcam", 
                split="train", 
                transform=get_pcam_transforms("train")
            )
            val_dataset = PCamDataset(
                root="data/pcam", 
                split="val", 
                transform=get_pcam_transforms("val")
            )
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                if batch_idx % 100 == 0:
                    logger.info(f"Epoch {epoch+1}/{self.epochs}, Batch {batch_idx}, Loss: {loss.item():.4f}")
