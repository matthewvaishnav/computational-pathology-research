    ...
    def _calculate_clinical_priority(
        ...
        if strategy == SamplingStrategy.UNCERTAINTY:
            ...
            return (
                0.5 * base_priority +
                0.3 * clinical_score +
                0.2 * (base_priority * diversity_score)
            )
...
class ActiveLearningSystem:
    ...
    def __init__(
        ...
        self.sampler = UncertaintyBasedSampler(
    ...
    def identify_uncertain_cases(
        ...
        cases = self.sampler.identify_uncertain_cases(
            ...
            self.sampling_strategy, feature_vectors
        )
        ...
        if success:
            ...
            if annotation.annotation_time_seconds > 0:
                ...
                self.stats['avg_annotation_time'] = (
                    (current_avg * (count - 1) + annotation.annotation_time_seconds) / count
                )
    ...
    def _store_cases(self, cases: List[CaseForReview]):
        ...
        for case in cases:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                case.case_id, case.slide_id, case.image_path,
                json.dumps(case.prediction), case.uncertainty_score,
                case.confidence, case.disease_type, case.clinical_priority,
                json.dumps(case.metadata), case.identified_at.isoformat()
            ))
    ...
    def _store_task(self, task: AnnotationTask):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            task.task_id, task.case_data.case_id, task.priority,
            task.assigned_expert, task.status.value,
    ...
    def _store_annotation(self, annotation: ExpertAnnotation):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            annotation.case_id, annotation.expert_id, annotation.diagnosis,
            annotation.confidence, annotation.grade, annotation.stage,
            json.dumps(annotation.molecular_markers), annotation.comments,
            annotation.annotation_time_seconds, annotation.quality_score,
