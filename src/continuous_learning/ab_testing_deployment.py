...
class ABTestingDeployment:
    ...
    async def _monitor_ab_test(self, experiment: ABTestExperiment) -> None:
        ...
        while experiment.status == ABTestStatus.RUNNING:
            try:
                ...
                control_metrics = await self._collect_group_metrics(experiment, "control")
                treatment_metrics = await self._collect_group_metrics(experiment, "treatment")
                ...
                if (
                    len(experiment.control_metrics_history) >= 5
                    and control_metrics.total_samples >= experiment.config.min_samples_per_group
                    and treatment_metrics.total_samples >= experiment.config.min_samples_per_group
                ):
    ...
    async def _collect_group_metrics(
        self, experiment: ABTestExperiment, group: str
    ) -> ABTestMetrics:
        """Collect metrics for a test group."""
        if self.metrics_collector:
            # Use provided metrics collector
            raw_metrics = await self._run_async(self.metrics_collector, group, experiment.test_id)
    ...
    async def _check_safety_conditions(self, experiment: ABTestExperiment) -> None:
        ...
        if accuracy_degradation > experiment.config.rollback_threshold_degradation:
            await self._rollback_experiment(
                experiment, f"Significant accuracy degradation: {accuracy_degradation:.3f}"
            )
            return
        ...
        if latest_treatment.error_rate > 0.05:  # 5% error rate threshold
            await self._rollback_experiment(
                experiment, f"High error rate in treatment group: {latest_treatment.error_rate:.3f}"
            )
            return
        ...
        if latest_treatment.mean_response_time_ms > latest_control.mean_response_time_ms * 2:
            await self._rollback_experiment(
                experiment, f"Response time degradation in treatment group"
            )
            return
    ...
    async def _perform_statistical_analysis(
        ...
        # Confidence interval
        t_critical = (
            stats.t.ppf(1 - experiment.config.significance_level / 2, df) if df > 0 else 1.96
        )
...
if __name__ == "__main__":
    ...
    async def wait_for_completion():
        ...
        print(
            f"Duration: {(final_experiment.end_time - final_experiment.start_time) / 60:.1f} minutes"
        )
