...
class PostDeploymentMonitor:
    ...
    async def _check_sla_compliance(self) -> None:
        ...
        recent_metrics = [m for m in self.performance_metrics if m.timestamp > hour_ago]
    ...
    async def _check_alert_conditions(self) -> None:
        ...
        if (
            ...
            > self.alert_thresholds["response_time_increase"] * 200
        ):  # 2x baseline
            await self._create_alert(
                AlertSeverity.WARNING,
    ...
    async def _create_alert(
        ...
        if (
            ...
            and current_time - self.last_alert_time[alert_key] < self.alert_cooldown
        ):
            return
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        for model_id in self.model_metrics:
            old_metrics = [m for m in self.model_metrics[model_id] if m.timestamp > cutoff_time]
    ...
    def register_model(self, model_id: str, model_info: Dict[str, Any]) -> None:
        ...
        self.active_models[model_id] = {**model_info, "registration_time": time.time()}
    ...
    def export_monitoring_report(self, filepath: str, hours: int = 24) -> None:
        ...
        report = {
            ...
            "summary": summary,
            "recent_metrics": [
                asdict(m) for m in self.performance_metrics if m.timestamp > cutoff_time
            ][
                -100:
            ],  # Last 100 metrics
            "recent_alerts": [
                asdict(alert) for alert in self.alerts if alert.timestamp > cutoff_time
            ],
            "sla_metrics": [asdict(sla) for sla in self.sla_metrics if sla.timestamp > cutoff_time],
            "configuration": {
...
if __name__ == "__main__":
    ...
    async def run_demo():
        ...
        print(
            f"Average response time: {summary['performance_summary']['average_response_time_ms']:.1f}ms"
        )
