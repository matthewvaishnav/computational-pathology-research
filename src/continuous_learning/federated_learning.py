...
class PrivacyBudget:
    ...
    def is_exhausted(self) -> bool:
        return (self.consumed_epsilon >= self.max_epsilon or 
                self.consumed_delta >= self.max_delta)
    ...
    async def train_local_model(
        ...
        try:
            ...
            if self.privacy_engine and OPACUS_AVAILABLE:
                local_model, optimizer, train_loader = self.privacy_engine.make_private_with_epsilon(
                    module=local_model,
                    optimizer=optimizer,
                    data_loader=train_loader,
                    epochs=self.config.local_epochs,
                    target_epsilon=self.config.privacy_budget.remaining_epsilon / 10,  # Conservative
            ...
            for epoch in range(self.config.local_epochs):
                ...
                for batch_idx, (data, target) in enumerate(train_loader):
                    ...
                    if isinstance(output, dict):
                        ...
                        pred_logits = next(iter([v for k, v in output.items() 
                ...
                self.logger.debug(f"Epoch {epoch + 1}/{self.config.local_epochs}, Loss: {epoch_loss:.4f}")
            
            # Get privacy spent
            if self.privacy_engine and OPACUS_AVAILABLE:
                privacy_spent = self.privacy_engine.get_epsilon(
                    delta=self.config.privacy_budget.delta
...
class FederatedLearningCoordinator:
    ...
    async def federated_training_round(
        ...
        active_participants = [
            hospital_id for hospital_id in participating_hospitals
            if (hospital_id in self.active_clients and 
                self.registered_hospitals[hospital_id].enabled and
                self.active_clients[hospital_id].status != ClientStatus.FAILED)
        ]
        
        if len(active_participants) < self.min_participants:
            raise ValueError(f"Not enough participants: {len(active_participants)} < {self.min_participants}")
        
        self.logger.info(f"Starting round {self.current_round} with {len(active_participants)} hospitals")
        ...
        self.logger.info(f"Round {self.current_round} completed in {round_result.round_duration:.2f}s")
        return round_result
    ...
    def _calculate_communication_overhead(self, updates: List[ModelUpdate]) -> float:
        ...
        model_size_mb = sum(
            ...
            for param in self.central_model.parameters()
        ) / (1024 * 1024)
