...
class UncertaintyEstimator:
    ...
    def __call__(
            ...
            for _ in range(self.num_samples):
                ...
                if hasattr(self.model, 'forward_with_attention'):
                    ...
                    attention_weights = torch.ones(
                        1, features.shape[0], device=self.device
                    )
        ...
        # Stack predictions
        stacked_logits = torch.stack(all_logits, dim=0)  # [num_samples, num_classes] or [num_samples]
    ...
    def estimate_batch_uncertainty(
        ...
        # Stack predictions
        stacked_logits = torch.stack(all_logits, dim=0)  # [num_samples, B, num_classes] or [num_samples, B]
        ...
        if self.normalize:
            ...
            if probabilities is not None:
                ...
                max_entropy = torch.log(torch.tensor(probabilities.shape[-1], dtype=aleatoric_uncertainties.dtype))
