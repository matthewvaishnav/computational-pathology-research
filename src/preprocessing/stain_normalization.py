...
class MacenkoNormalizer(StainNormalizer):
    ...
    def fit(self, target_image: np.ndarray) -> "MacenkoNormalizer":
        ...
        self.max_c_target = np.percentile(self.target_concentrations.reshape(-1, 2), 99, axis=0)
        return self
    ...
    def transform(self, image: np.ndarray) -> np.ndarray:
        ...
        max_c_source = np.percentile(source_concentrations.reshape(-1, 2), 99, axis=0)
...
def normalize_stain(image: np.ndarray, target: np.ndarray, method: str = "macenko") -> np.ndarray:
    """
