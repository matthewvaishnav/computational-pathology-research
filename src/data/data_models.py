...
class Bag:
    ...
    def __post_init__(self):
        ...
        if self.features.dim() != 2:
            raise ValueError(
                f"features must be 2D tensor [N, D], got {self.features.dim()}D"
            )
...
class TrainingBatch:
    ...
    def __post_init__(self):
        ...
        if self.features.dim() != 3:
            raise ValueError(
                f"features must be 3D tensor [B, M, D], got {self.features.dim()}D"
            )
        ...
        if self.masks.dtype != torch.bool:
            raise ValueError(
                f"masks must have dtype torch.bool, got {self.masks.dtype}"
            )
        ...
        if len(self.slide_ids) != batch_size:
            raise ValueError(
                f"slide_ids length {len(self.slide_ids)} does not match "
                f"batch size {batch_size}"
            )
...
class InferenceOutput:
    ...
    def __post_init__(self):
        ...
        if self.logits.dim() == 1:
            ...
            raise ValueError(
                f"logits must be 1D [B] or 2D [B, num_classes], "
        ...
        if self.probabilities is not None:
            if num_classes is None:
                raise ValueError(
                    "probabilities should be None for regression tasks"
                )
