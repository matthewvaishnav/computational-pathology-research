...
class FixedLengthBagSampler:
    ...
    def __init__(
        ...
        if not (100 <= bag_length <= 10000):
            raise ValueError(f"bag_length must be in range [100, 10000], got {bag_length}")
        ...
        if tissue_aware and tissue_classifier is None:
            raise ValueError("tissue_classifier must be provided when tissue_aware=True")
    ...
    def sample(self, features: torch.Tensor, num_patches: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def _pad_bag(
        self, features: torch.Tensor, N: int, M: int, D: int
    ...
    def _random_sample(
        self, features: torch.Tensor, N: int, M: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        if self.tissue_aware and self.tissue_classifier is not None:
            ...
            with torch.no_grad():
                ...
                importance = self.tissue_classifier.get_importance_weights(
                    features[:N], temperature=self.temperature
                )  # [N]
    ...
    def _sliding_window_sample(
        ...
        logger.debug(f"Sliding window sample: first {M} patches from {N} (inference mode)")
