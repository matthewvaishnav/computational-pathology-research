...
class BalancedBatchSampler(torch.utils.data.Sampler):
    ...
    def __init__(self, labels: torch.Tensor, batch_size: int = 32, shuffle: bool = True):
        """
...
class RegressionBatchSampler(torch.utils.data.Sampler):
    ...
    def __init__(self, targets: torch.Tensor, batch_size: int = 32, num_bins: int = 10):
        """
...
class SurvivalBatchSampler(torch.utils.data.Sampler):
    ...
    def __init__(self, times: torch.Tensor, events: torch.Tensor, batch_size: int = 32):
        """
        ...
        if len(times) != len(events):
            raise ValueError(
                f"times and events must have same length, " f"got {len(times)} and {len(events)}"
            )
        ...
        if not all(e in [0, 1] for e in unique_events.tolist()):
            raise ValueError(f"events must contain only 0 or 1, got {unique_events.tolist()}")
        ...
        if self.samples_per_event > 0:
            event_batches = (
                len(self.event_indices) + self.samples_per_event - 1
        ...
        if self.samples_per_censored > 0:
            censored_batches = (
                len(self.censored_indices) + self.samples_per_censored - 1
            ) // self.samples_per_censored
        else:
            censored_batches = 0
