...
from .wsi_stream_reader import StreamingProgressTracker, StreamingProgress, ProgressCallback
...
def demo_progress_tracker():
    ...
    def progress_callback(progress: StreamingProgress):
        print(f"📊 Progress Update: {progress.get_progress_percentage()} "
    ...
    for i, (proc_time, confidence) in enumerate(zip(processing_times, confidences)):
        ...
        if progress.early_stop_recommended:
            ...
            print(f"   Confidence: {progress.current_confidence:.3f} >= {progress.confidence_threshold:.3f}")
            break
...
def demo_eta_accuracy():
    ...
    for scenario in scenarios:
        ...
        print(f"     Accuracy: {(1 - abs(predicted_total - actual_total) / actual_total) * 100:.1f}%")
...
def demo_confidence_tracking():
    ...
    confidence_values = [
        0.1, 0.2, 0.35, 0.5, 0.65, 0.75, 0.82, 0.88, 0.91, 0.93, 0.95, 0.96
    ]
    ...
    for i, target_confidence in enumerate(confidence_values):
        ...
        print(f"📊 Batch {i+1}: Confidence {target_confidence:.3f}, "
