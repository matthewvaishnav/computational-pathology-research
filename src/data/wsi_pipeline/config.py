...
class ProcessingConfig:
    ...
    def validate(self) -> None:
        ...
        if not (64 <= self.patch_size <= 2048):
            errors.append(f"patch_size must be between 64 and 2048, got {self.patch_size}")

        # Validate tissue_threshold
        if not (0.0 <= self.tissue_threshold <= 1.0):
            errors.append(
                f"tissue_threshold must be between 0.0 and 1.0, " f"got {self.tissue_threshold}"
            )

        # Validate num_workers
        if not (1 <= self.num_workers <= 16):
            errors.append(f"num_workers must be between 1 and 16, got {self.num_workers}")

        # Validate max_retries
        if not (0 <= self.max_retries <= 5):
            errors.append(f"max_retries must be between 0 and 5, got {self.max_retries}")
        ...
        if self.target_mpp is not None and self.target_mpp <= 0:
            errors.append(f"target_mpp must be positive, got {self.target_mpp}")

        # Validate tissue_method
        valid_methods = ["otsu", "deep_learning", "hybrid"]
        if self.tissue_method not in valid_methods:
            errors.append(
                f"tissue_method must be one of {valid_methods}, " f"got {self.tissue_method}"
            )

        # Validate blur_threshold
        if self.blur_threshold < 0:
            errors.append(f"blur_threshold must be non-negative, got {self.blur_threshold}")
        ...
        if compression_check not in valid_compression:
            errors.append(
                f"compression must be one of {valid_compression}, " f"got {self.compression}"
            )
