...
class TileBufferConfig:
    ...
    def validate(self) -> None:
        ...
        if not (0.5 <= self.min_memory_gb <= 32.0):
            raise ValueError(f"min_memory_gb must be between 0.5 and 32.0, got {self.min_memory_gb}")
        
        if not (0.5 <= self.max_memory_gb <= 32.0):
            raise ValueError(f"max_memory_gb must be between 0.5 and 32.0, got {self.max_memory_gb}")
        
        if self.min_memory_gb > self.max_memory_gb:
            raise ValueError(f"min_memory_gb ({self.min_memory_gb}) cannot exceed max_memory_gb ({self.max_memory_gb})")
        
        if not (1 <= self.initial_buffer_size <= self.max_buffer_size):
            raise ValueError(f"initial_buffer_size must be between 1 and max_buffer_size ({self.max_buffer_size})")
        ...
        if self.min_tile_size > self.max_tile_size:
            raise ValueError(f"min_tile_size ({self.min_tile_size}) cannot exceed max_tile_size ({self.max_tile_size})")
        
        if not (self.min_tile_size <= self.tile_size <= self.max_tile_size):
            raise ValueError(f"tile_size ({self.tile_size}) must be between min_tile_size ({self.min_tile_size}) and max_tile_size ({self.max_tile_size})")
        
        if not (0.1 <= self.memory_pressure_tile_reduction <= 1.0):
            raise ValueError(f"memory_pressure_tile_reduction must be between 0.1 and 1.0, got {self.memory_pressure_tile_reduction}")
        
        if not (0.1 <= self.memory_pressure_threshold <= 1.0):
            raise ValueError(f"memory_pressure_threshold must be between 0.1 and 1.0, got {self.memory_pressure_threshold}")
        
        if not (0.1 <= self.cleanup_threshold <= 1.0):
            raise ValueError(f"cleanup_threshold must be between 0.1 and 1.0, got {self.cleanup_threshold}")
...
class TileBufferPool:
    ...
    def _compress_tile(self, tile_data: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if tile_data.dtype != np.uint8:
                ...
                tile_data = ((tile_data - tile_data.min()) / 
                           (tile_data.max() - tile_data.min()) * 255).astype(np.uint8)
    ...
    def _decompress_tile(self, compressed_data: np.ndarray, original_shape: Tuple[int, ...]) -> np.ndarray:
        """
    ...
    def _evict_lru_tiles(self, target_memory_bytes: int) -> int:
        ...
        while (self._current_memory_bytes > target_memory_bytes and 
               len(self._tiles) > 0):
    ...
    def calculate_adaptive_tile_size(self) -> int:
        ...
        if memory_pressure or gpu_memory_pressure:
            ...
            base_tile_size = max(
                ...
                int(base_tile_size * pressure_reduction)
            )
        ...
        if self._check_memory_pressure():
            ...
            base_tile_size = max(
                self.config.min_tile_size,
                int(base_tile_size * buffer_pressure_reduction)
            )
        ...
        if gpu_memory_pressure:
            ...
            base_tile_size = max(
                self.config.min_tile_size,
                int(base_tile_size * gpu_pressure_reduction)
            )
        
        # Ensure tile size is within bounds and is a power of 2 for efficiency
        adaptive_tile_size = max(self.config.min_tile_size, min(self.config.max_tile_size, base_tile_size))
        ...
        adaptive_tile_size = max(self.config.min_tile_size, min(self.config.max_tile_size, adaptive_tile_size))
    ...
    def get_adaptive_sizing_stats(self) -> Dict[str, Union[int, float, List]]:
        ...
        return {
            ...
            'recent_changes': self._adaptive_sizing_history[-10:] if self._adaptive_sizing_history else [],
            'recommended_tile_size': self.calculate_adaptive_tile_size(),
        }
    
    def estimate_tile_memory_usage(self, tile_size: int, channels: int = 3, dtype_bytes: int = 1) -> int:
        """
    ...
    def get_optimal_batch_size_for_tile_size(self, tile_size: int, target_memory_gb: float = 1.0) -> int:
        """
    ...
    def store_tile(
        ...
        def _store():
            try:
                ...
                needs_eviction = (
                    required_memory > self._max_memory_bytes or 
                    len(self._tiles) >= self.config.max_buffer_size
                )
                ...
                if needs_eviction:
                    ...
                    while (self._current_memory_bytes > target_memory or 
                           len(self._tiles) >= target_tile_count) and len(self._tiles) > 0:
                ...
                self._metadata[key] = TileMetadata(
                    ...
                    access_count=1,
                    compressed=self.config.compression_enabled and (compressed_size < original_size)
                )
