...
class TestStreamingProgressTracker(unittest.TestCase):
    ...
    def test_tile_processing_tracking(self):
        ...
        self.tracker.record_tile_processed(
            processing_time=0.1, tile_size=1024, success=True, skipped=False
        )
        ...
        # Record failed tile
        self.tracker.record_tile_processed(
            processing_time=0.05, tile_size=1024, success=False, skipped=False
        )
        ...
        # Record skipped tile
        self.tracker.record_tile_processed(
            processing_time=0.02, tile_size=1024, success=True, skipped=True
        )
    ...
    def test_eta_calculation(self):
        ...
        for i in range(10):
            self.tracker.record_tile_processed(
                processing_time=0.1, tile_size=1024, success=True, skipped=False  # 100ms per tile
            )
    ...
    def test_progress_callbacks(self):
        ...
        callback_config = ProgressCallback(
            callback_func=callback_mock, update_interval=0.1, min_progress_delta=0.05
        )

        tracker = StreamingProgressTracker(total_tiles=20, progress_callbacks=[callback_config])
...
class TestWSIStreamReaderProgressIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = TileBufferConfig(
            max_memory_gb=1.0, tile_size=1024, adaptive_sizing_enabled=True
        )
    ...
    def test_progress_tracking_initialization(self, mock_wsi_reader_class):
        ...
        reader = WSIStreamReader(
            self.mock_wsi_path, config=self.config, progress_callbacks=progress_callbacks
        )

        # Initialize streaming
        metadata = reader.initialize_streaming(
            target_processing_time=30.0, confidence_threshold=0.95
        )
    ...
    def test_progress_callback_integration(self, mock_wsi_reader_class):
        ...
        reader.add_progress_callback(
            callback_func=callback_mock, update_interval=0.1, min_progress_delta=0.01
        )
