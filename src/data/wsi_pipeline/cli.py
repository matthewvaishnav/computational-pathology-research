...
Examples:
  ...
  python -m data.wsi_pipeline.cli config --generate-docs
    ...
    # Process command
    process_parser = subparsers.add_parser("process", help="Process WSI files to extract features")
    process_parser.add_argument(
        "input_files", nargs="+", type=Path, help="WSI files to process (supports wildcards)"
    )
    process_parser.add_argument(
        "--output-dir",
        "-o",
    ...
    process_parser.add_argument(
        "--num-workers", type=int, default=4, help="Number of worker processes (default: 4)"
    )
    process_parser.add_argument(
        "--gpu-ids", nargs="*", type=int, help="GPU IDs to use (default: auto-detect)"
    )
    process_parser.add_argument("--batch-size", type=int, help="Batch size for feature extraction")
    process_parser.add_argument("--patch-size", type=int, help="Patch size in pixels")
    process_parser.add_argument(
        "--encoder",
    ...
    process_parser.add_argument(
        "--tissue-threshold", type=float, help="Tissue detection threshold (0.0-1.0)"
    )
    process_parser.add_argument(
    ...
    benchmark_parser.add_argument(
        "--quick", action="store_true", help="Run quick benchmarks with smaller test sizes"
    )
    benchmark_parser.add_argument("--output", type=Path, help="Save benchmark results to JSON file")
    benchmark_parser.add_argument("--config", type=Path, help="Configuration file for benchmarking")

    # Validate command
    validate_parser = subparsers.add_parser(
        "validate", help="Validate pipeline installation and functionality"
    )
    validate_parser.add_argument("--output", type=Path, help="Save validation results to JSON file")
    validate_parser.add_argument("--config", type=Path, help="Configuration file for validation")

    # Config command
    config_parser = subparsers.add_parser("config", help="Configuration management utilities")
    config_group = config_parser.add_mutually_exclusive_group(required=True)
    config_group.add_argument(
        "--generate-docs", action="store_true", help="Generate configuration documentation"
    )
    config_group.add_argument(
        "--create-template",
        type=str,
    ...
    config_group.add_argument("--validate", type=Path, help="Validate configuration file")
    config_parser.add_argument("--output", type=Path, help="Output file for generated content")
