...
class AzureMonitorIntegration:
    ...
    def _batch_processor(self) -> None:
        ...
        while not self.stop_batching.is_set():
            try:
                ...
                metrics_batch = []
                while (
                    len(metrics_batch) < self.config.batch_size and not self.metrics_queue.empty()
                ):
                    try:
            ...
            for metric in metrics:
                ...
                metric_entry = {
                    ...
                    "data": {
                        "baseType": "MetricData",
                        "baseData": {
                            "metrics": [{"name": metric.name, "value": metric.value, "count": 1}],
    ...
    def _send_logs_batch(self, logs: List[LogEntry]) -> None:
        ...
        try:
            ...
            self.logs_ingestion_client.upload(
                rule_id="histocore-logs", stream_name="Custom-HistoCoreLogs_CL", logs=log_records
            )
    ...
    def track_slide_processing(
        ...
        self.send_custom_metric(
            CustomMetric(
                ...
                dimensions={"slide_id": slide_id, "success": str(success)},
                unit="seconds",
            )
        )
    ...
    def track_model_inference(
        self, model_name: str, inference_time: float, confidence: float, slide_id: str
    ) -> None:
        ...
        self.send_custom_metric(
            CustomMetric(
                ...
                metric_type=MetricType.TIMER,
                dimensions={"model_name": model_name, "slide_id": slide_id},
                unit="seconds",
            )
        )
        ...
        self.send_custom_metric(
            CustomMetric(
                ...
                value=1,
                metric_type=MetricType.COUNTER,
                dimensions={"model_name": model_name},
            )
        )

    def track_system_performance(
        self, cpu_usage: float, memory_usage: float, gpu_usage: Optional[float] = None
    ) -> None:
    ...
    def get_metrics(
        self, metric_names: List[str], start_time: datetime, end_time: datetime, resource_id: str
    ) -> Dict[str, List[Dict]]:
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            test_metric = CustomMetric(
                name="histocore.health_check", value=1, metric_type=MetricType.COUNTER
            )
            self.send_custom_metric(test_metric)

            # Test log submission
            test_log = LogEntry(message="Azure Monitor health check", level="INFO")
...
def create_monitor_integration(
    subscription_id: str, resource_group: str, workspace_name: str, **kwargs
...
def setup_histocore_monitoring(
    subscription_id: str, resource_group: str, workspace_name: str, application_insights_key: str
) -> AzureMonitorIntegration:
