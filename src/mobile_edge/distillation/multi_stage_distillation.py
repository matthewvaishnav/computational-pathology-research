...
from .distillation_loss import DistillationConfig, DistillationLoss
from .temperature_scheduling import TemperatureScheduleConfig, TemperatureScheduler
...
class MultiStageDistiller:
    ...
    def distill(
        ...
        device: torch.device,
        create_intermediate_fn: callable,
    ) -> nn.Module:
        """
        ...
        for stage_idx, (intermediate, epochs) in enumerate(
            ...
            self.stage_history.append(
                {"stage": stage_idx + 1, "epochs": epochs, "stats": stage_stats}
            )
    ...
    def _train_stage(
        ...
        # Training loop
        best_acc = 0.0
        stats = {"train_loss": [], "val_acc": [], "temperatures": []}
    ...
    def _validate(self, model: nn.Module, val_loader: DataLoader, device: torch.device) -> float:
        """Validate model"""
...
class ProgressiveDistiller:
    ...
    def __init__(self, num_steps: int = 5, compression_ratio: float = 0.5):
    ...
    def distill(
        ...
        for step_idx, size_ratio in enumerate(size_schedule):
            self.logger.info(
                f"Step {step_idx + 1}/{len(size_schedule)} - " f"Size ratio: {size_ratio:.3f}"
            )
            ...
            # Distill
            config = MultiStageConfig(num_stages=1, stage_epochs=[epochs_per_step], alpha=0.7)
...
def create_multistage_distiller(
    num_stages: int = 3, total_epochs: int = 100
) -> MultiStageDistiller:
