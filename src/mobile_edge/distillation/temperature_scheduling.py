...
class TemperatureScheduler:
    ...
    def step(self, epoch: int, accuracy: float = None) -> float:
        ...
        # Record history
        self.history.append(
            {"epoch": epoch, "temperature": self.current_temp, "accuracy": accuracy}
        )
    ...
    def _cosine_schedule(self, epoch: int) -> float:
        ...
        # Cosine decay
        temp = self.config.final_temp + 0.5 * (
    ...
    def _linear_schedule(self, epoch: int) -> float:
        ...
        temp = self.config.initial_temp + progress * (
...
class MultiStageTemperatureScheduler:
    ...
    def step(self, epoch: int) -> float:
        ...
        self.history.append({"epoch": epoch, "temperature": self.current_temp})
...
class CurriculumTemperatureScheduler:
    ...
    def __init__(self, base_temp: float = 4.0, difficulty_range: Tuple[float, float] = (0.5, 2.0)):
...
def visualize_schedule(scheduler: TemperatureScheduler, total_epochs: int = 100) -> List[float]:
