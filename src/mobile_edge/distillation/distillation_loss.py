...
class DistillationLoss(nn.Module):
    ...
    def forward(
        ...
        loss_dict = {"total": total_loss.item(), "soft": soft_loss.item(), "hard": hard_loss.item()}
    ...
    def _compute_feature_loss(
        self, student_features: Dict[str, torch.Tensor], teacher_features: Dict[str, torch.Tensor]
    ) -> torch.Tensor:
...
class RelationDistillationLoss(nn.Module):
    ...
    def forward(
        ...
        total_loss = (
            self.config.alpha * soft_loss + self.config.beta * hard_loss + 0.1 * relation_loss
        )
...
def create_distillation_loss(
    loss_type: str = "standard", temperature: float = 4.0, alpha: float = 0.7
) -> nn.Module:
    """Factory for distillation losses"""

    config = DistillationConfig(
        temperature=temperature, alpha=alpha, beta=1.0 - alpha, loss_type="kl"
    )
