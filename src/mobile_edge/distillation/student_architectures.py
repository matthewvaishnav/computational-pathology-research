...
class MobileNetV3Small(nn.Module):
    ...
    def __init__(self, config: StudentConfig):
        ...
        for k, exp, out, se, nl, s in settings:
            out_ch = int(out * wm)
            exp_ch = int(exp * wm)
            self.blocks.append(InvertedResidual(in_ch, out_ch, exp_ch, k, s, se, nl))
            in_ch = out_ch
...
class EfficientNetB0Student(nn.Module):
    ...
    def __init__(self, config: StudentConfig):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, int(32 * wm), 3, 2, 1, bias=False), nn.BatchNorm2d(int(32 * wm)), nn.SiLU()
        )
        ...
        for k, exp_r, out, layers, s in settings:
            ...
            for i in range(num_layers):
                ...
                self.blocks.append(MBConvBlock(in_ch, out_ch, k, stride, exp_r, config.use_se))
                in_ch = out_ch
        ...
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.classifier = nn.Sequential(
            nn.Dropout(config.dropout), nn.Linear(int(1280 * wm), config.num_classes)
        )
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, exp_ch, kernel, stride, use_se, activation):
        ...
        if exp_ch != in_ch:
            layers.extend(
                [nn.Conv2d(in_ch, exp_ch, 1, 1, 0, bias=False), nn.BatchNorm2d(exp_ch), act]
            )
        ...
        # Project
        layers.extend([nn.Conv2d(exp_ch, out_ch, 1, 1, 0, bias=False), nn.BatchNorm2d(out_ch)])
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel, stride, exp_ratio, use_se):
        ...
        if exp_ratio != 1:
            layers.extend(
                [nn.Conv2d(in_ch, exp_ch, 1, bias=False), nn.BatchNorm2d(exp_ch), nn.SiLU()]
            )
...
class FireModule(nn.Module):
    ...
    def __init__(self, in_ch, squeeze_ch, expand1x1_ch, expand3x3_ch):
        super().__init__()
        self.squeeze = nn.Sequential(nn.Conv2d(in_ch, squeeze_ch, 1), nn.ReLU(inplace=True))
        self.expand1x1 = nn.Sequential(
            nn.Conv2d(squeeze_ch, expand1x1_ch, 1), nn.ReLU(inplace=True)
        )
        self.expand3x3 = nn.Sequential(
            nn.Conv2d(squeeze_ch, expand3x3_ch, 3, padding=1), nn.ReLU(inplace=True)
        )
...
def compare_student_architectures(num_classes: int = 2) -> Dict[str, Dict]:
    ...
    for arch, width in architectures:
        config = StudentConfig(architecture=arch, width_multiplier=width, num_classes=num_classes)
