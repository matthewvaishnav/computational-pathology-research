...
class TensorRTOptimizer:
    ...
    def _build_engine(self, onnx_path: Path,
                     input_shape: Tuple[int, ...]) -> trt.ICudaEngine:
...
def optimize_with_tensorrt(model: nn.Module,
...
def benchmark_tensorrt(pytorch_model: nn.Module,
                      trt_engine: TensorRTEngine,
                      input_shape: Tuple[int, ...],
                      num_iterations: int = 100) -> TensorRTResult:
...
def optimize_medical_model_tensorrt(model: nn.Module,
                                   input_shape: Tuple[int, ...] = (1, 3, 224, 224),
                                   save_path: Path = None) -> TensorRTEngine:
