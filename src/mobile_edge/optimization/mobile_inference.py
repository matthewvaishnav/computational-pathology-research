...
def convert_pytorch_to_tflite(model: nn.Module,
...
def convert_pytorch_to_mobile(model: nn.Module,
                              input_shape: Tuple[int, ...],
                              save_path: Path) -> Path:
...
def benchmark_all_backends(model: nn.Module,
                           input_shape: Tuple[int, ...],
                           model_dir: Path) -> List[BenchmarkResult]:
...
def create_medical_mobile_engine(model_path: Path,
                                backend: str = 'onnx_mobile',
                                num_threads: int = 4) -> MobileInferenceEngine:
