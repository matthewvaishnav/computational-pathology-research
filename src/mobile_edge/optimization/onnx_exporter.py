    ...
    def _validate_onnx(self, model_path: Path, input_shape: Tuple[int, ...]):
        ...
        try:
            ...
            outputs = session.run(
                ...
                {self.config.input_names[0]: dummy_input}
            )
...
def benchmark_onnx(pytorch_model: nn.Module,
                  onnx_model_path: Path,
                  input_shape: Tuple[int, ...]) -> ONNXResult:
...
def export_medical_model_onnx(model: nn.Module,
                             input_shape: Tuple[int, ...] = (1, 3, 224, 224),
...
def create_onnx_runtime_config(model_path: Path,
                               use_gpu: bool = False,
                               use_tensorrt: bool = False) -> ONNXRuntimeInference:
...
def compare_pytorch_onnx(pytorch_model: nn.Module,
                        onnx_model_path: Path,
                        input_shape: Tuple[int, ...],
...
def export_with_metadata(model: nn.Module,
                        input_shape: Tuple[int, ...],
                        save_path: Path,
                        metadata: Dict) -> Path:
