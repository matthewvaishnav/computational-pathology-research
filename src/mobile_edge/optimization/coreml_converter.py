...
class CoreMLConfig:
    ...
    def __post_init__(self):
        assert self.compute_precision in ['float32', 'float16', 'mixed']
        assert self.quantization_bits in [8, 16]
...
class CoreMLConverter:
    ...
    def convert_model(self, model: nn.Module,
                     ...
                     save_path: Path,
                     class_labels: List[str] = None) -> 'CoreMLModel':
        """
    ...
    def _trace_model(self, model: nn.Module,
                    input_shape: Tuple[int, ...]) -> torch.jit.ScriptModule:
        """Trace PyTorch model"""
    ...
    def _convert_to_coreml(self, traced_model: torch.jit.ScriptModule,
                          input_shape: Tuple[int, ...],
    ...
    def _quantize_weights(self, model):
        ...
        spec = quantization_utils.quantize_weights(
            spec,
            nbits=self.config.quantization_bits,
...
def convert_to_coreml(model: nn.Module,
...
def benchmark_coreml(pytorch_model: nn.Module,
                    coreml_model_path: Path,
                    input_shape: Tuple[int, ...]) -> CoreMLResult:
...
def convert_medical_model_coreml(model: nn.Module,
                                input_shape: Tuple[int, ...] = (1, 3, 224, 224),
...
def create_coreml_package(model_path: Path,
                         output_dir: Path,
                         model_name: str = 'MedicalAI'):
