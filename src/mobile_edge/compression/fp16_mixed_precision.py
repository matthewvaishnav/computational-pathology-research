...
class FP16MixedPrecision:
    ...
    def inference(self, model: nn.Module, data: torch.Tensor,
    ...
    def benchmark_training(self, model: nn.Module,
                          dataloader: torch.utils.data.DataLoader,
                          device: torch.device,
                          criterion: nn.Module,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'overflow_rate': self.overflow_count / self.total_steps if self.total_steps > 0 else 0.0,
            'current_scale': self.scaler.get_scale() if self.scaler is not None else self.config.loss_scale
        }
...
def create_fp16_config(opt_level: str = 'O1',
                      dynamic_loss_scale: bool = True) -> FP16Config:
...
def benchmark_fp16_vs_fp32(model: nn.Module,
                          train_loader: torch.utils.data.DataLoader,
                          val_loader: torch.utils.data.DataLoader,
                          device: torch.device,
                          criterion: nn.Module) -> FP16Result:
