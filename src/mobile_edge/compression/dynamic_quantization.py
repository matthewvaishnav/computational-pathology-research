...
class DynamicQuantizer:
    ...
    def _apply_dynamic_quantization(self, model: nn.Module) -> nn.Module:
        ...
        try:
            ...
            quantized_model = torch.quantization.quantize_dynamic(
                model, qconfig_spec=layer_types, dtype=self.config.dtype, inplace=False
            )
    ...
    def _measure_inference_speedup(
        ...
        try:
            ...
            self.logger.info(
                f"Inference timing - Original: {original_time*10:.2f}ms, "
...
def create_dynamic_quantization_config(
    dtype: torch.dtype = torch.qint8, quantize_all: bool = True
...
) -> Tuple[nn.Module, DynamicQuantizationResult]:
    """
    Apply dynamic quantization to model
...
def quantize_pathology_model_dynamic(
    model: nn.Module, val_loader: torch.utils.data.DataLoader
) -> DynamicQuantizationResult:
...
def compare_quantization_dtypes(
    model: nn.Module, val_loader: torch.utils.data.DataLoader
) -> Dict[str, DynamicQuantizationResult]:
