...
class CalibrationDatasetBuilder:
    ...
    def build_calibration_dataset(
        self, dataset: Dataset, model: nn.Module = None, device: torch.device = None
    ) -> Tuple[Dataset, CalibrationStats]:
        """
    ...
    def _stratified_sampling(self, dataset: Dataset) -> List[int]:
        ...
        try:
            ...
            if self.config.balance_classes:
                ...
                for label, indices in class_indices.items():
                    ...
                    samples_per_class[label] = max(
                        int(self.config.num_samples * proportion), self.config.min_samples_per_class
                    )
            ...
            self.logger.info(
                f"Stratified sampling: {len(selected_indices)} samples, " f"{num_classes} classes"
            )
    ...
    def _diverse_sampling(
        self, dataset: Dataset, model: nn.Module, device: torch.device
    ) -> List[int]:
        """Diverse sampling to maximize feature diversity"""
        try:
            if model is None or device is None:
                self.logger.warning(
                    "Model or device not provided, falling back to stratified sampling"
                )
    ...
    def _extract_features(
        self, dataset: Dataset, model: nn.Module, device: torch.device, max_samples: int = 5000
    ) -> np.ndarray:
        ...
        try:
            ...
            temp_loader = DataLoader(dataset, batch_size=32, shuffle=False, num_workers=0)
    ...
    def _kmeans_plus_plus_sampling(self, features: np.ndarray, num_samples: int) -> List[int]:
    ...
    def _compute_statistics(
        self, calibration_dataset: Dataset, full_dataset: Dataset
    ) -> CalibrationStats:
...
def create_calibration_config(
    num_samples: int = 1000, strategy: str = "stratified", balance_classes: bool = True
) -> CalibrationConfig:
...
def build_calibration_dataset(
    dataset: Dataset, num_samples: int = 1000, strategy: str = "stratified"
) -> Tuple[Dataset, DataLoader, CalibrationStats]:
    """
...
def create_medical_calibration_dataset(
    dataset: Dataset, num_samples: int = 2000
) -> Tuple[Dataset, DataLoader, CalibrationStats]:
