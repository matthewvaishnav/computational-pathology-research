...
class INT8Quantizer:
    ...
    def quantize_model(
        ...
        try:
            ...
            inference_speedup = self._measure_inference_speedup(model, quantized_model, device)
    ...
    def _post_training_quantization(
        self, model: nn.Module, val_loader: torch.utils.data.DataLoader, device: torch.device
    ) -> nn.Module:
    ...
    def _calibrate_model(
        self, model: nn.Module, dataloader: torch.utils.data.DataLoader, device: torch.device
    ):
        ...
        try:
            ...
            self.logger.info(
                f"Calibrated model with {min(batch_idx + 1, self.config.calibration_batches)} batches"
            )
    ...
    def _train_qat_model(
        ...
        try:
            ...
            for epoch in range(self.config.qat_epochs):
                ...
                self.logger.info(
                    f"QAT Epoch {epoch+1}/{self.config.qat_epochs} - "
                    f"Loss: {avg_loss:.4f}, "
    ...
    def _measure_inference_speedup(
        self, original_model: nn.Module, quantized_model: nn.Module, device: torch.device
    ) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"Inference timing - Original: {original_time:.2f}ms, "
...
def create_int8_quantization_config(
    quantization_type: str = "qat", backend: str = "fbgemm"
...
def quantize_pathology_model_int8(
    model: nn.Module,
    train_loader: torch.utils.data.DataLoader,
    val_loader: torch.utils.data.DataLoader,
    use_qat: bool = True,
