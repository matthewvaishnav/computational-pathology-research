...
class PolynomialSchedule(PruningSchedule):
    ...
    def should_prune_at_step(self, step: int, config: GradualPruningConfig) -> bool:
        return (step >= config.warmup_steps and 
                step < config.total_steps and
                step % config.pruning_frequency == 0)
...
class ExponentialSchedule(PruningSchedule):
    ...
    def get_sparsity_at_step(self, step: int, config: GradualPruningConfig) -> float:
        ...
        lambda_val = 5.0
        sparsity = config.target_sparsity - (config.target_sparsity - config.initial_sparsity) * \
...
class CosineSchedule(PruningSchedule):
    ...
    def get_sparsity_at_step(self, step: int, config: GradualPruningConfig) -> float:
        ...
        # Cosine schedule
        sparsity = config.initial_sparsity + (config.target_sparsity - config.initial_sparsity) * \
...
class GradualPruner:
    ...
    def initialize_masks(self, model: nn.Module):
        ...
        try:
            ...
            self.logger.info(f"Initialized masks for {len(self.masks)} layers, "
    ...
    def step(self, model: nn.Module, 
        ...
        try:
            ...
            self.logger.info(f"Pruning step {self.current_step} - "
            ...
            if accuracy_before > 0 and accuracy_after > 0:
                ...
                self.logger.info(f"Accuracy: {accuracy_before:.4f} → {accuracy_after:.4f} "
        ...
        for name, param in model.named_parameters():
            if name in self.masks:
                ...
                if layer_sparsity < self.config.min_sparsity_per_layer:
                    ...
                    num_to_keep_layer = int(num_weights_layer * (1 - self.config.min_sparsity_per_layer))
                    flat_weights = torch.abs(param.data).flatten()
                    threshold_layer = torch.kthvalue(flat_weights, num_weights_layer - num_to_keep_layer + 1)[0]
                    ...
                    num_weights_layer = param.numel()
                    num_to_keep_layer = int(num_weights_layer * (1 - self.config.max_sparsity_per_layer))
                    flat_weights = torch.abs(param.data).flatten()
                    threshold_layer = torch.kthvalue(flat_weights, num_weights_layer - num_to_keep_layer + 1)[0]
                    magnitude_mask = torch.abs(param.data) >= threshold_layer
...
def create_gradual_pruning_config(target_sparsity: float = 0.9,
                                total_steps: int = 10000,
                                schedule_type: str = 'polynomial') -> GradualPruningConfig:
...
def create_medical_gradual_pruner(target_sparsity: float = 0.8,
                                total_epochs: int = 100,
                                steps_per_epoch: int = 1000) -> GradualPruner:
