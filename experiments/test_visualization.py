...
def load_evaluation_results(json_path=None):
    ...
    default_results = {
        ...
        "y_pred": np.concatenate(
            [
                np.zeros(4500) + np.random.normal(0, 0.3, 4500),
...
ax.plot(
    epochs, metrics["train_loss"], "b-", label="Train Loss", linewidth=2, marker="o", markersize=4
)
...
ax.plot(
    epochs, metrics["val_acc"], "r-", label="Val Accuracy", linewidth=2, marker="s", markersize=4
)
ax.set_xlabel("Epoch", fontsize=12)
