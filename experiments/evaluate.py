...
class ModelEvaluator:
    ...
    def plot_embeddings(self):
        ...
        scatter = plt.scatter(
            embeddings_2d[:, 0], embeddings_2d[:, 1], c=all_labels, cmap="viridis", alpha=0.6, s=50
        )
    ...
    def run_ablation_study(self) -> Dict[str, Dict[str, float]]:
        ...
        for modality_to_remove in modalities:
            ...
            results[f"without_{modality_to_remove}"] = {"accuracy": accuracy, "f1": f1}
    ...
    def test_missing_modalities(self) -> Dict[str, Dict[str, float]]:
        ...
        for missing_rate in missing_rates:
            ...
            results[f"missing_{int(missing_rate*100)}pct"] = {"accuracy": accuracy, "f1": f1}
    ...
    def error_analysis(self):
        ...
        logger.info(
            f"Total errors: {error_report['total_errors']} ({error_report['error_rate']:.2%})"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--generate-plots", action="store_true", default=True, help="Generate visualization plots"
    )
    parser.add_argument("--run-ablation", action="store_true", help="Run ablation study")
    parser.add_argument(
        "--test-missing-modalities", action="store_true", help="Test missing modality robustness"
    )
    parser.add_argument(
        "--error-analysis", action="store_true", default=True, help="Perform error analysis"
    )
...
def main():
    ...
    try:
        test_dataset = MultimodalDataset(
            data_dir=args.data_dir, split=args.split, config=vars(args)
        )
