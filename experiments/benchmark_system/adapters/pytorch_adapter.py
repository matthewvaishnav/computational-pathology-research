"""
Baseline PyTorch Adapter for Competitor Benchmark System.

This adapter implements training execution using vanilla PyTorch without any
framework-specific abstractions. It serves as the baseline for comparison,
using the simplest possible implementation with raw PyTorch training loops.

The adapter matches HistoCore's PyTorch version (>=2.0.0) and provides a
minimal configuration baseline for fair comparison.
...
class PyTorchAdapter:
    """
    Adapter for executing training tasks using baseline PyTorch.
    
    This adapter uses vanilla PyTorch without any framework-specific abstractions:
    - Raw PyTorch training loop
    - Standard PyTorch optimizers and loss functions
    - Minimal configuration - just essential training components
    - No special model architectures or training tricks
    ...
    - Matching HistoCore's PyTorch version (Requirement 1.5)
    
    This serves as the baseline for comparison, representing the simplest
    possible PyTorch implementation without framework enhancements.
    """
    
    def __init__(self, env: FrameworkEnvironment):
        """
        Initialize baseline PyTorch adapter.
        ...
        logger.info(f"Initialized baseline PyTorch adapter on device: {self.device}")
    ...
    def execute_training(
        ...
        """
        Execute training task using baseline PyTorch.
        
        This method:
        1. Sets random seeds for reproducibility
        2. Creates model and optimizer based on task specification
        3. Loads or creates dataset
        4. Runs vanilla PyTorch training loop with metrics collection
        ...
        Args:
            ...
            config_dict: Framework-specific configuration dictionary (minimal for PyTorch)
        ...
        Requirements: 1.5, 2.1, 2.4, 4.1-4.8
        """
        logger.info(f"Starting baseline PyTorch training: {task_spec.dataset_name}")
        ...
        logger.info(
            f"Baseline PyTorch training completed successfully - "
    ...
    def _create_model_and_optimizer(
        ...
        # Create optimizer based on task specification
        if task_spec.optimizer == "Adam":
            optimizer = optim.Adam(
                model.parameters(),
                lr=task_spec.learning_rate,
                weight_decay=task_spec.weight_decay,
            )
        elif task_spec.optimizer == "AdamW":
            optimizer = optim.AdamW(
                model.parameters(),
                lr=task_spec.learning_rate,
                weight_decay=task_spec.weight_decay,
            )
        elif task_spec.optimizer == "SGD":
            optimizer = optim.SGD(
                model.parameters(),
                lr=task_spec.learning_rate,
                weight_decay=task_spec.weight_decay,
