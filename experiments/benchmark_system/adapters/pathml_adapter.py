"""
PathML Framework Adapter for Competitor Benchmark System.

This adapter implements training execution using PathML's native APIs,
translating the generic TaskSpecification into PathML-specific training and
extracting standardized metrics for fair comparison.
...
class PathMLAdapter:
    """
    Adapter for executing training tasks using PathML framework.
    
    This adapter uses PathML's training infrastructure including:
    - PathML's model creation APIs
    - PathML's training loop conventions
    - PathML-specific configuration handling
    - Metrics collection compatible with PathML's evaluation framework
    ...
    def __init__(self, env: FrameworkEnvironment):
        ...
        logger.info(f"Initialized PathML adapter on device: {self.device}")
    ...
    def execute_training(
        ...
        """
        Execute training task using PathML APIs.
        ...
        Requirements: 2.1, 2.4, 4.1-4.8
        """
        logger.info(f"Starting PathML training: {task_spec.dataset_name}")
        ...
        # Create model and optimizer using PathML conventions
        ...
        logger.info(
            f"PathML training completed successfully - "
    ...
    def _create_data_loaders(
        ...
        task_spec.dataset_name and task_spec.data_root using PathML's
        ...
        # In production, this would use PathML's dataset loaders
    ...
    def _create_model_and_optimizer(
        ...
        """
        Create model, optimizer, and loss criterion using PathML conventions.
        
        PathML may have specific model creation APIs or conventions that differ
        from raw PyTorch. This method adapts the configuration to PathML's
        expected format.
        ...
        # Create optimizer based on PathML configuration
        optimizer_name = train_config.get("optimizer", "adam").lower()
        learning_rate = train_config.get("lr", task_spec.learning_rate)
        weight_decay = train_config.get("weight_decay", task_spec.weight_decay)
