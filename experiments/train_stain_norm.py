...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, layers: Optional[list] = None, weights: Optional[list] = None):
        super().__init__()
...
class MorphologyPreservationMetrics:
    ...
    def compute_ssim(
        ...
        sigma1_sq = (
            F.conv2d(img1 * img1, kernel, padding=window_size // 2, groups=img1.size(1)) - mu1_sq
        )
        sigma2_sq = (
            F.conv2d(img2 * img2, kernel, padding=window_size // 2, groups=img2.size(1)) - mu2_sq
        )
        sigma12 = (
            F.conv2d(img1 * img2, kernel, padding=window_size // 2, groups=img1.size(1)) - mu1_mu2
        )
    ...
    @staticmethod
    def compute_edge_preservation(original: torch.Tensor, normalized: torch.Tensor) -> torch.Tensor:
        """
        ...
        edge_preservation = F.cosine_similarity(
            grad_mag_orig.flatten(1), grad_mag_norm.flatten(1), dim=1
        ).mean()

        return edge_preservation

    @staticmethod
    def compute_all_metrics(original: torch.Tensor, normalized: torch.Tensor) -> Dict[str, float]:
        """
        ...
        return {"ssim": ssim.item(), "edge_preservation": edge_preservation.item()}
...
class StainNormalizationTrainer:
    ...
    def __init__(
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=len(train_loader) * 100, eta_min=1e-6  # Assume 100 epochs max
        )
    ...
    def train(
        self, num_epochs: int, checkpoint_dir: Optional[Path] = None, save_interval: int = 10
    ):
        """
