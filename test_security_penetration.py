            ...
            if path.exists():
                try:
                    ...
                    if sensitive_file in ["/etc/passwd", "/proc/version"]:
                        results["passed"] += 1
                        results["details"].append(f"✅ System file readable as expected: {sensitive_file}")
                    else:
...
def test_environment_variables():
    ...
    for var_name, var_value in env_vars.items():
        ...
        if is_sensitive:
            ...
            elif var_value and len(var_value) > 5 and var_value not in ["test", "placeholder", "dummy"]:
                results["failed"] += 1
    ...
    if not any(any(sensitive in var.upper() for sensitive in sensitive_env_vars) 
