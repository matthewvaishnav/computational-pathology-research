...
def test_cli_edge_cases():
    ...
    test_cases = [
        ...
        (["train", "--dataset", "pcam", "--dataset", "camelyon"], "should_pass"),  # Duplicate args (use last)
    ]
    ...
    for args, expected in test_cases:
        try:
            ...
            if expected == "should_fail":
                ...
                should_fail = (
                    not cmd_str.strip() or  # Empty command
                    "nonexistent" in cmd_str or  # Unknown dataset/command
                    "../../../" in cmd_str or  # Path traversal
                    "--epochs -1" in cmd_str or  # Negative epochs
                    "--epochs abc" in cmd_str or  # Non-numeric epochs
                    "--batch-size 0" in cmd_str or  # Zero batch size
                    "--batch-size 999999" in cmd_str or  # Unreasonable batch size
                    '--output ""' in cmd_str or cmd_str.endswith('--output ')  # Empty output
                )
                ...
                is_valid = (
                    cmd_str.strip() and  # Not empty
                    "../../../" not in cmd_str and  # No path traversal
                    not any(bad in cmd_str for bad in ["nonexistent", "--epochs -1", "--epochs abc", "--batch-size 0"])
                )
