...
class MasterTestRunner:
    ...
    def run_all_categories(self, categories: list = None, fast_mode: bool = False) -> dict:
        ...
        for category in categories:
            ...
            print(
                f"[{emoji}] {category}: {category_result['passed']}/{category_result['tests']} passed ({category_result['duration']:.1f}s)"
            )
        ...
        results["summary"]["overall_success"] = (
            results["summary"]["failed_categories"] == 0 and results["summary"]["total_tests"] > 0
        )
...
"""
    ...
    def print_final_summary(self, results: dict):
        ...
        print(
            f"Tests: {summary['total_passed']}/{summary['total_tests']} passed ({summary['success_rate']:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument("--output-dir", default="dataset_test_results", help="Output directory")
    parser.add_argument(
        "--fast", action="store_true", help="Fast mode: stop on first failure, minimal output"
    )
    parser.add_argument(
        "--check-deps", action="store_true", help="Only check dependencies, don't run tests"
    )
    parser.add_argument(
        "--ci", action="store_true", help="CI mode: optimized for continuous integration"
    )
