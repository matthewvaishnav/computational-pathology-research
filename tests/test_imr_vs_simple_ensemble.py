...
def test_accuracy_comparison():
    ...
    for case in test_cases:
        ...
        simple_pred = simple_ensemble(
            case["fl_prob"], case["dmi_prob"], case["fl_confidence"], case["dmi_confidence"]
        )
        ...
        imr_result = imr.arbitrate_predictions(
            fl_result, dmi_result, {"case_id": f"test_{len(imr_errors)}"}
        )
...
def test_performance_comparison():
    ...
    # IMR timing
    fl_result = {"cancer_probability": 0.7, "model_confidence": 0.9, "contributing_hospitals": 50}
    dmi_result = {
...
def test_disagreement_handling():
    ...
    for case in disagreement_cases:
        ...
        print(
            f"  Case: GT={ground_truth:.1f}, FL={case['fl_prob']:.1f}, DMI={case['dmi_prob']:.1f}"
        )
...
def run_comparison_tests():
    ...
    print(
        f"Disagreement handling: {'IMR better' if imr_handles_disagreement_better else 'Simple better'}"
    )
