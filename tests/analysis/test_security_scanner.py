            ...
            with patch.object(self.scanner, "_detect_injection_risks", return_value=[]):
                with patch.object(self.scanner, "_validate_tls_ssl_config", return_value=[]):
                    with patch.object(self.scanner, "_detect_hardcoded_secrets", return_value=[]):
                        with patch.object(
                            self.scanner, "_assess_hipaa_compliance", return_value=85.0
                        ):
...
class TestSecurityScoreCalculation:
    ...
    def test_calculate_security_score_perfect(self):
        ...
        score = self.scanner._calculate_security_score(
            vulnerabilities=[], hipaa_score=100.0, secrets=[], injection_risks=[]
        )
    ...
    def test_calculate_security_score_without_tls_issues(self):
        ...
        score = self.scanner._calculate_security_score(
            vulnerabilities=[], hipaa_score=80.0, secrets=[], injection_risks=[]
        )
    ...
    def test_calculate_security_score_mixed(self):
        ...
        vulnerabilities = [{"severity": "MEDIUM", "confidence": "HIGH"}]
...
class TestIntegrationWithMockData:
    ...
    def test_full_analysis_with_mock_data(self):
        ...
        mock_secrets = [{"file": "src/config.py", "type": "api_key", "line": 5, "value": "sk-***"}]
        ...
        with patch.object(self.scanner, "_run_bandit_scan", return_value=mock_vulnerabilities):
            with patch.object(
                self.scanner, "_detect_injection_risks", return_value=mock_injection_risks
            ):
                with patch.object(self.scanner, "_validate_tls_ssl_config", return_value=[]):
                    with patch.object(
                        self.scanner, "_detect_hardcoded_secrets", return_value=mock_secrets
                    ):
                        with patch.object(
                            self.scanner, "_assess_hipaa_compliance", return_value=70.0
                        ):
        ...
        with patch.object(self.scanner, "_run_bandit_scan", return_value=[]):
            with patch.object(self.scanner, "_detect_injection_risks", return_value=[]):
                with patch.object(self.scanner, "_validate_tls_ssl_config", return_value=[]):
                    with patch.object(self.scanner, "_detect_hardcoded_secrets", return_value=[]):
                        with patch.object(
                            self.scanner, "_assess_hipaa_compliance", return_value=95.0
                        ):
    ...
    def test_analysis_with_poor_security(self):
        ...
        many_secrets = [{"type": "api_key"}, {"type": "password"}, {"type": "private_key"}]

        many_injection_risks = [{"type": "sql_injection"}, {"type": "command_injection"}]

        with patch.object(self.scanner, "_run_bandit_scan", return_value=many_vulnerabilities):
            with patch.object(
                self.scanner, "_detect_injection_risks", return_value=many_injection_risks
            ):
                with patch.object(
                    self.scanner, "_validate_tls_ssl_config", return_value=["TLS issue"]
                ):
                    with patch.object(
                        self.scanner, "_detect_hardcoded_secrets", return_value=many_secrets
                    ):
                        with patch.object(
                            self.scanner, "_assess_hipaa_compliance", return_value=40.0
                        ):
