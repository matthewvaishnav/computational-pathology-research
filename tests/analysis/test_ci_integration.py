...
"""
        ...
        # Generate summary
        critical_issues_summary = "No critical issues detected ✅" if not sample_analysis_result.critical_issues else \
            "\n".join([f"{i+1}. **[{issue.priority.value}]** {issue.title} ({issue.dimension})" 
...
*Generated by Project Analysis CI*"""
    ...
    def test_regression_comment_formatting(self, sample_analysis_result):
        ...
        current.security.vulnerabilities = [{"type": "xss", "severity": "high"}]  # New vulnerability
        ...
        if report.has_regressions:
            ...
            if critical_count > 0:
                ...
                regression_summary += "This PR introduces critical regressions that may block the build.\n"
            else:
                regression_summary = "## ℹ️ Regression Analysis\n\n**Non-critical regressions detected.**\n"
        else:
            regression_summary = "## ✅ Regression Analysis\n\nNo regressions detected compared to baseline."
