...
class TestScalingRecommendations:
    ...
    def test_generate_recommendations_no_ddp(self):
        ...
        ddp_strategy = next((s for s in strategies if s['category'] == 'distributed_training'), None)
    ...
    def test_generate_recommendations_memory_bottlenecks(self):
        ...
        strategies = recommendations['optimization_strategies']
        memory_strategy = next((s for s in strategies if s['category'] == 'memory_optimization'), None)
        assert memory_strategy is not None
        assert 'gradient checkpointing' in memory_strategy['implementation'].lower() or \
               'amp' in memory_strategy['implementation'].lower()
    ...
    def test_granular_efficiency_classification(self):
        ...
        classification = self.analyzer._classify_scaling_efficiency_granular(
            ddp_correct=True,
            bottlenecks=["issue" + str(i) for i in range(10)],
            comm_overhead=150.0
        )
            ...
            with patch.object(self.analyzer, '_identify_memory_bottlenecks', return_value=[]):
                with patch.object(self.analyzer, '_detect_data_loading_bottlenecks', return_value=[]):
                    with patch.object(self.analyzer, '_assess_large_dataset_handling', return_value=[]):
                        with patch.object(self.analyzer, '_estimate_communication_overhead', return_value=15.0):
