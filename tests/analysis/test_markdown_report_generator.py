...
def sample_result():
    ...
    return AnalysisResult(
        ...
        architecture=ArchitectureAnalysis(
            total_files=100,
            large_files=[
                {"path": "src/large.py", "lines": 600, "complexity": 15.0}
            ],
        ...
        performance=PerformanceAnalysis(
            ...
            bottlenecks=[
                {"function": "slow_func", "time_ms": 1200, "file": "src/perf.py"}
            ],
...
class TestReportGenerator:
    ...
    def test_key_metric_extraction(self, generator):
        ...
        assert "GPU utilization" in generator._get_key_metric('performance', {'gpu_utilization': 45.0})
        assert "line coverage" in generator._get_key_metric('coverage', {'line_coverage': 72.5})
        assert "avg complexity" in generator._get_key_metric('code_quality', {'average_complexity': 5.2})
        assert "vulnerabilities" in generator._get_key_metric('dependencies', {'vulnerabilities_count': 3})
        assert "CI/CD complete" in generator._get_key_metric('deployment', {'ci_cd_completeness': 85.0})
        assert "security issues" in generator._get_key_metric('security', {'vulnerabilities_count': 2})
    ...
    def test_long_file_paths(self, generator, sample_result):
        ...
        sample_result.architecture.large_files = [
            {"path": "src/very/long/path/to/some/deeply/nested/module/file.py", "lines": 600, "complexity": 15.0}
        ]
