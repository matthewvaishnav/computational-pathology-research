...
class TestCoverageDataParsing:
    ...
    def test_run_coverage_rounds_percentages(self):
        ...
        coverage_data = {
            "totals": {
                "percent_covered": 85.7654321,
                "percent_covered_display": 82.3456789
            }
        }
...
class TestQualityMetricsCalculation:
    ...
    def test_calculate_coverage_score_above_target(self):
        ...
        score = self.analyzer._calculate_coverage_score(
            line_cov=100.0,  # Above target
            branch_cov=80.0,  # Above target
            untested_paths=[]
        )
    ...
    def test_calculate_coverage_score_rounding(self):
        ...
        score = self.analyzer._calculate_coverage_score(
            line_cov=35.333,  # Results in fractional score
            branch_cov=30.666,
            untested_paths=[]
        )
...
class TestIntegrationWithMockData:
    ...
    def test_full_analysis_with_mock_data(self):
        ...
        with patch.object(self.analyzer, '_run_coverage', return_value=(65.5, 58.2)):
            with patch.object(self.analyzer, '_detect_untested_critical_paths', 
                            return_value=["src/module1.py:45", "src/module2.py:123"]):
                with patch.object(self.analyzer, '_detect_missing_property_tests',
                                return_value=["test_data_transform", "test_validation"]):
                    with patch.object(self.analyzer, '_detect_flaky_tests',
                                    return_value=["test_flaky_network"]):
                        with patch.object(self.analyzer, '_detect_slow_tests',
        ...
        expected_score = self.analyzer._calculate_coverage_score(65.5, 58.2, result.untested_critical_paths)
