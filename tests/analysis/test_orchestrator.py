...
def temp_project_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(['git', 'config', 'user.email', 'test@test.com'], cwd=project_path, check=False, capture_output=True)
        subprocess.run(['git', 'config', 'user.name', 'Test'], cwd=project_path, check=False, capture_output=True)
        subprocess.run(['git', 'add', '.'], cwd=project_path, check=False, capture_output=True)
        subprocess.run(['git', 'commit', '-m', 'init'], cwd=project_path, check=False, capture_output=True)
...
def test_orchestrator_git_commit_detection(temp_project_dir):
    ...
    assert commit_hash == 'unknown' or (len(commit_hash) == 40 and all(c in '0123456789abcdef' for c in commit_hash))
...
def test_orchestrator_overall_score_calculation(temp_project_dir):
    ...
    expected = (85*0.20 + 70*0.15 + 75*0.15 + 80*0.15 + 60*0.10 + 90*0.10 + 50*0.10 + 65*0.05)
...
def test_cli_argument_parsing():
    ...
    with patch('sys.argv', ['orchestrator.py', '.', '--output', 'test.json', '--format', 'json', '--no-parallel']):
