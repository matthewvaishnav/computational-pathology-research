...
def test_property_fedavg_weighted_average(num_clients, dataset_sizes):
    ...
    for i in range(num_clients):
        gradients = {"param_0": torch.randn(*param_shape), "param_1": torch.randn(*param_shape)}
        update = ClientUpdate(
...
def test_property_fedavg_equal_weights_simple_average(num_clients):
    ...
    simple_average = sum(update.gradients["param_0"] for update in client_updates) / num_clients
