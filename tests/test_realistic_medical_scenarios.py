...
def test_realistic_scenarios():
    ...
    for scenario in scenarios:
        ...
        simple_pred = (
            scenario["fl_prob"] * fl_weight + scenario["dmi_prob"] * dmi_weight
        ) / total_weight
        ...
        print(
            f"    IMR: {imr_pred:.2f} (error: {imr_error:.3f}) {'✅' if imr_error < simple_error else '❌'}"
        )
...
def test_high_stakes_scenarios():
    ...
    print(
        f"    Cost Reduction: {((total_cost_simple - total_cost_imr) / total_cost_simple * 100):+.1f}%"
    )
