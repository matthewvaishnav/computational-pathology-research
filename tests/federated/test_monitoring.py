...
class TestConvergenceDetector:
    ...
    def test_accuracy_mode(self):
        ...
        detector = ConvergenceDetector(patience=3, min_delta=0.01, metric_name="accuracy", mode="max")
...
class TestAlertGenerator:
    ...
    def test_check_byzantine_attack(self, tmp_path):
        ...
        generator = AlertGenerator(alert_file=str(tmp_path / "alerts.jsonl"), enable_console_alerts=False)
    ...
    def test_check_privacy_budget(self, tmp_path):
        ...
        # Budget OK
        generator.check_privacy_budget(current_epsilon=0.5, budget_limit=1.0, client_id="client1", round_id=1)
        assert len(generator.alerts) == 0
        
        # Budget warning
        generator.check_privacy_budget(current_epsilon=0.95, budget_limit=1.0, client_id="client1", round_id=2)
        assert len(generator.alerts) == 1
        assert generator.alerts[0]["severity"] == "warning"
        
        # Budget exhausted
        generator.check_privacy_budget(current_epsilon=1.1, budget_limit=1.0, client_id="client1", round_id=3)
        assert len(generator.alerts) == 2
        assert generator.alerts[1]["severity"] == "critical"
    ...
    def test_check_client_dropout(self, tmp_path):
        ...
        generator.check_client_dropout(expected_clients=expected, received_clients=received, round_id=1)
...
class TestMonitoringDashboard:
    ...
    def dashboard(self, tmp_path):
        ...
        convergence = ConvergenceDetector(patience=3, min_delta=0.01, metric_name="loss", mode="min")
        alerts = AlertGenerator(alert_file=str(tmp_path / "alerts.jsonl"), enable_console_alerts=False)
