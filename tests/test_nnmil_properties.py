...
def test_property_1_input_output_shape_invariance(
    ...
    **Validates: Requirements 1.2, 1.3**
    ...
    assert logits.shape == (batch_size, num_classes), (
        f"Expected output shape ({batch_size}, {num_classes}), " f"but got {logits.shape}"
    )
...
def test_property_2_configuration_validation_valid_params(hidden_dim, dropout, num_classes):
    """
    ...
    **Validates: Requirements 1.5**
    ...
    try:
        model = nnMIL(
            feature_dim=1024, hidden_dim=hidden_dim, num_classes=num_classes, dropout=dropout
        )
...
def test_property_2_configuration_validation_invalid_params(invalid_param):
    ...
    **Validates: Requirements 1.5**
    ...
    kwargs = {"feature_dim": 1024, "hidden_dim": 256, "num_classes": 2, "dropout": 0.25}
...
def test_property_3_parameter_efficiency(feature_dim, hidden_dim, num_classes, dropout):
    """
    ...
    **Validates: Requirements 1.6**
    ...
    model = nnMIL(
        feature_dim=feature_dim, hidden_dim=hidden_dim, num_classes=num_classes, dropout=dropout
    )
...
def test_property_batch_size_invariance(batch_size_1, batch_size_2, num_patches, feature_dim):
    """
    ...
    for i in range(batch_size_1 - 1):
        assert torch.allclose(
            logits_1[i], logits_1[i + 1], atol=1e-6
    ...
    assert torch.allclose(
        logits_1[0], logits_2[0], atol=1e-6
...
def test_property_attention_weights_normalized(batch_size, num_patches, feature_dim):
    """
    ...
    for i in range(batch_size):
        assert torch.allclose(attention_sums[i], torch.tensor(1.0), atol=1e-5), (
            f"Attention weights for sample {i} sum to {attention_sums[i].item()}, " f"expected 1.0"
        )
...
def test_property_masking_correctness(batch_size, max_patches, feature_dim):
    ...
    num_patches = torch.randint(low=10, high=max_patches + 1, size=(batch_size,))
...
def test_property_39_multi_scale_input_handling(batch_size, num_patches, feature_dim, num_scales):
    """
...
def test_property_40_early_fusion_concatenation(batch_size, num_patches, feature_dim, num_scales):
    """
    ...
    **Validates: Requirements 13.3**
    ...
    with torch.no_grad():
        logits, attention = model(multi_scale_features, num_patches_tensor, return_attention=True)

    # Verify that attention weights are computed (shape should be [B, M])
    assert attention.shape == (batch_size, num_patches), (
        f"Expected attention shape ({batch_size}, {num_patches}), " f"but got {attention.shape}"
    )
