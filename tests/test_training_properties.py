...
def test_property_37_best_model_tracking():
    ...
    **Validates: Requirements 12.2**
    ...
    best_checkpoint_calls = [
        call for call in trainer._save_checkpoint.call_args_list
...
def test_property_38_early_stopping(patience, num_epochs_no_improvement):
    ...
    **Validates: Requirements 12.3**
    ...
    for epoch in range(1, num_epochs_no_improvement + 1):
        ...
        if expected_stop:
            assert should_stop, f"Should stop after {epoch} epochs without improvement (patience={patience})"
            break
        else:
            assert not should_stop, f"Should not stop after {epoch} epochs without improvement (patience={patience})"
