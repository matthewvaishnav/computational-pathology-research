...
from multi_disease_model import MultiDiseaseFoundationModel, ModelConfig, create_foundation_model
from self_supervised_pretrainer import SelfSupervisedPreTrainer, PreTrainingConfig, HistopathologyAugmentation
from zero_shot_detection import ZeroShotDetector, DiseaseKnowledgeBase, VisionLanguageEncoder, DiseaseDescription
from data_collection import WSIDataCollector, SlideDatabase, WSIQualityAssessment, UnlabeledWSIDataset
...
class TestZeroShotDetection:
    ...
    def test_vision_language_encoder(self):
        ...
        assert torch.allclose(torch.norm(image_features, dim=1), torch.ones(2), atol=1e-5)  # Normalized
        ...
        assert torch.allclose(torch.norm(text_features, dim=1), torch.ones(2), atol=1e-5)  # Normalized
