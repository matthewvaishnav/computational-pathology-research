...
class TestBoundedQueue:
    ...
    def test_concurrent_producer_consumer(self):
        ...
        producers = [threading.Thread(target=producer, args=(i * 20, 20)) for i in range(3)]
        consumers = [threading.Thread(target=consumer, args=(20,)) for _ in range(3)]
...
class TestGracefulThread:
    ...
    def test_cleanup_callback_executed(self):
        ...
        thread = GracefulThread(
            target=worker, name="test_cleanup", cleanup_callback=cleanup_callback
        )
        thread.start()
    ...
    def test_cleanup_callback_runs_even_with_exception(self):
        ...
        thread = GracefulThread(
            target=worker, name="test_cleanup_exception", cleanup_callback=cleanup_callback
        )
        thread.start()
        thread.join(timeout=1.0)
...
class TestTimeoutLock:
    ...
    def test_lock_hold_time_warning(self):
        ...
        with patch("src.utils.safe_threading.logger") as mock_logger:
            ...
            warning_calls = [call[0][0] for call in mock_logger.warning.call_args_list]
            assert any(
                "test_hold_warning" in str(call) and "held for" in str(call).lower()
    ...
    def test_lock_acquisition_release_logging(self):
        ...
        with patch("src.utils.safe_threading.logger") as mock_logger:
            ...
            assert any(
                "acquired" in str(call).lower() and "test_logging" in str(call)
                for call in debug_calls
            )
            assert any(
                "released" in str(call).lower() and "test_logging" in str(call)
                for call in debug_calls
            )
...
class TestGPUMemoryCleanup:
    ...
    def test_tensors_deleted_on_exception(self):
        ...
        ), f"Memory not cleaned up after exception: {memory_diff} bytes"
...
class TestBoundedQueueProperties:
    ...
    @given(
        operations=st.lists(
            st.tuples(st.sampled_from(["put", "get"]), st.integers(min_value=0, max_value=1000)),
            min_size=100,
            max_size=1000,
        )
    )
...
class TestGracefulThreadProperties:
    ...
    def test_threads_stop_within_timeout(self, num_threads, work_duration):
        ...
        **Validates: Requirements 2.4, 2.5**
        ...
        for i in range(num_threads):
            thread = GracefulThread(
                target=worker, name=f"property_test_{i}", cleanup_callback=cleanup_callback
            )
            threads.append(thread)
            thread.start()
        ...
        assert (
            len(cleanup_counts) == num_threads
        ), f"Expected {num_threads} cleanups, got {len(cleanup_counts)}"

    @given(stop_delay=st.floats(min_value=0.1, max_value=1.0))
    @settings(max_examples=10, deadline=None)
    def test_wait_or_stop_exits_immediately_on_stop(self, stop_delay):
        ...
        **Validates: Requirements 2.2, 2.3**
        ...
        assert (
            exit_times[0] >= stop_delay
        ), f"Thread exited too early: {exit_times[0]}s < {stop_delay}s"
...
class TestLockTimeoutProperties:
    ...
    def test_lock_acquisition_fails_with_timeout_error_after_timeout(self, hold_time, num_threads):
        ...
        **Validates: Requirements 3.2**
        ...
        if hold_time > timeout:
            ...
            assert (
                len(timeouts) > 0
            ), "Some threads should timeout when lock held longer than timeout"

        # Verify no threads are still running
        for thread in threads:
            assert not thread.is_alive(), f"Thread {thread.name} still alive"

    @given(num_contentious_threads=st.integers(min_value=20, max_value=100))
    @settings(max_examples=10, deadline=None)
    def test_no_deadlocks_with_timeout_locks(self, num_contentious_threads):
        ...
        **Validates: Requirements 3.1, 3.2**
        ...
        # Create many threads
        threads = [
            threading.Thread(target=worker, args=(i,)) for i in range(num_contentious_threads)
        ]
        ...
        assert (
            len(completed) == expected_results
    ...
    def test_threadsafe_dict_final_state_consistent(self, operations):
        ...
        **Validates: Requirements 4.1-4.5**
        ...
        for key, value in d.items():
            assert (
                value == key * 10
            ), f"Inconsistent value for key {key}: expected {key * 10}, got {value}"
...
class TestStopEventProperties:
    ...
    @given(
        ...
        wait_intervals=st.lists(st.floats(min_value=0.1, max_value=2.0), min_size=5, max_size=20),
    )
    @settings(max_examples=10, deadline=None)
    def test_multiple_loops_all_exit_quickly(self, num_loops, wait_intervals):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        for i in range(num_loops):
            ...
            thread = GracefulThread(target=monitoring_loop, name=f"loop_{i}")
            threads.append(thread)
            thread.start()
...
class TestConcurrencyStress:
    ...
    @given(
        operations=st.lists(
            st.tuples(st.sampled_from(["put", "get"]), st.integers(min_value=0, max_value=10000)),
            min_size=500,
            max_size=2000,
        )
    )
    ...
    def test_graceful_thread_shutdown_stress(self):
        ...
        for i in range(num_threads):
            ...
            thread = GracefulThread(
                target=worker_func, name=f"stress-graceful-{i}", cleanup_callback=cleanup_func
            )
            threads.append(thread)
        ...
        # Verify all threads stopped
        assert all(stop_results), f"{stop_results.count(False)} threads did not stop within timeout"
    ...
    def test_lock_timeout_behavior_stress(self):
        ...
        assert total_attempts == 100, f"Only {total_attempts}/100 threads completed"

        # Verify no unexpected errors
        assert len(other_errors) == 0, f"Unexpected errors occurred: {other_errors[:5]}"

        # Verify some threads acquired lock successfully
        assert len(successful_acquisitions) > 0, "No threads successfully acquired lock"

        # Verify some threads timed out (due to contention)
        assert len(timeout_errors) > 0, "No threads timed out - lock contention not tested"

        # Verify TimeoutError messages contain lock name
        for worker_id, error_msg in timeout_errors[:5]:
            assert "stress_test_lock" in error_msg, f"TimeoutError missing lock name: {error_msg}"
    ...
    def test_stop_event_responsiveness_stress(self):
        ...
        for i in range(num_loops):
            ...
            thread = GracefulThread(
                target=create_monitoring_loop(interval), name=f"monitor-stress-{i}"
            )
            threads.append(thread)
        ...
        for exit_time, iterations in exit_times:
            ...
            assert exit_time < 3.0, f"Thread took {exit_time}s to exit, expected < 3s"

        # Verify total stop time is reasonable
        # Should not be 2s * 100 threads (sequential)
        assert (
            total_stop_time < 30.0
...
class TestMatplotlibCleanup:
    ...
    def test_figure_closed_on_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            visualizer = ProgressiveVisualizer(
                output_dir=tmpdir, slide_dimensions=(10000, 10000), tile_size=1024
            )
    ...
    def test_cleanup_with_null_figure(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "matplotlib.pyplot.subplots", side_effect=RuntimeError("Mock creation error")
            ):
                try:
                    visualizer._save_attention_heatmap(10)
                except RuntimeError:
...
def create_concurrent_workers(
    num_workers: int, target_func, *args, **kwargs
    ...
    for i in range(num_workers):
        thread = threading.Thread(target=target_func, args=args, kwargs=kwargs, name=f"worker-{i}")
        threads.append(thread)
    return threads
...
class TestConfigurationValidation:
    ...
    def test_valid_configuration_accepted(self, temp_db):
        ...
        valid_config = {
            ...
            "thresholds": {"cpu_percent": 75, "memory_percent": 80, "disk_percent": 85},
            "alerting": {"enabled": True, "channels": ["log"], "rate_limit_minutes": 10},
        }
    ...
    def test_invalid_configuration_rejected(self, temp_db):
        ...
        invalid_config = {
            ...
            "alerting": {"enabled": True, "channels": ["log"]},
        }
        ...
        try:
            ...
            assert "monitoring_interval_seconds" in str(exc_info.value).lower() or "5000" in str(
                exc_info.value
            )
        finally:
            config_path.unlink()
    ...
    def test_slack_webhook_validation(self, temp_db):
        ...
        invalid_config = {
            ...
            "alerting": {
                "enabled": True,
                "channels": ["slack"],
                "slack": {"webhook_url": "https://example.com/webhook"},  # Invalid prefix
            },
        }
    ...
    def test_slack_webhook_pattern_validation_in_schema(self, temp_db):
        ...
        invalid_config = {
            ...
            "alerting": {
                "enabled": True,
                "channels": ["slack"],
                "slack": {"webhook_url": "http://hooks.slack.com/invalid"},  # http instead of https
            },
        }
    ...
    def test_email_to_emails_must_be_list(self, temp_db):
        ...
        try:
            ...
            assert (
                "validation" in str(exc_info.value).lower() or "list" in str(exc_info.value).lower()
            )
        finally:
            config_path.unlink()
