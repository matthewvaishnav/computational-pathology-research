...
class TestDataFixtures:
    ...
    def generate_synthetic_pathology_image(
        ...
        stain_colors = {
            'H&E': {'nuclei': [100, 50, 150], 'cytoplasm': [200, 150, 200], 'background': [240, 220, 240]},
            'IHC': {'nuclei': [139, 69, 19], 'cytoplasm': [100, 150, 200], 'background': [230, 230, 230]},
            'Trichrome': {'nuclei': [0, 100, 200], 'cytoplasm': [200, 0, 0], 'background': [220, 220, 220]},
            'PAS': {'nuclei': [200, 0, 200], 'cytoplasm': [150, 150, 150], 'background': [240, 240, 240]}
        }
        ...
        for _ in range(num_cells):
            ...
            for y in range(max(0, center_y - cytoplasm_radius), min(height, center_y + cytoplasm_radius)):
                for x in range(max(0, center_x - cytoplasm_radius), min(width, center_x + cytoplasm_radius)):
                    dist = np.sqrt((x - center_x)**2 + (y - center_y)**2)
                    
                    if cell_radius < dist < cytoplasm_radius:
                        # Cytoplasm color (lighter)
                        cytoplasm_color = [min(255, c + 30) for c in colors.get('cytoplasm', colors['background'])]
    ...
    def create_test_dicom_file(
        ...
        **kwargs
        ...
        ds.PatientBirthDate = (datetime.now() - timedelta(days=random.randint(18*365, 80*365))).strftime('%Y%m%d')
    ...
    def create_test_case_data(self, num_cases: int = 10) -> List[Dict]:
        ...
        for i in range(num_cases):
            ...
            case_data = {
                ...
                'diagnosis': random.choice(['benign', 'malignant', 'atypical', 'insufficient']) if random.random() > 0.3 else None,
                'confidence_score': random.uniform(0.7, 0.99) if random.random() > 0.3 else None,
                'ai_prediction': random.choice(['positive', 'negative', 'uncertain']) if random.random() > 0.2 else None,
