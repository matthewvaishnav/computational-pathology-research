...
class PerformanceRegressionTests:
    ...
    def test_concurrent_load(self) -> Dict[str, float]:
        ...
        results = {
            ...
            'cpu_usage_increase': resources_after.get('cpu_percent', 0) - resources_before.get('cpu_percent', 0),
            'memory_usage_increase_mb': resources_after.get('memory_mb', 0) - resources_before.get('memory_mb', 0)
        }
        
        success_rate = len(successful_requests) / num_concurrent * 100
        throughput_ok = results['throughput_rps'] >= self.thresholds['throughput_requests_per_second']
        
        print(f"📊 Concurrent Load Results:")
        print(f"   Success Rate: {success_rate:.1f}% ({len(successful_requests)}/{num_concurrent})")
        print(f"   Throughput: {results['throughput_rps']:.2f} req/s {'✅' if throughput_ok else '❌'}")
    ...
    def run_all_performance_tests(self) -> Dict[str, Dict]:
        ...
        print(f"   Memory: {system_info.get('memory_mb', 0):.1f}MB ({system_info.get('memory_percent', 0):.1f}%)")
        ...
        for endpoint, metrics in api_results.items():
            if isinstance(metrics, dict) and metrics.get('avg_time', 0) > self.thresholds['api_response_time']:
                regressions.append(f"API {endpoint}: {metrics['avg_time']:.3f}s > {self.thresholds['api_response_time']}s")
        ...
        if inference_results.get('avg_inference_time', 0) > self.thresholds['inference_time']:
            regressions.append(f"Inference: {inference_results['avg_inference_time']:.2f}s > {self.thresholds['inference_time']}s")
        
        # Throughput regressions
        load_results = all_results.get("Concurrent Load", {})
        if load_results.get('throughput_rps', 0) < self.thresholds['throughput_requests_per_second']:
            regressions.append(f"Throughput: {load_results['throughput_rps']:.2f} < {self.thresholds['throughput_requests_per_second']} req/s")
