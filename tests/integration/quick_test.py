...
def quick_inference_test(base_url: str = "http://localhost:8000") -> bool:
    ...
    try:
        ...
        response = suite.session.post(f"{base_url}/api/v1/analyze/upload", files=files, timeout=60)
        ...
        while time.time() - poll_start < max_wait:
            response = suite.session.get(f"{base_url}/api/v1/analyze/{analysis_id}", timeout=10)
...
def main():
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:8000", help="Base URL for API server"
    )
    parser.add_argument("--inference", action="store_true", help="Include inference test (slower)")
