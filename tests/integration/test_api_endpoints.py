...
class APIEndpointTests:
    ...
    def test_mobile_app_endpoints(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/mobile/register-device",
                json=device_data,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Mobile sync successful: {len(sync_data.get('pending_cases', []))} pending cases")
            else:
            ...
            response = self.session.get(
                f"{self.base_url}/api/v1/mobile/cases/offline",
                timeout=10
            )
            ...
            response = self.session.get(
                f"{self.base_url}/api/v1/mobile/model/download",
                timeout=30
            )
    ...
    def test_reporting_endpoints(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Performance metrics retrieved: {performance_data.get('total_cases', 0)} cases")
            else:
    ...
    def test_websocket_endpoints(self) -> bool:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/api/v1/ws/info",
                timeout=10
            )
