...
class TestModelExceptions:
    ...
    def test_model_error_on_drift_check_failure(self):
        ...
        with patch.object(
            tracker, "calculate_performance_trends", side_effect=Exception("Calculation failed")
        ):
...
class TestExceptionInheritance:
    ...
    def test_all_exceptions_inherit_from_histocore_error(self):
        ...
        for exc_class in exceptions:
            assert issubclass(
                exc_class, HistoCoreError
            ), f"{exc_class.__name__} does not inherit from HistoCoreError"
