...
def test_ablation_runner_output_dir_alignment():
    """
    ...
    with patch("subprocess.run") as mock_run:
        ...
        expected_output_path = str(Path(output_dir) / "test_experiment")
        ...
        for arg in cmd:
            if arg.startswith("hydra.run.dir="):
                ...
                assert Path(override_path).resolve() == Path(expected_output_path).resolve()
                break
...
def test_ablation_runner_failed_experiment_detection():
    """
    ...
    with patch("subprocess.run") as mock_run:
        ...
        with patch("pathlib.Path.exists") as mock_exists:
            ...
            with patch("builtins.open", create=True) as mock_open:
                ...
                with patch("scripts.run_ablation_study.ABLATION_CONFIGS", [
                    {
                        "name": "exp1",
                        "description": "Experiment 1",
                        "model": "multimodal",
                        "data": {"wsi_enabled": True, "genomic_enabled": True, "clinical_text_enabled": True},
                    },
                    {
                        "name": "exp2",
                        ...
                        "model": "baseline",
                        "model_modality": "wsi",
                        "data": {"wsi_enabled": True, "genomic_enabled": False, "clinical_text_enabled": False},
                    },
                ]):
