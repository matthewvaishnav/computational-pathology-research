...
class TestEndpointResponseTimes:
    ...
    def test_auth_endpoint_response_times(self, mock_router):
        ...
        assert (
            avg_response_time <= baseline_auth_time * 1.05
        ), f"Average response time {avg_response_time}ms exceeds baseline"
        assert (
            max_response_time <= baseline_auth_time * 2
        ), f"Max response time {max_response_time}ms too high"
...
class TestStartupTime:
    ...
    def test_warm_start_performance(self):
        ...
        cached_components = {"database": Mock(), "auth_service": Mock(), "file_handler": Mock()}
