...
class SimpleRuleBasedDecision:
    ...
    def get_decision_stats(self):
        ...
        return {
            ...
            "avg_disagreement_imr": sum(
                d["disagreement"] for d in self.decision_log if d["method"] == "imr"
            )
            / max(1, imr_count),
            "avg_disagreement_simple": sum(
                d["disagreement"] for d in self.decision_log if d["method"] == "simple"
            )
            / max(1, total - imr_count),
        }
...
def test_rule_based_system():
    ...
    for case in test_cases:
        ...
        print(
            f"    Actual: {result['method']} ({result['reason']}) {'✅' if method_correct and reason_correct else '❌'}"
        )
...
def test_rule_transparency():
    ...
    has_clear_reason = result["reason"] in [
        "rare_cancer",
        "high_disagreement",
        "expert_confidence_advantage",
        "pediatric_case",
        "artifact_suspected",
        "routine_case",
    ]
