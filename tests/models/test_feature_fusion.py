...
class TestProjectionLayers:
    ...
    def test_proj_a_shape_transformation(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert not torch.isnan(
                projected
            ).any(), f"proj_a produced NaN values for batch_size={batch_size}"
    ...
    def test_proj_b_shape_transformation(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert not torch.isnan(
                projected
            ).any(), f"proj_b produced NaN values for batch_size={batch_size}"
    ...
    def test_relu_activation_applied(self):
        ...
        assert (projected_a >= 0).all(), "proj_a produced negative values, ReLU not applied"
        assert (projected_b >= 0).all(), "proj_b produced negative values, ReLU not applied"
    ...
    def test_dropout_applied_during_training(self):
        ...
        assert not torch.allclose(
            projected_a1, projected_a2
        ), "proj_a outputs are identical, dropout not applied"
        assert not torch.allclose(
            projected_b1, projected_b2
    ...
    def test_dropout_not_applied_during_eval(self):
        ...
        assert torch.allclose(projected_a1, projected_a2), "proj_a outputs differ in eval mode"
        assert torch.allclose(projected_b1, projected_b2), "proj_b outputs differ in eval mode"
    ...
    def test_projection_layers_are_sequential(self):
        ...
        assert isinstance(model.proj_a, torch.nn.Sequential), "proj_a is not nn.Sequential"
        assert isinstance(model.proj_b, torch.nn.Sequential), "proj_b is not nn.Sequential"

        # Check that proj_a has 3 components: Linear, ReLU, Dropout
        assert len(model.proj_a) == 3, f"proj_a should have 3 components, got {len(model.proj_a)}"
        assert isinstance(model.proj_a[0], torch.nn.Linear), "proj_a[0] should be Linear"
        assert isinstance(model.proj_a[1], torch.nn.ReLU), "proj_a[1] should be ReLU"
        assert isinstance(model.proj_a[2], torch.nn.Dropout), "proj_a[2] should be Dropout"

        # Check that proj_b has 3 components: Linear, ReLU, Dropout
        assert len(model.proj_b) == 3, f"proj_b should have 3 components, got {len(model.proj_b)}"
        assert isinstance(model.proj_b[0], torch.nn.Linear), "proj_b[0] should be Linear"
        assert isinstance(model.proj_b[1], torch.nn.ReLU), "proj_b[1] should be ReLU"
        assert isinstance(model.proj_b[2], torch.nn.Dropout), "proj_b[2] should be Dropout"
    ...
    def test_projection_dropout_rate(self):
        ...
        dropout_rate = 0.2
        model = TransnnMIL(feature_dim=1024, hidden_dim=256, num_classes=2, dropout=dropout_rate)

        # Check dropout rate for proj_a
        assert (
            model.proj_a[2].p == dropout_rate
        ), f"proj_a dropout rate should be {dropout_rate}, got {model.proj_a[2].p}"

        # Check dropout rate for proj_b
        assert (
            model.proj_b[2].p == dropout_rate
    ...
    **Validates: Requirements 10.2**
    ...
    def test_fusion_attention_accepts_correct_input_shapes(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert fused is not None, f"fusion_attention returned None for batch_size={batch_size}"
            assert (
    ...
    def test_attention_weights_are_computed(self):
        ...
        assert attn_weights is not None, "fusion_attention did not return attention weights"
    ...
    def test_batch_first_format_is_used(self):
        ...
        assert (
            model.fusion_attention.batch_first is True
    ...
    def test_fusion_attention_no_dropout_during_eval(self):
        ...
        # Outputs should be identical in eval mode
        assert torch.allclose(fused1, fused2), "fusion_attention outputs differ in eval mode"
    ...
    def test_fusion_attention_numerical_stability(self):
        ...
        assert not torch.isnan(fused).any(), "fusion_attention produced NaN values"
        assert not torch.isinf(fused).any(), "fusion_attention produced Inf values"
        assert not torch.isnan(attn_weights).any(), "Attention weights contain NaN values"
        assert not torch.isinf(attn_weights).any(), "Attention weights contain Inf values"
    ...
    def test_fusion_attention_is_multihead_attention(self):
        ...
        assert isinstance(
            model.fusion_attention, torch.nn.MultiheadAttention
    ...
    **Validates: Requirements 10.3**
    ...
    def test_fusion_classifier_shape_transformation(self):
        ...
        for num_classes in num_classes_list:
            model = TransnnMIL(feature_dim=1024, hidden_dim=256, num_classes=num_classes)
            model.eval()
    ...
    def test_relu_activation_between_layers(self):
        ...
        assert (
            intermediate >= 0
        ).all(), "fusion_classifier intermediate outputs contain negative values, ReLU not applied"
    ...
    def test_dropout_not_applied_during_eval(self):
        ...
        # Outputs should be identical in eval mode
        assert torch.allclose(logits1, logits2), "fusion_classifier outputs differ in eval mode"
    ...
    def test_fusion_classifier_gradient_flow(self):
        ...
        # Check gradients exist
        assert fused_features.grad is not None, "No gradient for fused_features"

        # Check gradients are non-zero
        assert fused_features.grad.abs().sum() > 0, "Zero gradient for fused_features"
    ...
    def test_fusion_classifier_numerical_stability(self):
        ...
        # Check for NaN or Inf
        assert not torch.isnan(logits).any(), "fusion_classifier produced NaN values"
        assert not torch.isinf(logits).any(), "fusion_classifier produced Inf values"
    ...
    def test_fusion_classifier_with_different_batch_sizes(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert not torch.isnan(logits).any(), f"NaN values for batch_size={batch_size}"
...
class TestFullForwardPassIntegration:
    ...
    **Validates: Requirements 10.4, 6.2, 6.3, 6.4, 7.3**
    ...
    def test_end_to_end_forward_pass_with_feature_fusion(self):
        ...
        assert (logits < 0).any() or (
            logits > 1
        ).any(), "Logits appear to be bounded to [0, 1], should be unbounded"
    ...
    def test_forward_pass_with_variable_length_bags(self):
        ...
        assert not torch.isnan(
            logits
        ).any(), "Forward pass with variable-length bags produced NaN values"
    ...
    def test_forward_pass_with_fixed_length_bags_no_num_patches(self):
        ...
        assert not torch.isnan(
            logits
        ).any(), "Forward pass with fixed-length bags produced NaN values"
    ...
    def test_forward_pass_with_different_batch_sizes(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert not torch.isnan(
                logits
            ).any(), f"Forward pass produced NaN values for batch_size={batch_size}"
    ...
    def test_forward_pass_gradient_flow(self):
        ...
        # Check gradients are non-zero
        assert features.grad.abs().sum() > 0, "Zero gradient for input features"

        # Check gradients for fusion components
        assert model.proj_a[0].weight.grad is not None, "No gradient for proj_a"
        assert model.proj_b[0].weight.grad is not None, "No gradient for proj_b"
        assert (
            model.fusion_classifier[0].weight.grad is not None
    ...
    def test_forward_pass_numerical_stability(self):
        ...
        assert not torch.isnan(
            logits_large
        ).any(), "Forward pass produced NaN values with large inputs"
        assert not torch.isinf(
            logits_large
        ).any(), "Forward pass produced Inf values with large inputs"
    ...
    def test_forward_pass_with_single_sample(self):
        ...
        # Check output shape
        assert logits.shape == (1, 2), f"Output shape mismatch: expected (1, 2), got {logits.shape}"

        # Test with return_attention=True
        logits, attention = model(features, num_patches=None, return_attention=True)

        assert logits.shape == (1, 2), f"Logits shape mismatch: expected (1, 2), got {logits.shape}"
    ...
    def test_forward_pass_with_different_feature_dimensions(self):
        ...
        for feature_dim in feature_dims:
            model = TransnnMIL(feature_dim=feature_dim, hidden_dim=256, num_classes=2)
            model.eval()
    ...
    def test_forward_pass_integration_with_all_components(self):
        ...
        assert torch.allclose(
            logits, expected_logits, atol=1e-5
...
class TestGetBranchOutputs:
    ...
    **Validates: Requirements 10.5, 5.1, 5.2, 5.3, 5.4**
    ...
    def test_returns_three_outputs(self):
        ...
        assert len(result) == 3, f"get_branch_outputs should return 3 outputs, got {len(result)}"
    ...
    def test_outputs_are_different(self):
        ...
        assert not torch.allclose(
            logits_a, logits_b, rtol=1e-3, atol=1e-3
        ...
        assert not torch.allclose(
            logits_fused, logits_a, rtol=1e-3, atol=1e-3
        ), "logits_fused and logits_a are too similar, fusion may not have effect"

        assert not torch.allclose(
            logits_fused, logits_b, rtol=1e-3, atol=1e-3
    ...
    def test_no_grad_context_is_used(self):
        ...
        assert not logits_a.requires_grad, "logits_a requires gradients, no_grad context not used"
        assert not logits_b.requires_grad, "logits_b requires gradients, no_grad context not used"
        assert (
            not logits_fused.requires_grad
        ...
        try:
            ...
            assert False, "Gradients were computed from get_branch_outputs, no_grad not working"
        except RuntimeError as e:
            # Expected: RuntimeError because tensors don't require gradients
            assert (
                "does not require grad" in str(e).lower()
                or "element 0 of tensors does not require grad" in str(e).lower()
    ...
    def test_works_with_variable_length_bags(self):
        ...
        # Check no NaN values
        assert not torch.isnan(logits_a).any(), "logits_a contains NaN with variable-length bags"
        assert not torch.isnan(logits_b).any(), "logits_b contains NaN with variable-length bags"
        assert not torch.isnan(
            logits_fused
        ).any(), "logits_fused contains NaN with variable-length bags"
    ...
    def test_numerical_stability(self):
        ...
        # Check for NaN
        assert not torch.isnan(logits_a).any(), "logits_a contains NaN values"
        assert not torch.isnan(logits_b).any(), "logits_b contains NaN values"
        assert not torch.isnan(logits_fused).any(), "logits_fused contains NaN values"

        # Check for Inf
        assert not torch.isinf(logits_a).any(), "logits_a contains Inf values"
        assert not torch.isinf(logits_b).any(), "logits_b contains Inf values"
        assert not torch.isinf(logits_fused).any(), "logits_fused contains Inf values"
    ...
    def test_consistency_with_forward_pass(self):
        ...
        assert torch.allclose(
            logits_fused_from_method, logits_from_forward, rtol=1e-5, atol=1e-5
    ...
    def test_branch_outputs_use_original_classifiers(self):
        ...
        assert torch.allclose(
            logits_a, logits_a_direct, rtol=1e-5, atol=1e-5
        ), "logits_a from get_branch_outputs does not match direct branch_a output"

        assert torch.allclose(
            logits_b, logits_b_direct, rtol=1e-5, atol=1e-5
    ...
    def test_efficiency_single_forward_pass(self):
        ...
        assert logits_a is not None and logits_a.numel() > 0, "logits_a is empty or None"
        assert logits_b is not None and logits_b.numel() > 0, "logits_b is empty or None"
        assert (
            logits_fused is not None and logits_fused.numel() > 0
        ), "logits_fused is empty or None"

        # Verify outputs have expected properties
        assert logits_a.dtype == torch.float32, f"logits_a has wrong dtype: {logits_a.dtype}"
        assert logits_b.dtype == torch.float32, f"logits_b has wrong dtype: {logits_b.dtype}"
        assert (
            logits_fused.dtype == torch.float32
    ...
    def test_device_compatibility(self):
        ...
        assert logits_a.device.type == "cpu", f"logits_a should be on CPU, got {logits_a.device}"
        assert logits_b.device.type == "cpu", f"logits_b should be on CPU, got {logits_b.device}"
        assert (
            logits_fused.device.type == "cpu"
        ...
        if torch.cuda.is_available():
            ...
            logits_a_cuda, logits_b_cuda, logits_fused_cuda = model_cuda.get_branch_outputs(
                features_cuda
            )

            # Check that outputs are on CUDA
            assert (
                logits_a_cuda.device.type == "cuda"
            ), f"logits_a should be on CUDA, got {logits_a_cuda.device}"
            assert (
                logits_b_cuda.device.type == "cuda"
            ), f"logits_b should be on CUDA, got {logits_b_cuda.device}"
            assert (
                logits_fused_cuda.device.type == "cuda"
    ...
    def test_large_batch_size(self):
        ...
        # Check no NaN values
        assert not torch.isnan(logits_a).any(), "logits_a contains NaN with large batch size"
        assert not torch.isnan(logits_b).any(), "logits_b contains NaN with large batch size"
        assert not torch.isnan(
            logits_fused
        ).any(), "logits_fused contains NaN with large batch size"
...
class TestBackwardCompatibility:
    ...
    **Validates: Requirements 10.6, 6.1, 6.2, 6.5, 7.1**
    ...
    def test_constructor_accepts_same_parameters(self):
        ...
        model = TransnnMIL(
            feature_dim=1024, hidden_dim=256, num_classes=2, dropout=0.1, num_layers=2, num_heads=8
        )
        ...
        model_minimal = TransnnMIL(feature_dim=1024, hidden_dim=256, num_classes=3)
        ...
        for feature_dim in [512, 768, 1024, 2048]:
            model_var = TransnnMIL(feature_dim=feature_dim, hidden_dim=256, num_classes=2)
            assert model_var is not None, f"Model creation failed for feature_dim={feature_dim}"
    ...
    def test_forward_signature_unchanged(self):
        ...
        logits = model(features, num_patches_tensor, return_attention=False)
        assert logits.shape == (batch_size, 2), "forward() with all parameters failed"

        # Test with positional arguments
        logits_pos = model(features, num_patches_tensor, False)
        assert logits_pos.shape == (batch_size, 2), "forward() with positional arguments failed"

        # Test with return_attention=True
        result = model(features, num_patches_tensor, return_attention=True)
        assert isinstance(result, tuple), "forward() with return_attention=True should return tuple"
        assert len(result) == 2, "forward() should return (logits, attention)"

        # Test without num_patches
        logits_no_np = model(features, return_attention=False)
        assert logits_no_np.shape == (batch_size, 2), "forward() without num_patches failed"

        # Test with only features (default parameters)
        logits_default = model(features)
        assert logits_default.shape == (batch_size, 2), "forward() with only features failed"
    ...
    def test_get_gate_value_method_exists_and_works(self):
        ...
        # Check method exists
        assert hasattr(model, "get_gate_value"), "get_gate_value() method does not exist"

        # Check method is callable
        assert callable(model.get_gate_value), "get_gate_value is not callable"
        ...
        # Check return type
        assert isinstance(
            gate_value, (int, float, torch.Tensor)
        ...
        if isinstance(gate_value, torch.Tensor):
            assert gate_value.numel() == 1, "get_gate_value() should return scalar tensor"
    ...
    def test_gate_param_exists_as_nn_parameter(self):
        ...
        assert hasattr(model, "gate_param"), "gate_param attribute does not exist"

        # Check it's an nn.Parameter
        assert isinstance(
            model.gate_param, torch.nn.Parameter
        ), f"gate_param should be nn.Parameter, got {type(model.gate_param)}"

        # Check it requires grad
        assert model.gate_param.requires_grad, "gate_param should require gradients"

        # Check shape (should be scalar)
        assert (
            model.gate_param.numel() == 1
        ), f"gate_param should be scalar, got shape {model.gate_param.shape}"

        # Check it's in model parameters
        param_names = [name for name, _ in model.named_parameters()]
        assert "gate_param" in param_names, "gate_param not in model.named_parameters()"
    ...
    def test_model_works_with_nnmil_trainer_interface(self):
        ...
        assert logits.shape == (batch_size, 2), "Standard forward pass failed"

        # Pattern 2: Get branch outputs for loss computation
        logits_a, logits_b, logits_fused = model.get_branch_outputs(features)
        assert logits_a.shape == (batch_size, 2), "Branch A output shape incorrect"
        assert logits_b.shape == (batch_size, 2), "Branch B output shape incorrect"
        assert logits_fused.shape == (batch_size, 2), "Fused output shape incorrect"

        # Pattern 3: Get gate value for logging
        gate_value = model.get_gate_value()
        assert isinstance(gate_value, (int, float, torch.Tensor)), "get_gate_value() failed"

        # Pattern 4: Training mode
        model.train()
        logits_train = model(features)
        assert logits_train.shape == (batch_size, 2), "Training mode forward pass failed"
        ...
        assert model.proj_a[0].weight.grad is not None, "proj_a gradient not computed"
        assert model.proj_b[0].weight.grad is not None, "proj_b gradient not computed"
        assert (
            model.fusion_classifier[0].weight.grad is not None
        ...
        assert logits_eval.shape == (batch_size, 2), "Eval mode forward pass failed"
    ...
    def test_model_state_dict_compatible(self):
        ...
        # Check it's a dict
        assert isinstance(state_dict, dict), "state_dict() should return dict"
        ...
        for key in key_components:
            assert key in state_dict, f"Key component '{key}' not in state_dict"
        ...
        assert logits.shape == (4, 2), "Loaded model forward pass failed"
    ...
    def test_model_to_device_works(self):
        ...
        if torch.cuda.is_available():
            ...
            assert logits_cuda.device.type == "cuda", "Output not on CUDA"
    ...
    def test_model_train_eval_modes_work(self):
        ...
        # Test train mode
        model.train()
        assert model.training, "model.train() did not set training=True"

        # Test eval mode
        model.eval()
        assert not model.training, "model.eval() did not set training=False"
        ...
        model.train()
        logits_train = model(features)
        assert logits_train.shape == (4, 2), "Forward pass in train mode failed"

        model.eval()
        logits_eval = model(features)
        assert logits_eval.shape == (4, 2), "Forward pass in eval mode failed"
    ...
    def test_model_parameters_accessible(self):
        ...
        assert len(params) > 0, "model.parameters() returned empty list"
        ...
        assert "gate_param" in named_params, "gate_param not in named_parameters"
        assert any(
            "proj_a" in name for name in named_params
        ), "proj_a parameters not in named_parameters"
        assert any(
            "proj_b" in name for name in named_params
        ), "proj_b parameters not in named_parameters"
        assert any(
            "fusion_classifier" in name for name in named_params
    ...
    def test_model_repr_works(self):
        ...
        assert len(repr_str) > 0, "repr() returned empty string"

        # Check it contains model name
        assert "TransnnMIL" in repr_str, "repr() should contain model name"
    ...
    **Validates: Requirements 10.7**
    ...
    def test_num_patches_variations(self):
        ...
        for num_patches in num_patches_configs:
            ...
            assert not torch.isnan(
                logits
            ).any(), f"NaN values for num_patches={num_patches.tolist()}"
    ...
    def test_3d_input_validation(self):
        ...
        assert logits.shape == (4, 2), "3D input should work"
...
class TestDeviceCompatibility:
    ...
    **Validates: Requirements 7.2**
    ...
    def test_model_works_on_cpu(self):
        ...
        assert logits.device.type == "cpu", f"Output should be on CPU, got {logits.device}"
    ...
    def test_model_works_on_cuda(self):
        ...
        assert logits.device.type == "cuda", f"Output should be on CUDA, got {logits.device}"
    ...
    def test_model_handles_device_transfers(self):
        ...
        assert model.gate_param.device.type == "cpu", "gate_param not on CPU"
        assert model.proj_a[0].weight.device.type == "cpu", "proj_a not on CPU"
        assert model.proj_b[0].weight.device.type == "cpu", "proj_b not on CPU"
        assert (
            model.fusion_classifier[0].weight.device.type == "cpu"
        ...
        if torch.cuda.is_available():
            ...
            assert model.gate_param.device.type == "cuda", "gate_param not on CUDA"
            assert model.proj_a[0].weight.device.type == "cuda", "proj_a not on CUDA"
            assert model.proj_b[0].weight.device.type == "cuda", "proj_b not on CUDA"
            assert (
                model.fusion_classifier[0].weight.device.type == "cuda"
            ...
            assert model.gate_param.device.type == "cpu", "gate_param not back on CPU"
            assert model.proj_a[0].weight.device.type == "cpu", "proj_a not back on CPU"
            assert model.proj_b[0].weight.device.type == "cpu", "proj_b not back on CPU"
            assert (
                model.fusion_classifier[0].weight.device.type == "cpu"
    ...
    def test_all_components_move_to_same_device(self):
        ...
        assert "cpu" in devices, "Not all parameters on CPU"
        ...
        if torch.cuda.is_available():
            ...
            assert len(devices) == 1, f"Parameters on multiple devices: {devices}"
            assert "cuda" in devices, "Not all parameters on CUDA"
    ...
    def test_forward_pass_produces_same_results_on_cpu_and_cuda(self):
        ...
        assert torch.allclose(
            logits_cpu, logits_cuda_cpu, rtol=1e-4, atol=1e-5
    ...
    def test_get_branch_outputs_device_consistency(self):
        ...
        assert logits_a.device.type == "cpu", "logits_a not on CPU"
        assert logits_b.device.type == "cpu", "logits_b not on CPU"
        assert logits_fused.device.type == "cpu", "logits_fused not on CPU"
        ...
        if torch.cuda.is_available():
            ...
            assert logits_a.device.type == "cuda", "logits_a not on CUDA"
            assert logits_b.device.type == "cuda", "logits_b not on CUDA"
            assert logits_fused.device.type == "cuda", "logits_fused not on CUDA"
    ...
    def test_attention_weights_device_consistency(self):
        ...
        assert logits.device.type == "cpu", "logits not on CPU"
        assert attention.device.type == "cpu", "attention not on CPU"
        ...
        if torch.cuda.is_available():
            ...
            assert attention.device.type == "cuda", "attention not on CUDA"
    ...
    def test_gradient_computation_on_different_devices(self):
        ...
        assert features_cpu.grad is not None, "No gradient on CPU"
        assert features_cpu.grad.device.type == "cpu", "Gradient not on CPU"

        # Test on CUDA if available
        if torch.cuda.is_available():
            model_cuda = model.to("cuda")
            features_cuda = torch.randn(
                batch_size, num_patches, 1024, device="cuda", requires_grad=True
            )
            ...
            assert features_cuda.grad is not None, "No gradient on CUDA"
            assert features_cuda.grad.device.type == "cuda", "Gradient not on CUDA"
