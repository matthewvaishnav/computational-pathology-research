        ...
        **Validates: Requirements 1.3**
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if node.name in ["TestPhikonEncoder", "TestLoadFoundationModel", "TestFoundationModelIntegration"]:
    ...
    def test_foundation_model_tests_collected_with_not_property_marker(self):
        ...
        **Validates: Requirements 1.1**
        ...
        result = subprocess.run(
            ["pytest", "tests/test_foundation_models.py", "--collect-only", "-m", "not property", "-q"],
            capture_output=True,
            text=True,
            timeout=30
        )
