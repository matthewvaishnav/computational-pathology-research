...
class TestAzureHealthDataServices:
    ...
    @patch('src.cloud.azure.health_data_services.DefaultAzureCredential')
    @patch('src.cloud.azure.health_data_services.requests')
    def test_create_diagnostic_report(self, mock_requests, mock_credential, health_config, diagnostic_report_data):
...
class TestAzureBlobStorageConnector:
    ...
    @patch('builtins.open')
    def test_upload_file(self, mock_open, mock_getsize, mock_exists, mock_blob_client, mock_credential, blob_config):
...
class TestAzureFunctionsIntegration:
    ...
    def test_initialization(self, mock_web_client, mock_credential, function_config):
        ...
        assert integration.function_app_url == f"https://{function_config.function_app_name}.azurewebsites.net"

    @patch('src.cloud.azure.functions.AZURE_AVAILABLE', True)
    @patch('src.cloud.azure.functions.DefaultAzureCredential')
    @patch('src.cloud.azure.functions.WebSiteManagementClient')
    @patch('src.cloud.azure.functions.requests')
    def test_invoke_function(self, mock_requests, mock_web_client, mock_credential, function_config):
...
class TestAzureMonitorIntegration:
    ...
    def test_initialization(self, mock_log_client, mock_monitor_client, mock_credential, monitor_config):
    ...
    @patch('src.cloud.azure.monitor.MonitorManagementClient')
    @patch('src.cloud.azure.monitor.LogAnalyticsManagementClient')
    def test_send_custom_metric(self, mock_log_client, mock_monitor_client, mock_credential, monitor_config, custom_metric):
    ...
    @patch('src.cloud.azure.monitor.MonitorManagementClient')
    @patch('src.cloud.azure.monitor.LogAnalyticsManagementClient')
    def test_track_slide_processing(self, mock_log_client, mock_monitor_client, mock_credential, monitor_config):
        ...
        integration = AzureMonitorIntegration(monitor_config)
        integration.track_slide_processing(
            slide_id="slide-123",
    ...
    @patch('src.cloud.azure.monitor.MonitorManagementClient')
    @patch('src.cloud.azure.monitor.LogAnalyticsManagementClient')
    def test_health_check(self, mock_log_client, mock_monitor_client, mock_credential, monitor_config):
