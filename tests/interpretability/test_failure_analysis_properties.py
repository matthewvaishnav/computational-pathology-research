...
def predictions_and_labels(draw, min_samples=1, max_samples=100):
    ...
    predictions = draw(npst.arrays(
        dtype=np.int64,
        shape=n_samples,
        elements=st.integers(min_value=0, max_value=n_classes-1)
    ))
    
    ground_truth = draw(npst.arrays(
        dtype=np.int64,
        shape=n_samples,
        elements=st.integers(min_value=0, max_value=n_classes-1)
    ))
    
    confidence_scores = draw(npst.arrays(
        dtype=np.float32,
        shape=n_samples,
...
def test_property_10_failure_identification_with_confidence(data):
    ...
    if len(failures) > 0:
        ...
        for _, row in failures.iterrows():
            assert row['prediction'] != row['ground_truth'], \
...
def test_property_11_failure_clustering_completeness(data):
    ...
    for method in ['kmeans', 'dbscan', 'hierarchical']:
        ...
        assert clustered['cluster_id'].dtype in [np.int32, np.int64, int], \
        ...
        for cluster_id, stats in cluster_stats.items():
            ...
            assert 'representative_samples' in stats, f"Cluster {cluster_id} must have 'representative_samples'"
            
            # Verify count matches actual cluster size
            actual_count = (clustered['cluster_id'] == cluster_id).sum()
            assert stats['count'] == actual_count, \
...
def test_property_13_systematic_bias_analysis_completeness(data, n_subgroups):
    ...
    assert len(bias_metrics) == n_subgroups, \
    ...
    for subgroup_name in clinical_subgroups.keys():
        assert subgroup_name in bias_metrics, \
            f"Bias metric missing for subgroup '{subgroup_name}'"
...
def test_property_12_failure_csv_export_completeness(data):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        required_columns = ['slide_id', 'prediction', 'ground_truth', 'confidence', 'cluster_assignment']
