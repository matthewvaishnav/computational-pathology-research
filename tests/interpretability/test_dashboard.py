...
from src.features.interpretability.gradcam.dashboard import (
    InMemoryCache,
...
class TestInterpretabilityDashboard:
    ...
    def test_filter_samples_route(self):
        ...
        filter_data = {"min_confidence": 0.7, "max_confidence": 0.95, "correctness": True}

        response = client.post(
            "/api/filter", data=json.dumps(filter_data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_compare_samples_route(self):
        ...
        response = client.post(
            "/api/compare", data=json.dumps(compare_data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_compare_samples_max_limit(self):
        ...
        compare_data = {"sample_ids": ["s1", "s2", "s3", "s4", "s5"]}  # 5 samples
    ...
    def test_export_route(self):
        ...
        export_data = {"sample_id": "test_sample", "format": "png", "dpi": 300}

        response = client.post(
            "/api/export", data=json.dumps(export_data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_export_invalid_format(self):
        ...
        export_data = {"sample_id": "test_sample", "format": "invalid_format"}
    ...
    def test_filter_samples(self):
        ...
        filtered = dashboard.filter_samples(
            min_confidence=0.8, max_confidence=0.95, correctness=True
        )
    ...
    def test_compare_samples(self):
        ...
        comparison = dashboard.compare_samples(
            sample_ids=["s1", "s2"], comparison_type="side_by_side"
        )
    ...
    def test_compare_samples_validates_count(self):
        ...
        with pytest.raises(ValueError, match="Maximum 4 samples"):
            dashboard.compare_samples(sample_ids=["s1", "s2", "s3", "s4", "s5"])
    ...
    def test_compare_samples_validates_type(self):
        ...
        with pytest.raises(ValueError, match="Invalid comparison_type"):
            dashboard.compare_samples(sample_ids=["s1", "s2"], comparison_type="invalid_type")
    ...
    def test_export_visualization(self):
        ...
        output_path = dashboard.export_visualization(
            sample_id="test_sample", output_format="png", dpi=300
        )
    ...
    def test_export_validates_format(self):
        ...
        with pytest.raises(ValueError, match="Invalid format"):
            dashboard.export_visualization(sample_id="test_sample", output_format="invalid")
