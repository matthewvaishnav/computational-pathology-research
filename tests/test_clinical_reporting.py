...
from src.features.clinical.workflow.reporting import (
...
class TestReportExport:
    ...
    def test_export_json(self, sample_report_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (
                content["primary_diagnosis"]["disease_name"]
                == sample_report_data.primary_diagnosis.disease_name
            )
            assert len(content["alternative_diagnoses"]) == len(
                sample_report_data.alternative_diagnoses
            )
    ...
    def test_export_dicom_sr_includes_findings(self, sample_report_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            findings = [
                item for item in dicom_sr["ContentSequence"] if item.get("ValueType") == "CODE"
            ]
