    ...
    **Validates: Requirements 10.2, 10.6**
    ...
    with torch.no_grad():
        ...
        if return_attention:
            ...
            assert isinstance(transmil_output, tuple), "TransMIL should return tuple when return_attention=True"
            assert isinstance(nnmil_output, tuple), "nnMIL should return tuple when return_attention=True"
            ...
            assert isinstance(transmil_logits, torch.Tensor), "TransMIL should return tensor when return_attention=False"
            assert isinstance(nnmil_logits, torch.Tensor), "nnMIL should return tensor when return_attention=False"
    
    # Verify output shapes are compatible
    expected_logits_shape = (batch_size, num_classes)
    assert transmil_logits.shape == expected_logits_shape, (
        f"TransMIL logits shape mismatch: expected {expected_logits_shape}, got {transmil_logits.shape}"
    )
    assert nnmil_logits.shape == expected_logits_shape, (
    ...
    if return_attention:
        ...
        assert transmil_attention.shape == expected_attention_shape, (
            f"TransMIL attention shape mismatch: expected {expected_attention_shape}, got {transmil_attention.shape}"
        )
        assert nnmil_attention.shape == expected_attention_shape, (
...
def test_property_34_weight_transfer(feature_dim, num_classes, hidden_dim):
    ...
    **Validates: Requirements 10.4**
    ...
    assert abs(transmil_mean - nnmil_mean) < 5.0, (
...
def test_checkpoint_migration_end_to_end():
    ...
    try:
        ...
        nnmil_checkpoint_path = migrator.migrate_checkpoint(
            checkpoint_path, 
            output_path=checkpoint_path.replace('.pth', '_nnmil.pth')
        )
...
def test_performance_regression_check():
    ...
    assert nnmil_time < transmil_time * 2.0, (
...
def test_gradient_compatibility():
    ...
    nnmil_has_gradients = any(
        param.grad is not None and param.grad.abs().sum() > 0
        for param in nnmil_model.parameters()
    )
