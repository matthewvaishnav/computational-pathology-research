...
class TestActiveLearningConnector:
    ...
    def mock_al_system(self):
        ...
        al_system.get_statistics = Mock(return_value={
            'cases_identified': 10,
...
class TestPACSConnector:
    ...
    def mock_pacs_adapter(self):
        ...
        adapter.retrieve_study = Mock(return_value=OperationResult.success_result(
            operation_id="retrieve_1",
            message="Success"
        ))
        adapter.query_studies = Mock(return_value=([], OperationResult.success_result(
...
class TestClinicalWorkflowIntegrator:
    ...
    def mock_pacs_adapter(self):
        """Create mock PACS adapter."""
        adapter = Mock(spec=PACSAdapter)
        adapter.test_connection = Mock(return_value=OperationResult.success_result(
            operation_id="test_1",
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_workflow(self, tmp_path):
        ...
        mock_al_system.get_statistics = Mock(return_value={
            'annotations_received': 10
        })
        mock_al_system.min_annotations_for_retraining = 50
        
        mock_pacs_adapter = Mock(spec=PACSAdapter)
        mock_pacs_adapter.retrieve_study = Mock(return_value=OperationResult.success_result(
            operation_id="retrieve_1",
            message="Success"
        ))
        mock_pacs_adapter.test_connection = Mock(return_value=OperationResult.success_result(
            operation_id="test_1",
            message="Success"
        ))
