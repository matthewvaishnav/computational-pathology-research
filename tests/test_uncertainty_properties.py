...
def test_property_20_uncertainty_output_shape(batch_size, num_windows, num_classes, task_type):
    ...
    **Validates: Requirements 6.3**
    ...
    assert uncertainties['total'].shape == (batch_size,), (
        f"Expected total uncertainty shape ({batch_size},), "
        f"got {uncertainties['total'].shape}"
    )
...
def test_property_21_dropout_activation_for_uncertainty():
    ...
    **Validates: Requirements 6.1, 6.4**
    ...
    assert torch.all(uncertainties['epistemic'] >= 0), "Epistemic uncertainty should be non-negative"
...
def test_property_22_uncertainty_normalization(batch_size, num_windows, num_classes, task_type):
    ...
    **Validates: Requirements 6.5**
    ...
    assert torch.all(epistemic <= 1), f"Epistemic uncertainty should be <= 1, got max {epistemic.max()}"
    ...
    assert torch.all(aleatoric <= 1), f"Aleatoric uncertainty should be <= 1, got max {aleatoric.max()}"
...
def test_survival_uncertainty():
    ...
    expected_total = torch.sqrt(
        uncertainties['epistemic']**2 + uncertainties['aleatoric']**2
    )
