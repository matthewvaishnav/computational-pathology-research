...
def task_specification_strategy(draw):
    ...
    return TaskSpecification(
        ...
        model_architecture=draw(
            st.sampled_from(["resnet18_transformer", "resnet50_transformer", "vit"])
        ),
...
def test_configuration_equivalence_random_seeds(task_spec):
    ...
    assert reference_seed == task_spec.random_seed, (
        f"Random seed {reference_seed} does not match task spec seed " f"{task_spec.random_seed}"
    )
...
def test_configuration_equivalence_validation(task_spec):
    ...
    assert report.is_equivalent, (
        f"Configurations should be equivalent, but found differences: " f"{report.differences}"
    )
    ...
    assert all(
        seed == seed_values[0] for seed in seed_values
    ...
    assert all(
        split == split_values[0] for split in split_values
...
@given(
    task_spec=task_specification_strategy(), modified_seed=st.integers(min_value=0, max_value=10000)
)
def test_configuration_equivalence_detects_seed_mismatch(task_spec, modified_seed):
    ...
    assert not report.is_equivalent, "Should detect random seed mismatch"

    # Should have a difference about random seeds
    seed_diff_found = any("seed" in diff.lower() for diff in report.differences)
    assert seed_diff_found, f"Should report random seed difference, but got: {report.differences}"


# Feature: competitor-benchmark-system, Property 1: Configuration Equivalence Across Frameworks
@settings(max_examples=50)
@given(
    task_spec=task_specification_strategy(), modified_lr=st.floats(min_value=1e-6, max_value=1e-2)
)
def test_configuration_equivalence_detects_hyperparameter_mismatch(task_spec, modified_lr):
    ...
    # Should detect the mismatch
    assert not report.is_equivalent, "Should detect learning rate mismatch"

    # Should have a difference about learning rate
    lr_diff_found = any("learning rate" in diff.lower() for diff in report.differences)
    assert lr_diff_found, f"Should report learning rate difference, but got: {report.differences}"
