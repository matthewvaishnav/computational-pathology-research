...
def checkpoint_manager(checkpoint_dir: Path) -> CheckpointManager:
    ...
    return CheckpointManager(checkpoint_dir=checkpoint_dir, checkpoint_interval_minutes=30)
...
class TestCheckpointManagerInitialization:
    ...
    def test_initialization_with_invalid_interval(self, checkpoint_dir: Path):
        ...
        with pytest.raises(ValueError, match="checkpoint_interval_minutes must be positive"):
            CheckpointManager(checkpoint_dir=checkpoint_dir, checkpoint_interval_minutes=0)

        with pytest.raises(ValueError, match="checkpoint_interval_minutes must be positive"):
            CheckpointManager(checkpoint_dir=checkpoint_dir, checkpoint_interval_minutes=-10)
...
class TestCheckpointSaveLoad:
    ...
    def test_save_checkpoint_creates_file(
        self, checkpoint_manager: CheckpointManager, sample_benchmark_state: Dict[str, Any]
    ):
        """Test that save_checkpoint creates checkpoint file."""
        checkpoint_path = checkpoint_manager.save_checkpoint(sample_benchmark_state, force=True)
    ...
    def test_save_checkpoint_creates_latest_link(
        ...
        checkpoint_manager.save_checkpoint(sample_benchmark_state, force=True)

        latest_path = checkpoint_manager.checkpoint_dir / "checkpoint_latest.json"
        assert latest_path.exists()

    def test_save_checkpoint_with_missing_fields(self, checkpoint_manager: CheckpointManager):
    ...
    def test_load_checkpoint_nonexistent_file(
        self, checkpoint_manager: CheckpointManager, checkpoint_dir: Path
    ):
    ...
    def test_checkpoint_roundtrip_preserves_data(
        ...
        **Validates: Requirement 5.3**
        ...
        assert (
            restored_state["completed_frameworks"] == sample_benchmark_state["completed_frameworks"]
        )
...
class TestCheckpointIntervalEnforcement:
    ...
    def test_checkpoint_interval_enforcement(
        ...
        **Validates: Requirement 5.3**
        ...
        checkpoint_path1 = checkpoint_manager.save_checkpoint(sample_benchmark_state, force=True)
        assert checkpoint_path1 is not None

        # Immediate second checkpoint should not save (interval not elapsed)
        checkpoint_path2 = checkpoint_manager.save_checkpoint(sample_benchmark_state, force=False)
    ...
    def test_force_checkpoint_bypasses_interval(
        ...
        checkpoint_path2 = checkpoint_manager.save_checkpoint(sample_benchmark_state, force=True)
        assert checkpoint_path2 is not None
        assert checkpoint_path1 != checkpoint_path2

    def test_checkpoint_saves_after_interval(
        self, checkpoint_dir: Path, sample_benchmark_state: Dict[str, Any]
    ):
        """Test that checkpoint saves after interval elapses."""
        # Use very short interval for testing (1 second)
        manager = CheckpointManager(
            checkpoint_dir=checkpoint_dir, checkpoint_interval_minutes=1 / 60  # 1 second
        )

        # Save first checkpoint
        checkpoint_path1 = manager.save_checkpoint(sample_benchmark_state, force=True)
        assert checkpoint_path1 is not None
        ...
        checkpoint_path2 = manager.save_checkpoint(sample_benchmark_state, force=False)
        assert checkpoint_path2 is not None
        assert checkpoint_path1 != checkpoint_path2
...
class TestCrashRecovery:
    ...
    def test_resume_with_no_checkpoint_raises_error(self, checkpoint_manager: CheckpointManager):
    ...
    def test_crash_recovery_simulation(
        ...
        **Validates: Requirement 5.4**
        ...
        checkpoint_path = manager1.save_checkpoint(sample_benchmark_state, force=True)
        assert checkpoint_path is not None
...
class TestCheckpointSerialization:
    ...
    def test_serialize_datetime(self, checkpoint_manager: CheckpointManager):
    ...
    def test_serialize_path(self, checkpoint_manager: CheckpointManager):
