...
class TestVersionTracker:
    ...
    def test_generate_requirements_txt_filtered(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result_path = tracker.generate_requirements_txt(
                output_path,
    ...
    def test_validate_config_strict_mode(self):
        ...
        env_info = EnvironmentInfo(
            ...
            cudnn_version="8.6.0",
            gpu_model=torch.cuda.get_device_name(0) if torch.cuda.is_available() else "not_available",
            gpu_driver_version="535.54.03",
            gpu_memory_mb=12288.0,
            gpu_count=1,
            os_name=platform.system(),
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid_strict, warnings_strict = tracker.validate_config(
                config_path,
                strict=True
            )
            
            # Validate in non-strict mode
            is_valid_normal, warnings_normal = tracker.validate_config(
                config_path,
                strict=False
            )
