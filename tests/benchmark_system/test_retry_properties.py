    ...
    **Validates: Requirement 8.2**
    ...
    def test_backoff_respects_max_delay(self, base_delay, max_delay, retry_count):
        ...
        assert delay <= max_delay, (
            f"Delay should be capped at max_delay: " f"expected <= {max_delay:.6f}, got {delay:.6f}"
        )
    ...
    @settings(max_examples=100)
    def test_backoff_increases_monotonically(self, base_delay, retry_count_1, retry_count_2):
        """
    ...
    def test_retry_with_backoff_respects_max_retries(self, base_delay, max_retries):
        ...
        assert result.attempts == max_retries + 1, (
            f"Should attempt {max_retries + 1} times, " f"but attempted {result.attempts} times"
        )
    ...
    def test_retry_with_backoff_succeeds_on_nth_attempt(
        self, base_delay, max_retries, success_on_attempt
    ):
        """
        ...
        assert result.attempts == success_on_attempt, (
            f"Should report {success_on_attempt} attempts, " f"but reported {result.attempts}"
        )
    ...
    def test_retry_with_backoff_accumulates_delay(self, base_delay, max_retries):
        ...
        expected_total_delay = sum(handler.calculate_backoff_delay(i) for i in range(max_retries))
