...
class TestConfigurationDifferenceLogging:
    ...
    def test_logs_configuration_differences(self, executor, caplog):
        ...
        assert any(
            "Configuration differences detected" in record.message for record in caplog.records
        )
        assert any("learning rate" in record.message.lower() for record in caplog.records)
    ...
    def test_logs_success_for_equivalent_configs(self, executor, sample_task_spec, caplog):
        ...
        assert any(
            "equivalent" in record.message.lower() and "fair comparison" in record.message.lower()
