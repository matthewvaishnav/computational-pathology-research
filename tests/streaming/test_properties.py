...
class TestMemoryUsageProperty:
    ...
    @given(tile_size=tile_sizes(), max_memory_gb=memory_budgets())
    @settings(
    ...
    @given(dimensions=slide_dimensions(), tile_size=tile_sizes())
    ...
    @given(initial_batch=batch_sizes(), memory_pressure=st.floats(min_value=0.0, max_value=1.0))
    ...
    def test_batch_optimizer_respects_bounds(self, initial_batch, memory_pressure):
        ...
        optimizer = BatchSizeOptimizer(
            initial_batch_size=initial_batch, min_batch_size=min_batch, max_batch_size=max_batch
        )
...
class TestAttentionWeightNormalization:
    ...
    @given(num_patches=st.integers(min_value=10, max_value=500), feature_dim=feature_dims())
    @settings(
...
class TestConfidenceMonotonicity:
    ...
    @given(num_patches=st.integers(min_value=50, max_value=200))
    @settings(
        max_examples=20, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
...
class TestProcessingTimeBounds:
    ...
    @given(batch_size=batch_sizes(), num_batches=st.integers(min_value=1, max_value=10))
    @settings(
        max_examples=20, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    ...
    @given(initial_batch=st.integers(min_value=4, max_value=64))
...
class TestNumericalStability:
    ...
    @given(num_patches=st.integers(min_value=10, max_value=100))
    @settings(
        max_examples=20, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
