...
from src.streaming.gpu_pipeline import GPUPipeline, GPUMemoryManager, BatchSizeOptimizer
...
class TestResourceConstraints:
    ...
    @given(
        memory_limit=memory_constraints(),
...
class TestErrorRecovery:
    ...
    def test_aggregator_handles_nan_features(self, num_patches):
        ...
        try:
            ...
            assert ('nan' in error_msg or 'finite' in error_msg or 
                    'confidence' in error_msg or 'between' in error_msg)
    ...
    def test_aggregator_handles_inf_features(self, num_patches):
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert ('inf' in error_msg or 'finite' in error_msg or 
                    'confidence' in error_msg or 'between' in error_msg)
    ...
    @given(
        num_updates=st.integers(min_value=5, max_value=15)
    )
