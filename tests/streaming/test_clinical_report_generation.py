...
def report_generator():
    ...
    return StreamingReportGenerator(
        institution_name="Demo Hospital", institution_logo=None, template_config=None, language="en"
    )
...
class TestPDFReportGeneration:
    ...
    def test_generate_basic_pdf_report(
        self, report_generator, sample_processing_result, sample_patient_metadata, tmp_path
    ):
        ...
        report_data = StreamingReportData(
            ...
            throughput=sample_processing_result.patches_processed
            / sample_processing_result.processing_time,
            memory_usage_gb=1.5,
        )

        # Generate report
        result_path = report_generator.generate_pdf_report(
            report_data=report_data, output_path=output_path
        )
        ...
        print(
            f"Diagnosis: {'Positive' if sample_processing_result.prediction == 1 else 'Negative'}"
        )
    ...
    def test_pdf_report_with_attention_heatmap(
        ...
        print(
            f"✓ Heatmap shows {len(sample_processing_result.attention_weights)} attention weights"
        )

    def test_pdf_report_with_institutional_branding(
        self, sample_processing_result, sample_patient_metadata, tmp_path
    ):
        ...
        branded_generator = StreamingReportGenerator(
            ...
            template_config=ReportTemplate(primary_color="#003366"),
            language="en",
        )
    ...
    def test_pdf_report_multi_language_support(
        ...
        for lang in languages:
            ...
            lang_generator = StreamingReportGenerator(
                institution_name="Demo Hospital", language=lang
            )
            ...
            report_data = StreamingReportData(
                ...
                patient_id=sample_patient_metadata.patient_id,
                prediction_class=(
                    "Positive" if sample_processing_result.prediction == 1 else "Negative"
                ),
                confidence=sample_processing_result.confidence,
                processing_time=sample_processing_result.processing_time,
                patches_processed=sample_processing_result.patches_processed,
...
class TestFHIRDiagnosticReport:
    ...
    def test_generate_fhir_diagnostic_report(
        self, sample_processing_result, sample_patient_metadata
    ):
        ...
        fhir_report = {
            ...
            "subject": {"reference": f"Patient/{sample_patient_metadata.patient_id}"},
            "effectiveDateTime": datetime.fromtimestamp(
                sample_processing_result.timestamp
            ).isoformat(),
                    ...
                    "coding": [
                        {
                            ...
                            "code": (
                                "10828004"
                                if sample_processing_result.prediction == 1
                                else "260385009"
                            ),
                            "display": (
                                "Positive"
    ...
    def test_fhir_observation_resource(self, sample_processing_result):
                ...
                "coding": [
                    {
                        "system": "http://snomed.info/sct",
                        "code": (
                            "10828004" if sample_processing_result.prediction == 1 else "260385009"
                        ),
                        "display": (
                            "Positive" if sample_processing_result.prediction == 1 else "Negative"
                        ),
...
class TestMultiFormatExport:
    ...
    def test_export_to_xml(self, sample_processing_result, sample_patient_metadata, tmp_path):
...
<ClinicalReport>
    ...
    def test_export_to_hl7(self, sample_processing_result, sample_patient_metadata):
...
class TestRegulatoryCompliance:
    ...
    def test_fda_510k_documentation(self, sample_processing_result):
...
class TestReportValidation:
    ...
    def test_report_accuracy_validation(self, sample_processing_result):
