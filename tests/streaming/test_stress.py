...
from src.streaming.memory_optimizer import MemoryOptimizer, MemoryConfig
...
class TestHighConcurrentLoad:
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_50_concurrent_slides(
        self,
        mock_cnn_model,
        ...
        avg_processing_time = np.mean([r['processing_time'] for r in successful]) if successful else 0
    ...
    @pytest.mark.slow
    def test_concurrent_load_with_queue(
        self,
        mock_cnn_model,
        mock_attention_model,
        gpu_available
...
class TestMemoryPressure:
    ...
    def test_memory_pressure_recovery(
        ...
        # Assertions
        assert success_rate >= 0.90, f"Success rate {success_rate:.1%} too low under memory pressure"
    ...
    @pytest.mark.slow
    def test_batch_size_adaptation_under_pressure(
        self,
        mock_cnn_model,
        gpu_available
    ):
        ...
        logger.info(f"Batch size adaptation: avg={avg_batch_size:.1f} min={min_used} max={max_used}")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        # Exponential backoff
                        delay = 0.5 * (2 ** attempt)
                        logger.warning(f"Call {call_id}: Retry {attempt+1}/{max_retries} after {delay}s")
        ...
        while time.time() - start_time < duration_seconds:
            try:
                ...
                if batches_processed % 100 == 0:
                    ...
                    logger.info(f"Sustained load: {batches_processed} batches, {throughput:.1f} batches/sec")
