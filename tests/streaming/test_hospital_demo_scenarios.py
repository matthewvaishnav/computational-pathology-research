...
class TestLivePACSIntegrationDemo:
    ...
    def test_worklist_management(self, demo_slides):
        ...
        for entry in worklist:
            print(
                f"{entry.priority:<10} {entry.patient_id:<15} {entry.study_uid:<20} {entry.status:<10}"
            )
    ...
    async def test_end_to_end_pacs_workflow_demo(
        self, demo_slides, mock_cnn_encoder, mock_attention_model
    ):
        ...
        for i in range(5):
            ...
            print(
                f"[{time.time() - start_time:.1f}s] Processed {patches_processed}/{stat_case.patches} patches | Confidence: {confidence:.3f}"
            )
    ...
    async def test_multi_slide_triage_demo(
        ...
        for idx, slide in enumerate(sorted_slides):
            ...
            result = {
                ...
                "flag_urgent": slide.priority in ["STAT", "URGENT"]
...
class TestMemoryConstrainedDemo:
    ...
    @pytest.mark.asyncio
    async def test_laptop_deployment_demo(self, mock_cnn_encoder, mock_attention_model):
        ...
        try:
            ...
            for i in range(5):
                ...
                print(
                    f"[{patches_processed}/150 patches] Memory: {current_mem:.3f} GB / 2.0 GB ({current_mem/2.0*100:.1f}%)"
                )
            ...
            assert (
                analytics.peak_usage_gb <= 2.0
            ), f"Memory exceeded limit: {analytics.peak_usage_gb:.3f} GB"
...
class TestProgressiveConfidenceDemo:
    ...
    @pytest.mark.asyncio
    async def test_progressive_confidence_demo(self, mock_cnn_encoder, mock_attention_model):
...
class TestBenchmarkComparisonDemo:
    ...
    def test_streaming_vs_batch_comparison(self):
        ...
        # Processing time
        time_improvement = (
            (batch_metrics["processing_time"] - streaming_metrics["processing_time"])
            / batch_metrics["processing_time"]
            * 100
        )
        print(
            f"{'Processing Time':<30} {batch_metrics['processing_time']:.1f}s{'':<9} {streaming_metrics['processing_time']:.1f}s{'':<9} {time_improvement:.1f}% faster"
        )

        # Memory
        memory_reduction = (
            (batch_metrics["peak_memory_gb"] - streaming_metrics["peak_memory_gb"])
            / batch_metrics["peak_memory_gb"]
            * 100
        )
        print(
            f"{'Peak Memory':<30} {batch_metrics['peak_memory_gb']:.1f} GB{'':<8} {streaming_metrics['peak_memory_gb']:.1f} GB{'':<8} {memory_reduction:.1f}% less"
        )

        # Accuracy
        print(
            f"{'Accuracy':<30} {batch_metrics['accuracy']:.1%}{'':<10} {streaming_metrics['accuracy']:.1%}{'':<10} Equivalent"
        )
