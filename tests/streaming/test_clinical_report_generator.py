...
from src.streaming.clinical_report_generator import StreamingReportData, StreamingReportGenerator
...
def sample_report_data(temp_output_dir):
    ...
    heatmap_img = Image.fromarray((np.random.rand(400, 400, 3) * 255).astype(np.uint8))
    heatmap_img.save(heatmap_path)

    conf_img = Image.fromarray((np.random.rand(300, 400, 3) * 255).astype(np.uint8))
    ...
    return StreamingReportData(
        ...
        recommendations=["Recommend further immunohistochemistry", "Consider molecular profiling"],
    )
...
def report_generator(temp_output_dir):
    ...
    return StreamingReportGenerator(institution_name="Test Medical Center", institution_logo=None)
...
class TestStreamingReportGenerator:
    ...
    def test_initialization(self, temp_output_dir):
        ...
        generator = StreamingReportGenerator(
            institution_name="Test Hospital", institution_logo=None
        )
    ...
    def test_initialization_with_logo(self, temp_output_dir):
        ...
        logo_img = Image.fromarray((np.random.rand(100, 200, 3) * 255).astype(np.uint8))
        logo_img.save(logo_path)

        generator = StreamingReportGenerator(
            institution_name="Test Hospital", institution_logo=logo_path
        )
...
class TestPDFReportGeneration:
    ...
    def test_generate_pdf_basic(self, report_generator, sample_report_data, temp_output_dir):
        ...
        result_path = report_generator.generate_pdf_report(sample_report_data, output_path)
    ...
    def test_generate_pdf_minimal_data(self, report_generator, temp_output_dir):
        ...
        minimal_data = StreamingReportData(
            slide_id="MIN-001", prediction_class="Normal", confidence=0.75
        )

        output_path = temp_output_dir / "minimal_report.pdf"
        result_path = report_generator.generate_pdf_report(minimal_data, output_path)
    ...
    def test_generate_pdf_high_confidence(self, report_generator, temp_output_dir):
        ...
        data = StreamingReportData(
            slide_id="HIGH-CONF-001", prediction_class="Tumor", confidence=0.95
        )
    ...
    def test_generate_pdf_medium_confidence(self, report_generator, temp_output_dir):
        ...
        data = StreamingReportData(
            slide_id="MED-CONF-001", prediction_class="Tumor", confidence=0.80
        )
    ...
    def test_generate_pdf_low_confidence(self, report_generator, temp_output_dir):
        ...
        data = StreamingReportData(
            slide_id="LOW-CONF-001", prediction_class="Tumor", confidence=0.65
        )
    ...
    def test_generate_pdf_with_visualizations(
        self, report_generator, sample_report_data, temp_output_dir
    ):
    ...
    def test_generate_pdf_creates_parent_dirs(
        self, report_generator, sample_report_data, temp_output_dir
    ):
...
class TestHTMLReportGeneration:
    ...
    def test_generate_html_basic(self, report_generator, sample_report_data, temp_output_dir):
        ...
        result_path = report_generator.generate_html_report(sample_report_data, output_path)

        assert result_path.exists()
        assert result_path.suffix == ".html"
        assert result_path.stat().st_size > 0

    def test_generate_html_content_structure(
        self, report_generator, sample_report_data, temp_output_dir
    ):
    ...
    def test_generate_html_confidence_colors(self, report_generator, temp_output_dir):
        ...
        high_data = StreamingReportData(
            slide_id="HIGH-001", prediction_class="Tumor", confidence=0.95
        )
        ...
        med_data = StreamingReportData(
            slide_id="MED-001", prediction_class="Tumor", confidence=0.80
        )
        ...
        low_data = StreamingReportData(
            slide_id="LOW-001", prediction_class="Tumor", confidence=0.65
        )
    ...
    def test_generate_html_minimal_data(self, report_generator, temp_output_dir):
        ...
        minimal_data = StreamingReportData(
            slide_id="MIN-HTML-001", prediction_class="Normal", confidence=0.70
        )

        output_path = temp_output_dir / "minimal_html_report.html"
        result_path = report_generator.generate_html_report(minimal_data, output_path)

        assert result_path.exists()
        assert result_path.stat().st_size > 0

    def test_generate_html_creates_parent_dirs(
        self, report_generator, sample_report_data, temp_output_dir
    ):
...
class TestInstitutionalBranding:
    ...
    def test_custom_institution_name_pdf(self, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Johns Hopkins Medical Center")

        data = StreamingReportData(slide_id="BRAND-001", prediction_class="Tumor", confidence=0.88)
    ...
    def test_custom_institution_name_html(self, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Mayo Clinic")

        data = StreamingReportData(slide_id="BRAND-002", prediction_class="Normal", confidence=0.92)
    ...
    def test_institution_logo_pdf(self, temp_output_dir):
        ...
        logo_img = Image.fromarray((np.random.rand(75, 200, 3) * 255).astype(np.uint8))
        logo_img.save(logo_path)
        ...
        data = StreamingReportData(slide_id="LOGO-001", prediction_class="Tumor", confidence=0.85)
...
class TestReportDataValidation:
    ...
    def test_confidence_bounds(self, report_generator, temp_output_dir):
        ...
        data_zero = StreamingReportData(
            slide_id="CONF-ZERO", prediction_class="Unknown", confidence=0.0
        )
        ...
        data_one = StreamingReportData(
            slide_id="CONF-ONE", prediction_class="Tumor", confidence=1.0
        )
    ...
    def test_empty_recommendations_list(self, report_generator, temp_output_dir):
        ...
        data = StreamingReportData(
            slide_id="EMPTY-REC-001", prediction_class="Normal", confidence=0.95, recommendations=[]
        )
...
class TestReportTemplateCustomization:
    ...
    def test_custom_template_page_size_a4(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(
            institution_name="Test Hospital", template_config=template
        )
...
class TestMultiLanguageSupport:
    ...
    def test_english_report(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Test Hospital", language="en")
    ...
    def test_spanish_report(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Hospital de Prueba", language="es")
    ...
    def test_french_report(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Hôpital de Test", language="fr")
    ...
    def test_german_report(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(institution_name="Test Krankenhaus", language="de")
    ...
    def test_unsupported_language_fallback(self, sample_report_data, temp_output_dir):
        ...
        generator = StreamingReportGenerator(
            institution_name="Test Hospital", language="zh"  # Chinese not supported
        )
