...
from src.features.clinical.workflow.pacs.data_models import OperationResult, StudyInfo
...
class TestNetworkResilience:
    ...
    def test_query_with_retry_failure(self, temp_cache_dir, mock_pacs_adapter):
        ...
        client = PACSWSIStreamingClient(cache_dir=str(temp_cache_dir), max_retries=2)
    ...
    def test_connection_test_with_retry(self, temp_cache_dir, mock_pacs_adapter):
        ...
        client = PACSWSIStreamingClient(cache_dir=str(temp_cache_dir), max_retries=3)
...
class TestDownloadProgress:
    ...
    def test_progress_calculation(self):
        ...
        progress = DownloadProgress(study_uid="1.2.3.4", total_bytes=1000, downloaded_bytes=250)

        assert progress.progress_percent == 25.0

    def test_progress_zero_total(self):
        """Test progress with zero total bytes."""
        progress = DownloadProgress(study_uid="1.2.3.4", total_bytes=0, downloaded_bytes=0)

        assert progress.progress_percent == 0.0

    def test_is_complete(self):
        """Test completion check."""
        progress = DownloadProgress(study_uid="1.2.3.4", total_bytes=1000, downloaded_bytes=1000)

        assert progress.is_complete is True

    def test_is_not_complete(self):
        """Test incomplete check."""
        progress = DownloadProgress(study_uid="1.2.3.4", total_bytes=1000, downloaded_bytes=500)
