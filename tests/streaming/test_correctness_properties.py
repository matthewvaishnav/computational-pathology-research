...
class TestSpatialCoverageCompleteness:
    ...
    def test_tile_batch_coordinates_unique(self, tile_size, num_tiles):
        ...
        batch = TileBatch(tiles=tiles, coordinates=coords, batch_id=0, total_batches=1)
    ...
    @given(num_tiles=st.integers(min_value=10, max_value=100))
    @settings(max_examples=20, deadline=None)
    ...
    @given(num_patches=st.integers(min_value=20, max_value=100))
    @settings(
...
class TestAccuracyMaintenance:
    ...
    @given(num_patches=st.integers(min_value=20, max_value=80))
    @settings(
