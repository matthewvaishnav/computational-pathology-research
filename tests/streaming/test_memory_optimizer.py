...
def gc_collector(device):
    ...
    return SmartGarbageCollector(
        device=device, memory_pressure_threshold=0.8, collection_interval_seconds=1.0
    )
...
class TestSmartGarbageCollector:
    ...
    def test_initialization(self, device):
        ...
        gc = SmartGarbageCollector(
            device=device, memory_pressure_threshold=0.8, collection_interval_seconds=10.0
        )
    ...
    def test_should_collect_pressure(self, gc_collector):
        ...
        assert not gc_collector.should_collect(current_memory_gb=2.0, total_memory_gb=8.0)

        # High pressure - should collect
        assert gc_collector.should_collect(current_memory_gb=7.0, total_memory_gb=8.0)
    ...
    def test_should_collect_interval(self, gc_collector):
        ...
        result1 = gc_collector.should_collect(current_memory_gb=7.0, total_memory_gb=8.0)

        if result1:
            gc_collector.last_collection_time = time.time()

        # Immediate second check - should not collect (too soon)
        result2 = gc_collector.should_collect(current_memory_gb=7.0, total_memory_gb=8.0)
...
class TestMemoryOptimizerIntegration:
    ...
    def test_pool_with_gc(self, device):
        ...
        should_collect = gc.should_collect(
            current_memory_gb=stats.allocated_size_gb, total_memory_gb=pool.max_pool_size_gb
        )
