...
class PCamSyntheticGenerator(DatasetGenerator):
    ...
    def corrupt_samples(self, samples: Dict[str, Any], corruption_type: str) -> Dict[str, Any]:
        ...
        if corruption_type == "image_noise":
            ...
            wrong_shape_indices = self.rng.choice(
                len(images), size=max(1, len(images) // 10), replace=False
            )
            ...
            missing_indices = self.rng.choice(
                len(images), size=max(1, len(images) // 20), replace=False
            )
    ...
    def create_dataset_splits(
        self, samples: Dict[str, Any], split_ratios: Dict[str, float] = None
