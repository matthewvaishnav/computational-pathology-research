...
class CAMELYONSyntheticGenerator(DatasetGenerator):
        ...
        for patient_id, num_patient_slides in spec.patient_slide_distribution.items():
            for slide_idx in range(num_patient_slides):
                slide_data = self._generate_slide(
                    slide_id=f"slide_{slide_id:05d}",
                ...
                slide_index.append({
                    ...
                    "num_patches": int(slide_data["num_patches"]),  # Convert numpy int to Python int
    ...
    def _generate_slide(self, slide_id: str, patient_id: str, spec: CAMELYONSyntheticSpec) -> Dict[str, Any]:
    ...
    def _generate_coordinates(
        ...
        if pattern["spatial_clustering"] > 0.5:
            ...
            for i in range(num_clusters):
                ...
                cluster_coords = self.rng.normal(
                    cluster_centers[i], 500, (cluster_size, 2)
                )
            ...
            coordinates = self.rng.uniform(
                min_coord, max_coord, (num_patches, 2)
            )
    ...
    def _generate_attention_weights(self, num_patches: int, pattern: Dict[str, Any]) -> np.ndarray:
        ...
        if pattern["attention_sparsity"] > 0.5:
            ...
            high_attention_indices = self.rng.choice(
                num_patches, 
                size=max(1, int(num_patches * 0.1)), 
                replace=False
            )
    ...
    def validate_samples(self, samples: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for i, slide in enumerate(slides):
                ...
                required_fields = ["slide_id", "patient_id", "label", "num_patches", 
                                 "features", "coordinates", "attention_weights"]
                ...
                if not isinstance(attention_weights, np.ndarray) or attention_weights.dtype != np.float32:
                    return False
    ...
    def _save_dataset(self, dataset: Dict[str, Any], output_dir: Path):
        ...
        for slide in dataset["slides"]:
            ...
            with h5py.File(slide_path, "w") as f:
                ...
                f.create_dataset("attention_weights", data=slide["attention_weights"], compression="gzip")
    ...
    def create_patient_splits(
        ...
        for split_name, split_patients in patient_splits.items():
            split_slides = [
                slide for slide in dataset["slides"] 
                if slide["patient_id"] in split_patients
            ]
            
            split_index = [
                entry for entry in dataset["slide_index"]
                if entry["patient_id"] in split_patients
            ]
