    ...
    def test_patch_extraction_coordinate_consistency(
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        try:
            ...
            for patch, (x, y) in patches:
                assert x >= 0 and y >= 0, \
                    f"Patch coordinates ({x}, {y}) are negative"
                assert x + patch_size <= slide_width, \
                    f"Patch at ({x}, {y}) extends beyond slide width"
                assert y + patch_size <= slide_height, \
                    f"Patch at ({y}, {y}) extends beyond slide height"
            
            # Property 3: Coordinates should match read_region calls
            assert len(patches) == len(read_region_calls), \
                "Number of patches should match number of read_region calls"
            
            for (patch, (patch_x, patch_y)), (call_location, call_level, call_size) in zip(patches, read_region_calls):
                assert call_location == (patch_x, patch_y), \
                    f"Coordinate mismatch: patch ({patch_x}, {patch_y}) vs call {call_location}"
                assert call_level == level, \
                    f"Level mismatch: expected {level}, got {call_level}"
                assert call_size == (patch_size, patch_size), \
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        try:
            ...
            for patch, (x, y) in patches:
                ...
                assert x >= 0 and y >= 0, \
                    f"Level 0 coordinates ({x}, {y}) should be non-negative"
                
                # Level 0 coordinates should be within slide bounds
                assert x + patch_size * downsample <= slide_width, \
                    f"Level 0 coordinates ({x}, {y}) extend beyond slide width"
                assert y + patch_size * downsample <= slide_height, \
            ...
            for (patch, (patch_x, patch_y)), (call_location, call_level, call_size) in zip(patches, read_region_calls):
                ...
                assert level_x + patch_size <= level_width, \
                    f"Level {level} coordinates ({level_x}, {level_y}) extend beyond level bounds"
                assert level_y + patch_size <= level_height, \
                    f"Level {level} coordinates ({level_x}, {level_y}) extend beyond level bounds"
