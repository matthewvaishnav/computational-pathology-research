                ...
                if slide_data is not None:
                    ...
                    for patch_idx in range(min(5, num_patches)):  # Test first 5 patches
                        ...
                        for i in range(len(patch_dataset)):
                            ...
                            if (sample["slide_id"] == slide_id and 
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for slide_idx in range(len(slide_dataset)):
                ...
                if patch_slide_data is not None:
                    ...
                    assert slide_sample["coordinates"].shape == patch_slide_data["coordinates"].shape
                    
                    # Property: Feature and coordinate values should be identical
                    assert torch.allclose(slide_sample["features"], patch_slide_data["features"], atol=1e-5)
                    assert torch.allclose(slide_sample["coordinates"], patch_slide_data["coordinates"])
        ...
        **Validates: Requirements 2.5**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if total_patients >= 3:
                ...
                assert len(val_patients) >= 1 or total_patients < 5  # Allow empty val for very small datasets
                assert len(test_patients) >= 1 or total_patients < 4  # Allow empty test for small datasets
                ...
                for patient_id, slide_ids in patient_slides.items():
                    ...
                    for other_split_name, other_split_data in splits.items():
                        if other_split_name != split_name:
                            other_patient_ids = [s["patient_id"] for s in other_split_data["slides"]]
                            assert patient_id not in other_patient_ids, (
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            dataset = generator.generate_samples(
                num_slides=num_slides,
                spec=spec,
                output_dir=Path(tmp_dir) / "batch_test"
            )
...
class TestCAMELYONErrorDetection:
    ...
    @given(st.sampled_from(["feature_corruption", "coordinate_mismatch", "missing_patches", "invalid_labels"]))
    @settings(max_examples=20, deadline=60000)
        ...
        **Validates: Requirements 2.7**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            dataset = generator.generate_samples(
                num_slides=3,
                spec=spec,
                output_dir=Path(tmp_dir) / "clean_data"
            )
            ...
            for slide_entry in corrupted_dataset["slide_index"]:
                ...
                if feature_path.exists():
                    ...
                    if not result["valid"]:
                        ...
                        actionable_keywords = [
                            "missing", "mismatch", "invalid", "corrupt", 
                            "expected", "found", "shape", "dimension"
                        ]
        ...
        **Validates: Requirements 2.7**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            dataset = generator.generate_samples(
                num_slides=5,
                spec=spec,
                output_dir=Path(tmp_dir) / "test_data"
            )
