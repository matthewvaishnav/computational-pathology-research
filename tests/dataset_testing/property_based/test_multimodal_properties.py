        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(len(dataset)):
                ...
                assert patient_id not in patient_ids, \
                    f"Duplicate patient ID found: {patient_id}"
                ...
                if sample['wsi_features'] is not None:
                    ...
                    assert sample['wsi_features'].shape[1] == wsi_dim, \
                ...
                if sample['genomic'] is not None:
                    ...
                    assert sample['genomic'].dtype == torch.float32, \
                        "Genomic features should be float32"
                    assert sample['genomic'].shape[0] == genomic_dim, \
                ...
                if sample['clinical_text'] is not None:
                    ...
                    assert sample['clinical_text'].dtype == torch.long, \
            ...
            if len(dataset) >= 2:
                ...
                assert len(collated_batch['patient_ids']) == batch_size, \
                    f"Collated batch should have {batch_size} patients"
                
                # Batch should preserve label information
                assert collated_batch['label'].shape[0] == batch_size, \
                ...
                for i, patient_id in enumerate(collated_batch['patient_ids']):
                    ...
                    if 'wsi_features' in collated_batch and collated_batch['wsi_features'] is not None:
                        if original_sample['has_wsi']:
                            ...
                            assert not torch.all(wsi_sample == 0), \
                    ...
                    if 'genomic' in collated_batch and collated_batch['genomic'] is not None:
                        if original_sample['has_genomic']:
                            ...
                            assert not torch.all(genomic_sample == 0), \
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for batch_size in batch_sizes:
                ...
                for i in range(batch_size):
                    ...
                    modalities_present = []
                    if (collated_batch.get('wsi_features') is not None and 
                        not torch.all(collated_batch['wsi_features'][i] == 0)):
                        modalities_present.append('wsi')
                    if (collated_batch.get('genomic') is not None and 
                        not torch.all(collated_batch['genomic'][i] == 0)):
                        modalities_present.append('genomic')
                    if (collated_batch.get('clinical_text') is not None and 
                        not torch.all(collated_batch['clinical_text'][i] == 0)):
                    ...
                    if len(modalities_present) > 1:
                        ...
                        assert patient_id == original_sample['patient_id'], \
                            f"Cross-modal alignment broken for patient {patient_id}"

    @given(
        config=multimodal_config_strategy(),
        num_patients=st.integers(min_value=3, max_value=10)
    )
    @settings(max_examples=100, deadline=60000)
        ...
        **Validates: Requirements 3.5, 3.7**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(total_samples):
                ...
                if not config.get('wsi_enabled', True):
                    assert sample['wsi_features'] is None, \
                        "Disabled WSI modality should be None"
                if not config.get('genomic_enabled', True):
                    assert sample['genomic'] is None, \
                        "Disabled genomic modality should be None"
            ...
            if total_samples >= 2:
                ...
                assert len(collated_batch['patient_ids']) == len(batch), \
                    "Batch size should be preserved in collation"
                assert collated_batch['label'].shape[0] == len(batch), \
                ...
                for modality in ['wsi_features', 'genomic', 'clinical_text']:
                    if modality in collated_batch:
                        if collated_batch[modality] is not None:
                            assert collated_batch[modality].shape[0] == len(batch), \
