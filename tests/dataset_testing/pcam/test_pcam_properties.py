    ...
    **Validates: Requirements 1.1, 1.2**
    ...
    def test_property_1_dataset_format_validation(self, num_samples, noise_level, label_dist_p0):
        ...
        **Validates: Requirements 1.1, 1.2**
        ...
        assert np.all(images >= 0) and np.all(images <= 255), \
        ...
        if num_samples >= 10:
            ...
            if len(unique_labels) == 2:  # Both labels present
                ...
                tolerance = max(0.3, 3.0 / np.sqrt(num_samples))  # Larger tolerance for small samples
                
                assert abs(actual_p0 - expected_p0) <= tolerance, \
    ...
    def test_property_1_validation_method_correctness(self, num_samples, corruption_rate):
        ...
        **Validates: Requirements 1.1, 1.2**
        ...
        assert self.generator.validate_samples(valid_samples), \
            "Valid samples must pass validation"
        ...
        if corruption_rate > 0:
            ...
            for corruption_type in corruption_types:
                ...
                assert isinstance(validation_result, bool), f"Validation should return boolean for {corruption_type}"
...
class TestPCamTransformConsistencyProperties(PropertyTestBase):
    ...
    **Validates: Requirements 1.3, 1.4**
    ...
    def test_property_3_transform_consistency(self, sample):
        ...
        **Validates: Requirements 1.3, 1.4**
        ...
        np.testing.assert_array_equal(norm1, norm2, 
            "Normalization transform should be deterministic")
        
        # Test brightness consistency
        bright1 = brightness_transform(image)
        bright2 = brightness_transform(image)
        
        np.testing.assert_array_equal(bright1, bright2,
            "Brightness transform should be deterministic")
        
        # Property: Transform should preserve image structure
        assert norm1.shape == image.shape, \
            "Normalization should preserve image shape"
        
        assert bright1.shape == image.shape, \
            "Brightness transform should preserve image shape"
    ...
    @settings(max_examples=50, deadline=60000)
    def test_property_3_parameterized_transform_consistency(self, sample, brightness_factor, contrast_factor):
        """
        ...
        **Validates: Requirements 1.3, 1.4**
        ...
        np.testing.assert_array_equal(result1, result2,
            "Parameterized transform should be deterministic")
        
        # Property: Transform should preserve shape and type
        assert result1.shape == image.shape, \
            "Transform should preserve image shape"
        
        assert result1.dtype == np.uint8, \
            "Transform should preserve uint8 type"
        
        # Property: Output should be in valid range
        assert np.all(result1 >= 0) and np.all(result1 <= 255), \
    ...
    def test_property_3_transform_chain_consistency(self, sample):
        ...
        **Validates: Requirements 1.3, 1.4**
        ...
        np.testing.assert_array_equal(result1, result2,
            "Transform chain should be deterministic")
        
        # Property: Chain should preserve essential properties
        assert result1.shape == image.shape, \
            "Transform chain should preserve shape"
        
        assert result1.dtype == np.uint8, \
            "Transform chain should preserve dtype"
        ...
        assert output_mean >= input_mean * 0.95, \
    ...
    def test_property_3_random_transform_seeded_consistency(self, sample, seed):
        ...
        **Validates: Requirements 1.3, 1.4**
        ...
        np.testing.assert_array_equal(result1, result2,
            "Seeded random transform should be reproducible")
        
        # Property: Transform should preserve essential properties
        assert result1.shape == image.shape, \
            "Random transform should preserve shape"
        
        assert result1.dtype == image.dtype, \
            "Random transform should preserve dtype"
...
class TestPCamDataIntegrityPreservationProperties(PropertyTestBase):
    ...
    **Validates: Requirements 1.5**
    ...
    @given(
        num_samples=st.integers(min_value=5, max_value=30)
    )
    @settings(max_examples=50, deadline=60000)
    def test_property_2_data_integrity_preservation(self, num_samples):
        ...
        **Validates: Requirements 1.5**
        ...
        np.testing.assert_array_equal(loaded_images, original_samples["images"],
            "Loading should preserve image data exactly")
        
        np.testing.assert_array_equal(loaded_labels, original_samples["labels"],
            "Loading should preserve label data exactly")
        ...
        np.testing.assert_array_equal(original_label_counts, loaded_label_counts,
            "Loading should preserve label distribution")
    ...
    def test_property_2_preprocessing_integrity_preservation(self, sample):
        ...
        **Validates: Requirements 1.5**
        ...
        np.testing.assert_array_equal(result1, image,
            "Normalize-denormalize should preserve image exactly")
        ...
        assert result1.shape == image.shape, \
            "Reversible operation should preserve shape"
        
        assert result1.dtype == image.dtype, \
            "Reversible operation should preserve dtype"
