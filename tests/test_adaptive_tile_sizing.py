...
from src.data.wsi_pipeline.wsi_stream_reader import WSIStreamReader, StreamingMetadata
...
class TestTileBufferConfigValidation:
    ...
    def test_tile_size_out_of_bounds(self):
        ...
        with pytest.raises(ValueError, match="tile_size .* must be between min_tile_size .* and max_tile_size"):
            config.validate()
    ...
    def test_invalid_memory_pressure_reduction(self):
        ...
        with pytest.raises(ValueError, match="memory_pressure_tile_reduction must be between 0.1 and 1.0"):
            config.validate()
...
class TestWSIStreamReaderAdaptiveSizing:
    ...
    def mock_wsi_reader(self):
        ...
        mock_reader.read_region.return_value = np.random.randint(0, 255, (1024, 1024, 3), dtype=np.uint8)
        return mock_reader
    ...
    def test_initialization_with_adaptive_sizing(self, config, mock_wsi_reader):
        ...
        with patch('src.data.wsi_pipeline.wsi_stream_reader.WSIReader', return_value=mock_wsi_reader):
    ...
    def test_memory_budget_exceeded_raises_error(self, config, mock_wsi_reader):
        ...
        with patch('src.data.wsi_pipeline.wsi_stream_reader.WSIReader', return_value=mock_wsi_reader):
            ...
            with pytest.raises(ResourceError, match="Estimated memory usage .* exceeds maximum allowed"):
