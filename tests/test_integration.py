...
def temp_workspace():
    ...
    for split in ['train', 'val', 'test']:
        ...
        for i in range(5):
            clinical_file = workspace / "data" / "clinical_text" / f"{split}_patient_{i}_clinical.npy"
...
def test_batch_processing(temp_workspace):
    ...
    for batch_size in batch_sizes:
        ...
        assert embeddings.shape == (batch_size, 128), f"Embedding shape incorrect for batch size {batch_size}"
        assert logits.shape == (batch_size, 4), f"Logits shape incorrect for batch size {batch_size}"
