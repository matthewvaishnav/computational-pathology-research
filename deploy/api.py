...
class PredictionRequest(BaseModel):
    ...
    wsi_features: Optional[List[List[float]]] = Field(None, description="WSI patch features [num_patches, 1024]")
    genomic: Optional[List[float]] = Field(None, description="Genomic features [2000]")
    clinical_text: Optional[List[int]] = Field(None, description="Tokenized clinical text [seq_len]")
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        if request.wsi_features:
            wsi_tensor = torch.tensor(request.wsi_features, dtype=torch.float32).unsqueeze(0)  # [1, num_patches, 1024]
        ...
        if request.genomic:
            ...
            genomic_tensor = torch.tensor(request.genomic, dtype=torch.float32).unsqueeze(0)  # [1, 2000]
        ...
        if request.clinical_text:
            clinical_tensor = torch.tensor(request.clinical_text, dtype=torch.long).unsqueeze(0)  # [1, seq_len]
            batch['clinical_text'] = clinical_tensor.to(DEVICE)
            batch['clinical_mask'] = torch.ones(1, clinical_tensor.shape[1], dtype=torch.bool).to(DEVICE)
