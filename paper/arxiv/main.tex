...
\input{study_specific_packages.tex}
...
Pair-integrity falsification further separated scanner suppression from tissue preservation. Broken-pair controls reduced scanner-probe accuracy similarly or more than true same-region pairs, but degraded paired-tissue consistency and same-region retrieval (Tables~\ref{tab:pair_integrity} and~\ref{tab:crossbackbone_pair_integrity}). This indicates that the useful scanner-suppression/tissue-preservation tradeoff depends on true same-tissue pairing, rather than scanner suppression alone.

\begin{table}[t]
\centering
\caption{SCORPION DINOv2 pair-integrity falsification across five folds and five seeds. Lower scanner probe is not sufficient: higher cosine and retrieval indicate retained tissue identity.}
\scriptsize
...
\begin{table*}[t]
\centering
\caption{SCORPION cross-backbone pair-integrity falsification. Lower scanner probe alone is not sufficient; higher cosine and retrieval indicate retained tissue identity.}
\scriptsize
...
\bottomrule
\end{tabular}
\label{tab:crossbackbone_pair_integrity}
\end{table*}
...
External pair-integrity falsification repeated the same control on canine SCC. True same-region pairs preserved tissue identity better than both shuffled controls (Table~\ref{tab:canine_pair_integrity}). Region-shuffled pairs reduced scanner-probe accuracy more than true pairs, but degraded paired cosine and same-region retrieval. This separates scanner suppression from useful tissue preservation outside the primary SCORPION benchmark.

\begin{table}[t]
\centering
\caption{External canine SCC DINOv2 pair-integrity falsification across five sample-blocked folds and five seeds. Lower scanner probe alone is not sufficient; higher cosine and retrieval indicate retained tissue identity.}
\scriptsize
...
\bottomrule
\end{tabular}
\label{tab:canine_pair_integrity}
\end{table}

\input{paired_acquisition_figure1_benchmark_table.tex}

\subsection{Simple scanner-removal baseline controls}
...
\begin{table}[t]
\centering
\caption{Scanner-removal baseline controls. Simple post-hoc operations on frozen DINOv2 features cannot match the scanner-suppression/tissue-preservation tradeoff of Paired-Acquisition Neural Factorization on either dataset.}
\scriptsize
...
SCORPION & Best linear ($k{\ge}4$) & 0.724 & 0.881 & 0.850 & 0.9999 \\
...
Canine SCC & Best linear ($k{\ge}4$) & 0.707 & 0.739 & 0.665 & 0.8738 \\
Canine SCC & Best PCA ($k{=}32$) & 0.641 & 0.598 & 0.481 & 0.9347 \\
Canine SCC & Original frozen & 0.863 & 0.919 & 0.891 & 0.8329 \\
\bottomrule
\end{tabular}
\label{tab:baseline_murder}
\end{table}

Linear scanner-subspace projection preserved tissue structure (cosine 0.881 SCORPION, 0.739 canine) but saturated at the learned scanner-subspace rank ($k{\ge}4$), leaving scanner probe at 0.724 and 0.707---far above the neural references of 0.399 and 0.361. PCA component removal suppressed scanner further (0.560 SCORPION, 0.641 canine) but caused substantial tissue damage: cosine fell from 0.987 to 0.806 on SCORPION and from 0.919 to 0.598 on canine SCC. No simple baseline came within 0.03 scanner probe of neural factorization on either dataset.
...
\begin{figure*}[t]
\centering
\resizebox{0.78\textwidth}{!}{\input{paired_acquisition_resource_allocation_figure.tex}}
...
The whole-slide studies include a historical pre-repair execution record alongside current baselines. Mean-pooled Phikon plus an MLP reached validation QWK 0.7274, and gated AttentionMIL reached 0.8100. Under the historical TransnnMIL execution path, three listed tuned runs reached 0.8155, 0.8225, and 0.8086, while an 18-run grid across six learning rates and three seeds reached a best mean validation QWK of $0.8257\pm0.0169$ at learning rate $10^{-4}$.

The TransnnMIL values above are retained as historical prediction records, not current evidence for genuine TransMIL--nnMIL branch fusion, topology, or the repaired canonical implementation. The historical branch-attention path and topology path were later found to contain mechanism defects documented in the repository claim boundary. Current architectural conclusions therefore require the preregistered repaired matched rerun against AttentionMIL, TransMIL, nnMIL, concat, gate, and learned branch-attention controls.
...
\section{Discussion}
The main result is not merely that a scanner classifier became worse. Scanner information remained strongly available in the acquisition branch, tissue identity remained almost saturated in biology, and the frozen objective transferred without retuning across a general self-supervised encoder, a pathology-native encoder, and a conventional ImageNet encoder. Cross-backbone pair-integrity falsification further showed that broken-pair controls can suppress scanner probes while damaging tissue preservation on Phikon and ResNet50. Simple scanner-removal baselines---linear scanner-subspace projection and PCA component removal on frozen features---could not reproduce the neural tradeoff on SCORPION or external canine SCC. This pattern is difficult to explain as one-backbone overfitting, wholesale representational collapse, or scanner suppression alone.
