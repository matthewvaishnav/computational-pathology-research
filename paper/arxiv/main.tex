...
\begin{enumerate}[leftmargin=*]
  \item \textbf{A sample-volume / site-signal alignment failure-mode framing.} The paper reframes dominant-client failure not as institutional reliability ranking, but as an audit of a modeling assumption that FedAvg already makes silently: larger client equals more influence.
  \item \textbf{A simulated federated pathology stress test over real pathology-derived features.} The experiments use PANDA-derived Phikon feature vectors and perturb the dominant simulated site's training signal while keeping validation labels clean.
  \item \textbf{Two stress modes.} The study examines dominant-site label corruption and systematic ordinal threshold shift. The latter is intended to approximate a more pathology-plausible failure mode than purely random label corruption.
  \item \textbf{A fixed detector-switch transfer result.} A detector rule calibrated under label-noise stress transfers to conservative ordinal threshold shift, with low clean-regime switching and positive 35\% / 45\% shift gains.
  \item \textbf{Diagnostic and calibration robustness checks.} The detector result is supported by diagnostic-frequency analysis, leave-one-diagnostic-family-out ablation, and calibration-sensitivity analysis over nearby detector settings.
\end{enumerate}
...
The experiments use PANDA-derived Phikon feature representations and simulate multi-site federated learning over pathology-derived feature vectors. PANDA is a prostate cancer grading dataset \citep{bulten2022artificial}. Phikon is a pathology foundation model used here as the feature extractor \citep{filiot2023scaling}.
...
The conservative threshold-shift result is the strongest transfer setting. It is treated as the headline systematic-bias result in this paper. Aggressive threshold shift is weaker and is presented as a supplementary asymmetric-stress result rather than the main claim.
...
Let $A_k$ denote the task-specific alignment of client $k$'s training signal with the declared validation objective. The paper's claim is not that $A_k$ is an institutional property. It is a run-specific modeling quantity. The failure mode appears when the dominant sample-volume client has high $n_k$ but reduced $A_k$.
...
A fixed label-noise-calibrated detector rule was evaluated on conservative threshold-shift stress. Table~\ref{tab:transfer} summarizes the result.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/dominant-site-figure-3-detector-transfer.png}
...
This asymmetry is important: the detector should not be presented as a universal stress detector. The conservative threshold-shift result is the headline; aggressive threshold shift is a supplementary weak-transfer result that strengthens the paper by showing where the detector is less responsive.
