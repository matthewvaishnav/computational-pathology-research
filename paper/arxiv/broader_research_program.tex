...
The detailed SCORPION paired-scanner study above is the newest and strongest locked PathoAlign result, but it is not the whole research record. Earlier public PDF versions treated the repository as a three-level platform---representation, whole-slide aggregation, and institutional aggregation---and reported several distinct studies inside those levels. This section restores that study-by-study structure and adds the newer evidence without using one experiment to overstate another.
...
\midrule
Adversarial PathoAlign confirmation & Pathology-derived representations & Does lower site leakage improve tumor prediction? \\
...
Matched-budget anchor allocation & Controlled nonlinear high-overlap regime & Under fixed paired exposure, how much does unique-anchor diversity matter relative to repetition? \\
...
\section{Earlier PathoAlign studies}

\subsection{Locked adversarial confirmation}
The first neural PathoAlign study used a shared feature encoder with a tumor head and a gradient-reversal site head. Development experiments suggested a small task improvement and reduced site leakage. A locked confirmation over new seeds did not reproduce the task advantage: the adversarial model reached approximately 0.9264 held-out tumor accuracy versus 0.9270 for the tumor-only baseline, a paired difference of approximately $-0.000625$ with an interval crossing zero. The site-probe difference was approximately $-0.003258$, with the paired bootstrap interval excluding zero.
...
\subsection{Matched-budget anchor diversity versus repetition}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Earlier PathoAlign} falsifies unconditional site removal and motivates paired supervision.
  \item \textbf{Synthetic PathoAlign} measures recovery when the ground-truth latent factors are known.
  \item \textbf{SCORPION PathoAlign} tests paired-acquisition separation on real human tissue across multiple representation families.
  \item \textbf{PANDA federated stress} audits sample-volume dominance under controlled label-process misalignment.
  \item \textbf{CAMELYON17} tests related aggregation hypotheses under natural held-out-center shift.
