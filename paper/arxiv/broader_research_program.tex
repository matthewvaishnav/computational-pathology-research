...
PANDA baseline hierarchy & 10,611 verified Phikon feature bags & How much do mean pooling and gated attention differ, and what historical predictions were produced by the pre-repair TransnnMIL path? \\
Historical TransnnMIL stability grid & PANDA feature bags & What optimizer behavior was recorded on the pre-repair execution path? \\
...
\subsection{Mean pooling, AttentionMIL, and historical TransnnMIL executions}
The PANDA work studies weakly supervised prostate-cancer grading from whole-slide Phikon feature bags on the PANDA challenge data \citep{bulten2022artificial}. After HDF5 readability verification, 10,611 slide-level feature files were retained. Mean-pooled Phikon MLP and gated AttentionMIL \citep{ilse2018attention} provide active baseline records. The listed TransnnMIL runs were produced on the historical pre-repair execution path; they are retained as prediction records but do not validate genuine repaired TransMIL-style \citep{shao2021transmil} / nnMIL-style \citep{luo2025nnmil} branch fusion or topology.

\begin{table}[!htbp]
\centering
\caption{PANDA whole-slide records. Mean-pooling and AttentionMIL are active baselines; TransnnMIL rows are historical pre-repair predictions. QWK denotes quadratic weighted kappa.}
\small
\begin{tabular}{lr}
\toprule
...
The active baseline hierarchy establishes that gated attention improved on simple mean pooling in this feature-bag setting. The historical TransnnMIL predictions were numerically above AttentionMIL in two of three listed runs, but because the old fusion/topology implementation was later remediated, those comparisons are not current architecture evidence.

\subsection{Historical pre-repair TransnnMIL optimization-stability grid}
The historical stabilization study used the same 10,611 readable PANDA bags, three seeds, and six learning rates. The recipe used AdamW, warmup-cosine scheduling, two warmup epochs, gradient clipping at norm 1.0, and early stopping. Because these runs predate the fusion/topology remediation, the table is retained as optimizer/execution history rather than evidence for the repaired canonical architecture.

\begin{table}[!htbp]
\centering
\caption{Historical pre-repair TransnnMIL optimizer grid across three seeds per learning rate.}
\small
\begin{tabular}{lrrrr}
\toprule
...
On the historical execution path, the recorded optimizer recipe reduced observed learning-rate sensitivity. This does not validate the repaired TransnnMIL fusion/topology mechanisms. Current architecture evidence is deferred to the preregistered repaired matched rerun against AttentionMIL, TransMIL, nnMIL, concat, gate, and learned branch-attention controls.
...
The conservative ordinal threshold-shift detector-transfer results are reported in Table~\ref{tab:main_detector_transfer}. We do not duplicate the table here. The key pattern is that clean-regime triggering remains low, while trigger rates increase under stronger conservative shift and the detector-transfer deltas become positive for global QWK, macro-F1, and worst-site QWK in shifted regimes.

At 25\%, 35\%, and 45\% conservative shift, the fixed detector increasingly switches away from FedAvg and yields positive deltas across the reported summary metrics, while the 0\% clean condition remains near neutral.
