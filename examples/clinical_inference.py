...
def main():
    ...
    classifier = MultiClassDiseaseClassifier(
        embedding_dim=embedding_dim, taxonomy=taxonomy, hidden_dim=512, dropout=0.3
    )
    ...
    uncertainty_quantifier = UncertaintyQuantifier(
        num_classes=taxonomy.get_num_classes(), calibration_method="temperature"
    )
    ...
    with torch.no_grad():
        ...
        print(
            f"  Primary diagnosis uncertainty: {uncertainty_scores[0, primary_diagnosis_idx].item():.3f}"
        )
    ...
    report_generator = ClinicalReportGenerator(template_dir="configs/clinical/templates")
    ...
    print(
        f"  Primary Diagnosis: {report_data['primary_diagnosis']} ({report_data['primary_probability']:.2%})"
    )
