...
from clinical.regulatory import RegulatoryComplianceManager, RegulatoryStandard, ValidationStatus
...
def main():
    ...
    print(
        f"✓ Created V&V plan with {len(verification_activities)} verification and {len(validation_activities)} validation activities"
    )
    ...
    vv_report = compliance_manager.vv_system.generate_vv_report(
        device_name="PathologyAI_Clinical", device_version="1.0.0"
    )
