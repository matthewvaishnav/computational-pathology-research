...
def example_reconstruction_only():
    ...
    class EncoderDecoder(nn.Module):
        def __init__(self):
            super().__init__()
            self.encoder = nn.Sequential(nn.Linear(1024, 512), nn.ReLU(), nn.Linear(512, 256))
            self.decoder = nn.Sequential(nn.Linear(256, 512), nn.ReLU(), nn.Linear(512, 1024))
...
def example_standalone_losses():
    ...
    # Reconstruction loss
    print("\nReconstruction Loss:")
    reconstruction_loss = MaskedPatchReconstruction(mask_ratio=0.75, mask_strategy="random")
