...
def main():
    ...
    print(
        f"  Added scan 1: {scan1.disease_state} (confidence: {scan1.disease_probabilities[scan1.disease_state]:.2%})"
    )
    ...
    print(
        f"  Added scan 2: {scan2.disease_state} (confidence: {scan2.disease_probabilities[scan2.disease_state]:.2%})"
    )
    ...
    print(
        f"  Added scan 3: {scan3.disease_state} (confidence: {scan3.disease_probabilities[scan3.disease_state]:.2%})"
    )
    ...
    for horizon in ["1_year", "5_year", "10_year"]:
        if horizon in risk_evolution:
            ...
            print(
                f"    {horizon}: {trend['initial']:.2%} → {trend['current']:.2%} (change: {trend['change']:.2%})"
            )
    ...
    print(
        f"  Disease progression: {scan1.disease_state} → {scan2.disease_state} → {scan3.disease_state}"
    )
