...
def demo_batch_inference(pipeline):
    ...
    for i, result in enumerate(results):
        logger.info(f"    Case {i+1}: {result['inference_time']:.3f}s, "
    ...
    try:
        ...
        for i, request_id in enumerate(request_ids):
            ...
            if result:
                ...
                logger.info(f"  Request {i+1}: {result.processing_time:.3f}s processing, "
        ...
        logger.info(f"  Meets latency target: {'✓' if current_perf['meets_latency_target'] else '✗'}")
