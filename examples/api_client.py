...
class PredictionResult:
    ...
    def __repr__(self):
        return (
            f"PredictionResult(class={self.predicted_class}, "
...
class PathologyAPIClient:
    ...
    def __init__(
        self, base_url: str, timeout: int = 30, max_retries: int = 3, retry_delay: float = 1.0
    ):
    ...
    def batch_predict(
        self, samples: List[Dict[str, Any]], batch_size: int = 32
    ...
    def predict_from_file(self, file_path: str, modality: str = "genomic") -> PredictionResult:
        """
...
class AsyncPathologyAPIClient:
    ...
    async def predict(
        ...
        if wsi_features is not None:
            data["wsi_features"] = (
                wsi_features.tolist() if isinstance(wsi_features, np.ndarray) else wsi_features
            )
        if genomic is not None:
            data["genomic"] = genomic.tolist() if isinstance(genomic, np.ndarray) else genomic
        if clinical_text is not None:
            data["clinical_text"] = (
                clinical_text.tolist() if isinstance(clinical_text, np.ndarray) else clinical_text
            )
...
def main():
    ...
    if args.test:
        ...
        result = client.predict(
            wsi_features=wsi_features, genomic=genomic, clinical_text=clinical_text
        )
        ...
        for i, result in enumerate(results):
            if isinstance(result, PredictionResult):
                print(
                    f"  Sample {i+1}: class={result.predicted_class}, confidence={result.confidence:.3f}"
                )
            else:
