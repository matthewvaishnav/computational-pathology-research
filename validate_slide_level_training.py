"""
Quick validation script to demonstrate slide-level training path works.

This script creates minimal synthetic data and runs one training step
to verify the slide-level training pipeline is functional.
"""
...
from src.data.camelyon_dataset import CAMELYONSlideIndex, CAMELYONSlideDataset, SlideMetadata, collate_slide_bags
...
def create_synthetic_slide_data(temp_dir: Path, num_slides: int = 4):
    ...
    for i in range(num_slides):
        ...
        # Create HDF5 feature file with variable number of patches
        num_patches = 10 + i * 5  # Variable: 10, 15, 20, 25 patches
        feature_dim = 128
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   ✓ Created {len(slide_index)} slides")
        ...
        print(f"   ✓ Dataset contains {len(dataset)} slides")
        ...
        # Test batching with collate function
        ...
        print(f"   ✓ Batch num_patches: {batch['num_patches']}")
        print(f"   ✓ Slide IDs: {batch['slide_ids']}")
